export const STORAGE_KEYS = {
  ...
  // Journal de douleurs/blessures (Point 5) : historique des gênes signalées par
  // l'athlète, exploité par lib/gemini.js pour adapter le plan (voir buildInjuryBlock)
  // et affiché/édité dans components/InjuryJournal.js.
  injuryLog: 'tri_injury_log',
  // Suivi cycle menstruel (Point 8, optionnel — jamais activé par défaut) : voir
  // components/CycleTracker.js. Contient uniquement des dates de règles déclarées et un
  // réglage de longueur de cycle moyenne — aucune autre donnée de santé.
  menstrualCycle: 'tri_menstrual_cycle',
  // Calendrier de courses multi-saisons (Point 7) : liste d'échéances {name, date,
  // priority: 'A'|'B'|'C', sportType, distances/format} — voir lib/periodization.js
  // (computeMultiRacePeriodization) et components/RaceCalendar.js. Remplace
  // progressivement l'usage d'un unique `constraints.targetDate` quand l'athlète a
  // renseigné plusieurs échéances ; reste vide (comportement mono-objectif inchangé)
  // tant que l'athlète n'ajoute rien ici.
  raceCalendar: 'tri_race_calendar',
};
...
// Distingue "rien encore enregistré sous cette clé" de "une valeur (même fausse/vide)
// y est stockée" — loadFromStorage ne permet pas cette distinction puisqu'il renvoie
// le fallback dans les deux cas. Utilisé par ZoneCharts.js pour savoir si les bornes
// FC/Puissance/Allure actuellement affichées viennent d'une validation manuelle déjà
// faite par l'athlète (raw présent) ou d'un calcul de repli encore jamais validé
// (raw absent) — donc si le libellé de source ("Manuel" / "Estimé Strava" / "Générique")
// doit encore être affiché ou non.
