...
export const SWIM_SPORTS = ['Swim', 'OpenWaterSwim'];
 ...
 */
// Bornes basses de Z1..Z5 en % de VMA. MAJ 09/2026 : l'endurance fondamentale (Z2) commence
// à 62 % VMA au lieu de 70 % — 70-80 % correspondait plutôt à une allure marathon pour un
// coureur entraîné, et poussait les footings "faciles" en zone grise. Mêmes valeurs que
// RUN_ZONE_PCTS dans lib/coachPrompts.js (prompt IA).
const PACE_PCTS = [0, 0.62, 0.75, 0.85, 0.92];
export { PACE_PCTS };
...
/**
 * Allure de seuil natation (CSS, "critical swim speed") estimée depuis la meilleure
 * séance nage continue réelle dans la fenêtre 18-70min — même protocole et même facteur
 * que estimateThresholdPowerFromActivities/estimateThresholdHrFromActivities ci-dessus,
 * exprimé en allure /100m (format "m:ss", même convention que profile.nat100 saisi à la
 * main) plutôt qu'en vitesse, pour rester directement comparable à ce que l'athlète
 * saisirait lui-même dans l'onglet Profil.
 */
export function estimateSwimPaceFromActivities(activities, sports = SWIM_SPORTS) {
  const efforts = qualifyingEfforts(activities, sports).filter(
    (a) => Number.isFinite(a.average_speed_ms) && a.average_speed_ms > 0
  );
  if (!efforts.length) return null;
  const best = efforts.reduce((max, a) => (a.average_speed_ms > max.average_speed_ms ? a : max));
  const thresholdSpeedMs = best.average_speed_ms * THRESHOLD_TEST_FACTOR;
  const secPer100 = 100 / thresholdSpeedMs;
  ...
  return {
    ...
    basedOn: {
      activityId: best.id,
      date: best.start_date,
      averageSpeedMs: Math.round(best.average_speed_ms * 100) / 100,
      durationMin: Math.round(best.moving_time_s / 60),
    },
  };
}
...
/**
 * Point d'entrée unique pour le rafraîchissement AUTOMATIQUE du profil (voir
 * components/ProfileHealth.js:autoUpdateFromActivities) et pour ZoneCharts.js en mode
 * "Automatique" : rassemble les trois seuils physiologiques détectables depuis les
 * séances Strava réelles (FTP vélo, VMA course, allure CSS natation), chacun `null` si
 * aucune séance qualifiante n'a été trouvée pour cette discipline — jamais de valeur
 * inventée (même règle que lib/physiology.js).
 */
export function estimatePhysiologyFromActivities(activities) {
  return {
    ftp: estimateThresholdPowerFromActivities(activities, BIKE_SPORTS),
    vma: estimateVmaFromActivities(activities, RUN_SPORTS),
    nat100: estimateSwimPaceFromActivities(activities, SWIM_SPORTS),
  };
}
