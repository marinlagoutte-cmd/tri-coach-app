...
function fallbackEfSpeedKmh(profile) {
  ...
  // CORRECTIF : 75 % de la VMA était trop rapide pour de l'endurance fondamentale (pour une
  // VMA de 20 km/h : 4:00/km, alors qu'une EF réelle se court plutôt autour de 60-70 % VMA).
  // Milieu de fourchette : 65 % VMA. Les zones calibrées manuellement restent prioritaires.
  return profile?.vma ? profile.vma * 0.65 : null;
}

/** Fourchette de vitesse (km/h) considérée comme de l'endurance fondamentale pour ce profil. */
function efSpeedBandKmh(profile) {
  if (hasValidPaceZones(profile?.paceZones)) {
    const sorted = [...profile.paceZones].sort((a, b) => Number(a.min) - Number(b.min));
    const z1 = Number(sorted[0]?.min) || 0;
    const z3 = Number(sorted[2]?.min);
    const low = z1 > 0 ? z1 : Number(sorted[1]?.min) * 0.85;
    return Number.isFinite(z3) ? [low, z3] : null;
  }
  return profile?.vma ? [profile.vma * 0.55, profile.vma * 0.76] : null;
}
...
function swimDescFormatIssue(desc) {
  ...
  if (!/(échauffement|echauffement|warm[- ]?up|calentamiento)\s*:/i.test(text)) {
    return 'Format natation non conforme : bloc "Échauffement :" manquant';
  }
  if (!/(corps de s[ée]ance|main set|parte principal|serie principal)\s*:/i.test(text)) {
...
export function parseClubSessionDesc(desc) {
  const text = String(desc || '').trim();
  if (!text) return null;
  const mainIdx = text.search(/(corps de s[ée]ance|main set|parte principal|serie principal)\s*:/i);
  if (mainIdx === -1) return null;
  const warmup = text.slice(0, mainIdx).replace(/^(échauffement|echauffement|warm[- ]?up|calentamiento)\s*:\s*/i, '').trim();
  const rest = text.slice(mainIdx).replace(/^(corps de s[ée]ance|main set|parte principal|serie principal)\s*:\s*/i, '');
...
export function enrichWorkoutMetrics(workout, profile) {
  ...
  if (type.includes('C.A.P') || type.includes('RUN')) {
    ...
    const isOwnFallbackPace = /\(EF, zones calibr[ée]es\)|\((?:65|75)% VMA\)/i.test(String(intensity || ''));
    ...
    // CORRECTIF (banc d'essai 09/2026) : l'ancienne détection "séance continue en Z2"
    // cherchait "Z2"/"EF" n'importe où dans title+desc+structure, puis REMPLAÇAIT l'allure
    // choisie par l'IA par 75 % VMA. Effets mesurés sur un plan cohérent : un footing EF à
    // 4:55/km devenait 4:00/km, et une course enchaînée à allure course (4:05/km, Z3) était
    // aussi écrasée parce que sa description mentionnait "8' Z2" en fin de séance.
    // Nouvelle règle : on ne touche à l'allure d'une séance d'endurance (zone PRINCIPALE Z1/Z2,
    // non fractionnée) que si elle est absente, issue de notre propre repli, ou HORS de la
    // fourchette d'endurance de l'athlète (ce qui couvre toujours le cas "l'athlète a
    // recalibré ses zones dans le Profil" : une allure devenue incohérente est recalculée).
    const primaryZoneMatch = String(effortZone || cardio || '').toUpperCase().match(/Z\s?([1-7])/);
    const primaryZone = primaryZoneMatch ? Number(primaryZoneMatch[1]) : null;
    const aiPaceMatch = String(intensity || '').match(/(\d+):(\d{2})\s*\/?\s*(?:min\/)?km/i);
    const aiSpeed = aiPaceMatch ? 60 / (Number(aiPaceMatch[1]) + Number(aiPaceMatch[2]) / 60) : null;
    const band = efSpeedBandKmh(profile);
    const isContinuousEf = !isInterval && primaryZone !== null && primaryZone <= 2;
    const efPaceOutOfBand = isContinuousEf && aiSpeed && band && (aiSpeed < band[0] || aiSpeed > band[1]);
    ...
    if (!efSpeed && !alreadyRpeBased) {
      ...
      if (kmhMatch) {
        ...
        const min = Math.floor(paceMin);
        const sec = Math.round((paceMin - min) * 60);
        const source = hasValidPaceZones(profile?.paceZones) ? '(EF, zones calibrées)' : '(65% VMA)';
    ...
    const hasValidSwimPace = !alreadyRpeBasedSwim && /\d+:\d{2}\s*\/?\s*100/.test(String(intensity || ''));
...
export function checkWorkoutCoherence(workout, profile) {
  ...
  if (!isRpeBased && (type.includes('CYCLISME') || type.includes('VELO') || type.includes('BIKE')) && profile?.ftp) {
    ...
    // CORRECTIF : borne haute alignée sur les zones envoyées à l'IA (Z6 anaérobie jusqu'à 150 %
    // FTP). Avec 130 %, un sprint à 500 W (FTP 360 W) était réécrit en "270W (75% FTP)".
    if (wMatch && (Number(wMatch[1]) < 40 || Number(wMatch[1]) > profile.ftp * 1.6)) {
  ...
  if (!isRpeBased && (type.includes('NATATION') || type.includes('SWIM')) && profile?.nat100) {
    if (!/\d:\d{2}\s*\/?\s*100/.test(String(workout.intensity || ''))) {
...
/**
 * ACTUALISATION MANUELLE DES ALLURES (bouton "Actualiser séance") -------------------
 * BUG RÉEL CORRIGÉ (allures figées sur les séances fractionnées) : sanitizeWorkout
 * ci-dessus (appelé automatiquement à chaque modification du profil, voir pages/
 * index.js:handleProfileChange/handlePaceZonesChange) préserve DÉLIBÉRÉMENT l'allure
 * choisie par l'IA pour une séance fractionnée/côtes une fois générée (`hasValidPace`
 * la considère "déjà valide" dès qu'elle a un format exploitable) — pour ne pas
 * l'écraser à la moindre correction mineure du profil. Seules les séances continues
 * en endurance fondamentale (Z2) et les allures de repli internes sont recalculées
 * automatiquement (voir isContinuousEfZone2/isOwnFallbackPace plus haut). Conséquence
 * concrète signalée par un athlète : après avoir corrigé sa VMA/FTP/CSS dans l'onglet
 * Profil, les séances fractionnées déjà générées gardent l'allure calculée avec
 * l'ANCIENNE valeur, potentiellement pendant toute la durée du plan.
 *
 * Ces fonctions recalculent TOUJOURS l'allure/puissance réelle depuis les données
 * ACTUELLES du profil, y compris pour ces séances jusqu'ici figées, en repartant du
 * pourcentage (% VMA / % FTP) que l'IA avait choisi pour CETTE séance précise (visible
 * dans le texte de `intensity`, ex. "4:13 /km (85% VMA)") plutôt que d'inventer une
 * nouvelle intensité relative — seul le chiffre absolu change, jamais le sens de la
 * séance. Déclenchées UNIQUEMENT à la demande explicite de l'athlète (bouton dédié
 * "Actualiser séance", voir pages/index.js:handleRefreshWeekPaces), jamais en tâche de
 * fond, pour rester prévisible et ne jamais surprendre par une réécriture silencieuse.
 */
/**
 * BUG RÉEL CORRIGÉ (allures fractionnées jamais affectées par les zones calibrées) :
 * recomputeRunPaceFromPct ci-dessous ne lisait QUE profile.vma (VMA brute), jamais
 * profile.paceZones — contrairement à fallbackEfSpeedKmh/paceSanityBoundsKmh plus haut,
 * qui priorisent déjà les zones calibrées manuellement. Concrètement : éditer ses zones
 * d'allure dans l'onglet Profil (sans toucher sa VMA déclarée) ne changeait RIEN à
 * l'allure affichée des séances fractionnées/côtes — puisque la formule qui les génère
 * ignorait complètement ces bornes personnalisées.
 *
 * Ici, si des zones calibrées valides existent, on interpole la vitesse cible le long
 * des bornes RÉELLES de l'athlète (Z1..Z5) au lieu du calcul théorique %VMA × VMA — en
 * utilisant les mêmes repères %VMA que defaultPaceZones (lib/zones.js:PACE_PCTS) pour
 * savoir où se situe, par défaut, chaque borne de zone. Une intensité entre deux
 * repères (ex. 75% VMA, entre Z2=70% et Z3=80%) est interpolée linéairement entre les
 * deux vitesses calibrées correspondantes ; au-delà du dernier repère (>95% VMA, allures
 * VMA/VO2max, hors zones) on extrapole avec la pente du dernier segment plutôt que de
 * retomber brutalement sur le calcul générique.
 */
function speedFromCustomPaceZones(pctVma, paceZones) {
  if (!Array.isArray(paceZones) || paceZones.length !== 5) return null;
  const sorted = [...paceZones].sort((a, b) => Number(a.min) - Number(b.min));
  const speedPoints = sorted.map((z) => Number(z.min));
  if (speedPoints.some((v) => !Number.isFinite(v))) return null;
  const pctPoints = PACE_PCTS.map((p) => p * 100);

  if (pctVma <= pctPoints[0]) return speedPoints[0];
  for (let i = 1; i < pctPoints.length; i++) {
    if (pctVma <= pctPoints[i]) {
      const t = (pctVma - pctPoints[i - 1]) / (pctPoints[i] - pctPoints[i - 1]);
      return speedPoints[i - 1] + t * (speedPoints[i] - speedPoints[i - 1]);
    }
  }
  const lastIdx = pctPoints.length - 1;
  const slope = (speedPoints[lastIdx] - speedPoints[lastIdx - 1]) / (pctPoints[lastIdx] - pctPoints[lastIdx - 1]);
  return speedPoints[lastIdx] + slope * (pctVma - pctPoints[lastIdx]);
}

function recomputeRunPaceFromPct(pct, profile) {
  const fromZones = hasValidPaceZones(profile?.paceZones)
    ? speedFromCustomPaceZones(pct, profile.paceZones)
    : null;
  const speed = fromZones ?? (profile?.vma ? Number(profile.vma) * (pct / 100) : null);
  if (!speed || !Number.isFinite(speed)) return null;
  const paceMin = 60 / speed;
  const min = Math.floor(paceMin);
  const sec = Math.round((paceMin - min) * 60);
  return `${min}:${String(sec).padStart(2, '0')} /km (${pct}% VMA)`;
}

function recomputeBikePowerFromPct(pct, profile) {
  if (!profile?.ftp) return null;
  const watts = Math.round(Number(profile.ftp) * (pct / 100));
  if (!Number.isFinite(watts) || watts <= 0) return null;
  return `${watts}W (${pct}% FTP)`;
}

/**
 * Recalcule les champs dépendant du profil (allure/puissance, FC cible) d'UNE séance,
 * en repartant du % VMA/FTP déjà choisi par l'IA pour cette séance précise, puis
 * propage le nouveau texte partout où l'ancien apparaissait tel quel (desc/structure —
 * ces champs interpolent souvent l'allure en texte libre, pas de manière "vivante").
 */
export function forceRecalcWorkoutPaces(workout, profile) {
  if (!workout) return workout;
  let w = sanitizeWorkoutFieldTypes(workout);
  const canonicalType = classifyDiscipline(w.type);
  if (canonicalType) w = { ...w, type: canonicalType };
  const type = (w.type || '').toUpperCase();
  const oldIntensity = String(w.intensity || '');

  const propagate = (oldText, newText) => (text) => (
    typeof text === 'string' && oldText && text.includes(oldText)
      ? text.split(oldText).join(newText)
      : text
  );

  const applyNewIntensity = (newIntensity) => {
    if (!newIntensity || newIntensity === oldIntensity) return;
    const replace = propagate(oldIntensity, newIntensity);
    w = { ...w, intensity: newIntensity, structure: replace(w.structure), desc: replace(w.desc) };
  };

  if (type.includes('C.A.P') || type.includes('RUN')) {
    if (/\(EF, zones calibr[ée]es\)|\(75% VMA\)/i.test(oldIntensity)) {
      // Allure de repli interne : déjà recalculée à chaque changement de profil par
      // sanitizeWorkout (isOwnFallbackPace) — rien à faire de plus ici.
    } else {
      const pctMatch = oldIntensity.match(/(\d{1,3})\s*%\s*VMA/i);
      if (pctMatch) applyNewIntensity(recomputeRunPaceFromPct(Number(pctMatch[1]), profile));
    }
  } else if (type.includes('CYCLISME') || type.includes('VELO') || type.includes('BIKE')) {
    const pctMatch = oldIntensity.match(/(\d{1,3})\s*%\s*FTP/i);
    if (pctMatch) applyNewIntensity(recomputeBikePowerFromPct(Number(pctMatch[1]), profile));
  } else if (type.includes('NATATION') || type.includes('SWIM')) {
    if (profile?.nat100 && /^\d:\d{2}\s*\/\s*100\s*m/i.test(oldIntensity)) {
      applyNewIntensity(`${profile.nat100} /100m`);
    }
  }

  // FC cible (avgBpm/cardio) : simple estimation dérivée de fcMax/fcRepos (jamais un
  // choix délibéré de l'IA à préserver) — toujours recalculée avec les valeurs actuelles.
  if (profile?.fcMax && profile?.fcRepos) {
    const isBike = type.includes('CYCLISME') || type.includes('VELO') || type.includes('BIKE');
    const ratio = isBike ? 0.6 : 0.65;
    const oldBpm = w.avgBpm;
    const newBpm = `${Math.round(profile.fcRepos + (profile.fcMax - profile.fcRepos) * ratio)} bpm`;
    if (newBpm !== oldBpm) {
      const replaceBpm = propagate(oldBpm, newBpm);
      w = { ...w, avgBpm: newBpm, cardio: replaceBpm(w.cardio) };
    }
  }

  // Repasse par le pipeline standard (types de champs, valeurs manquantes, bornes
  // physiologiques) sans jamais revenir sur ce qui vient d'être recalculé ci-dessus.
  return enrichWorkoutMetrics(w, profile);
}

/** Applique forceRecalcWorkoutPaces à toute une semaine de séances. */
export function forceRecalcWeekPaces(weekWorkouts, profile) {
  return (weekWorkouts || []).map((w) => forceRecalcWorkoutPaces(w, profile));
}
...
export function getIncompleteWorkouts(workoutsObj) {
  ...
  checkList.forEach(w => {
    // CORRECTIF (mesuré au banc d'essai) : une entrée REPOS sans champ `intensity` était jugée
    // incomplète, ce qui déclenchait 2 appels IA de correction complets (~33 000 caractères
    // chacun) à chaque génération, alors que le plan était complet.
...
export function mergeWorkoutPatches(workoutsObj, patches, profile) {
  ...
  const PATCH_META = ['week', 'patchMode', 'targetId'];
  const stripMeta = (p) => Object.fromEntries(Object.entries(p).filter(([k]) => !PATCH_META.includes(k)));

  patches.forEach((patch) => {
    if (!patch || typeof patch !== 'object') return;
    // CORRECTIF : `week` n'était jamais demandé à l'IA, donc toujours absent → toutes les
    // modifications tombaient sur la semaine N ("modifie mardi prochain" changeait CE mardi).
    // Si la semaine n'est pas précisée mais que l'id visé n'existe qu'en N+1, on la déduit.
    const targetId = patch.targetId || (patch.patchMode !== 'add' ? patch.id : '') || '';
    let weekKey = patch.week === 'N+1' ? 'N+1' : patch.week === 'N' ? 'N' : null;
    if (!weekKey) weekKey = targetId && copy['N+1'].some((w) => w.id === targetId) && !copy.N.some((w) => w.id === targetId) ? 'N+1' : 'N';
    const week = copy[weekKey];
    const mode = ['add', 'modify', 'remove'].includes(patch.patchMode) ? patch.patchMode : 'modify';
    const normalizedDay = normalizeDayName(patch.day);
    const fields = stripMeta(patch);

    if (mode === 'remove') {
      const idx = week.findIndex((w) => w.id === targetId);
      if (idx === -1) return;
      const removed = week[idx];
      const others = week.filter((w, i) => i !== idx && w.day === removed.day && w.type !== 'REPOS');
      if (others.length) {
        week.splice(idx, 1);
      } else {
        week[idx] = sanitizeWorkout({
          id: removed.id, day: removed.day, type: 'REPOS', title: 'Repos', duration: '0 min',
          intensity: 'Repos', desc: 'Séance annulée à la demande de l\'athlète.', modified: true, previous: { ...removed },
        }, profile);
      }
      return;
    }
    ...
    if (mode === 'add') {
      const id = fields.id && !week.some((w) => w.id === fields.id) ? fields.id : `w_${Math.random().toString(36).substring(2, 8)}`;
      const added = sanitizeWorkout({
        title: 'Séance ajoutée', duration: '45 min', desc: '', ...fields, id, day: normalizedDay, modified: true, added: true,
      }, profile);
      // Un jour qui reçoit une séance n'est plus un jour de repos.
      const restIdx = week.findIndex((w) => w.day === normalizedDay && w.type === 'REPOS');
      if (restIdx !== -1 && added.type !== 'REPOS') week.splice(restIdx, 1, added);
      else week.push(added);
      return;
    }

    // modify : par id d'abord (fiable sur un jour double), sinon par jour + discipline, sinon par jour.
    let index = targetId ? week.findIndex((w) => w.id === targetId) : -1;
    if (index === -1) {
      const disc = classifyDiscipline(fields.type);
      index = week.findIndex((w) => w.day?.toLowerCase() === normalizedDay.toLowerCase() && classifyDiscipline(w.type) === disc);
    }
    if (index === -1) index = week.findIndex((w) => w.day?.toLowerCase() === normalizedDay.toLowerCase());
    if (index !== -1) {
      const previous = { ...week[index] };
      delete previous.previous;
      week[index] = sanitizeWorkout({
        ...previous,
        ...fields,
        id: previous.id,
      ...
      week.push(sanitizeWorkout({
        title: 'Séance ajustée', duration: '1h00', desc: '', ...fields,
        id: fields.id || `w_${Math.random().toString(36).substring(2, 8)}`, day: normalizedDay, modified: true,
      }, profile));
    }
  });
  ...
  Object.values(byDay).forEach((sessionsOnDay) => {
    ...
    sessionsOnDay.forEach((w, idx) => {
      ...
      if (seenPerDiscipline[disc].has(key.split('::')[1])) {
        ...
        w.rpe = alt.rpe;
        w.desc = alt.desc;
        w.autoNote = 'Remplacée automatiquement : une autre séance identique était prévue le même jour.';
      }
 ...
 */
export function enforceThirdSessionLowIntensity(weekWorkouts, profile = null) {
  if (!Array.isArray(weekWorkouts) || weekWorkouts.length === 0) return weekWorkouts;
  const list = weekWorkouts.map((w) => ({ ...w }));
  const allDays = [...new Set(list.map((w) => w.day))];
  const sessionsOn = (day) => list.filter((w) => w.day === day && w.type !== 'REPOS');
  ...
  allDays.forEach((day) => {
    ...
    if (idx === -1) return;
    list[idx] = easeSession(list[idx], profile, '3e séance de la journée, obligatoirement peu intensive');
  });
...
/**
 * Séance "dure" (qualité) ? Utilisé pour la récupération entre séances clés.
 *
 * CORRECTIF : l'ancienne version cherchait les mots-clés dans TOUTE la description — une
 * séance facile qui mentionnait "sans aller au seuil" ou un bloc de 100m "allure seuil" en
 * natation était comptée comme dure, et un "(allégée)" était ensuite appliqué en cascade.
 * On regarde désormais la zone d'effort DÉCLARÉE (effortZone, puis cardio) et les mots-clés
 * du TITRE et du résumé (structure) uniquement. Les côtes, la VO2max, le tempo et les tests
 * terrain comptent aussi (ils manquaient).
 */
export function isHardSession(workout) {
  if (!workout || workout.type === 'REPOS') return false;
  // Une séance déjà allégée par un garde-fou ne compte plus comme dure (évite les cascades).
  if (/\(allégée\)/i.test(workout.title || '')) return false;
  const zoneText = String(workout.effortZone || workout.cardio || '').toUpperCase();
  const zones = [...zoneText.matchAll(/Z\s?([1-7])/g)].map((m) => Number(m[1]));
  if (zones.length && Math.max(...zones) >= 4) return true;
  const label = `${workout.title || ''} ${workout.structure || ''}`.toLowerCase();
  return /seuil|vma|sweet\s?spot|fractionn|threshold|interval|côtes|vo2|test terrain|allure (course|spécifique|cible)|à bloc|anaérobie/.test(label);
}

/** Durée "1h30" / "45 min" à partir d'un nombre de minutes. */
...
function formatPaceFromKmh(speedKmh) {
...
/**
 * ALLÈGEMENT COHÉRENT d'une séance. Remplace l'ancien motif répété dans 4 garde-fous
 * (titre + " (allégée)", intensité "Endurance fondamentale", description INCHANGÉE) qui
 * produisait des séances contradictoires. Ici TOUT est réécrit ensemble : titre,
 * intensité chiffrée en Z2 depuis le profil, zones, RPE, résumé et feuille de séance,
 * en gardant la durée d'origine (le volume de la semaine ne bouge pas). La raison est
 * exposée dans `autoNote` (affichée discrètement dans le détail de la séance), jamais
 * mélangée à la description.
 */
export function easeSession(workout, profile, reason) {
  if (!workout || workout.type === 'REPOS') return workout;
  const disc = classifyDiscipline(workout.type);
  const durMin = parseDurationMinutes(workout.duration) || 45;
  const originalTitle = String(workout.title || '').replace(/\s*\(allégée\)\s*/gi, '').trim();
  const note = `Allégée automatiquement${reason ? ` — ${reason}` : ''}. Séance prévue à l'origine : « ${originalTitle} ».`;
  const base = {
    ...workout,
    effortZone: 'Z1-Z2',
    cardio: profile?.fcMax && profile?.fcRepos ? `Z2 (${Math.round(profile.fcRepos + (profile.fcMax - profile.fcRepos) * 0.65)} bpm)` : 'Z2',
    avgBpm: profile?.fcMax && profile?.fcRepos ? `${Math.round(profile.fcRepos + (profile.fcMax - profile.fcRepos) * 0.65)} bpm` : '-',
    rpe: 'RPE 3-4/10',
    restTime: '-',
    autoNote: note,
    modified: true,
  ...
  if (disc === 'ENCHAÎNEMENT') {
    const speed = fallbackEfSpeedKmh(profile);
    const pace = formatPaceFromKmh(speed);
    const watts = profile?.ftp ? `${Math.round(profile.ftp * 0.65)}W` : null;
    const run = Math.min(20, Math.max(10, Math.round(durMin * 0.25)));
    const bike = Math.max(20, durMin - run - 10);
    return {
      ...base,
      title: 'Enchaînement vélo-course facile (allégée)',
      intensity: pace ? `${pace} (EF)` : 'Facile — RPE 3-4/10',
      structure: `Vélo ${bike}' Z2 puis course ${run}' Z2, transition calme, aucune intensité.`,
      desc: `Échauffement :\n10' vélo progressif\nCorps de séance :\n${bike}' vélo Z2${watts ? ` @${watts}` : ''}\nTransition calme\n${run}' course Z2${pace ? ` (~${pace})` : ''}\nRetour au calme : marche 5'`,
    };
  }
  if (disc === 'C.A.P') {
    const speed = fallbackEfSpeedKmh(profile);
    const pace = formatPaceFromKmh(speed);
    const main = Math.max(10, durMin - 15);
    return {
      ...base,
      title: `Footing endurance fondamentale (allégée)`,
      intensity: pace ? `${pace} (EF)` : 'Allure conversationnelle — RPE 3-4/10',
      structure: `Footing continu en endurance fondamentale (${formatDurationMinutes(durMin)}), aucune intensité.`,
      desc: `Échauffement :\n10' très souple\nCorps de séance :\n${main}' continu Z2${pace ? ` (~${pace})` : ''}, allure conversationnelle\n5' retour au calme`,
    };
  }
  if (disc === 'CYCLISME') {
    const watts = profile?.ftp ? `${Math.round(profile.ftp * 0.65)}W` : null;
    const main = Math.max(10, durMin - 20);
    return {
      ...base,
      title: 'Vélo endurance Z2 (allégée)',
      intensity: watts || 'Effort facile — RPE 3-4/10',
      cadence: '85-95 rpm',
      structure: `Sortie continue en endurance (${formatDurationMinutes(durMin)}), cadence libre, aucune intensité.`,
      desc: `Échauffement :\n10' progressif\nCorps de séance :\n${main}' continu Z2${watts ? ` @${watts}` : ''}, cadence 85-95 rpm\n10' souple`,
    };
  }
  if (disc === 'NATATION') {
    const announced = (String(workout.desc || '').match(/total\s*:\s*~?\s*(\d[\d\s.,]*)\s*m\b/i) || [])[1];
    const cssMatch = String(profile?.nat100 || '').match(/(\d+):(\d{2})/);
    const css = cssMatch ? Number(cssMatch[1]) + Number(cssMatch[2]) / 60 : 2;
    const fromDuration = Math.round(((durMin / (css * 1.3)) * 100) / 100) * 100;
    const total = Math.max(800, Number(String(announced || '').replace(/[\s.,]/g, '')) || fromDuration);
    const warm = 400;
    const cool = 200;
    const drills = 400;
    const reps = Math.max(1, Math.round((total - warm - cool - drills) / 200));
    const realTotal = warm + drills + reps * 200 + cool;
    const pace = cssMatch ? `${Math.floor(css * 1.12)}:${String(Math.round(((css * 1.12) % 1) * 60)).padStart(2, '0')} /100m` : null;
    return {
      ...base,
      title: 'Natation endurance technique (allégée)',
      intensity: pace || 'Allure confortable — RPE 3-4/10',
      structure: `Nage continue facile et éducatifs (${realTotal}m), aucune intensité.`,
      desc: `Échauffement :\n${warm} NC souple Z1\nCorps de séance :\n8*50 educ (rattrapé / doigts traînants) R : 15''\n${reps}*200 NC Z2 R : 20''\n---\n${cool} souple\nTotal : ${realTotal}m`,
    };
  }
  return base;
}

/** Ajoute (ou complète) la note automatique d'une séance, sans toucher à sa description. */
function withAutoNote(workout, note) {
  const previous = workout.autoNote ? `${workout.autoNote} ` : '';
  return { ...workout, autoNote: `${previous}${note}`.trim() };
}
 ...
 */
export function enforceNoConsecutiveHardDays(weekN, weekNPlus1, { expRank = 3, profile = null } = {}) {
  ...
  const hardOn = (week, day) => getList(week).filter((w) => w.day === day && w.type !== 'REPOS' && isHardSession(w));

  // CORRECTIF (banc d'essai : 3 séances clés sur 5 détruites dans un plan expert cohérent) :
  // un triathlète confirmé/expert enchaîne classiquement une qualité natation le lendemain
  // d'une qualité vélo. Pour ce profil, seul un enchaînement de la MÊME discipline, ou de la
  // course à pied (impact), est corrigé — la natation (sans impact, qualité quotidienne
  // courante chez les nageurs) n'est pas concernée. Débutant → intermédiaire : règle stricte inchangée.
  for (let i = 1; i < orderedDays.length; i += 1) {
    const prev = orderedDays[i - 1];
    const cur = orderedDays[i];
    const prevHard = hardOn(prev.week, prev.day);
    const curHard = hardOn(cur.week, cur.day);
    if (!prevHard.length || !curHard.length) continue;
    const conflict = expRank <= 3
      ? curHard[0]
      : curHard.find((c) => prevHard.some((p) => (classifyDiscipline(p.type) === classifyDiscipline(c.type) && classifyDiscipline(c.type) !== 'NATATION')
        || (classifyDiscipline(c.type) === 'C.A.P' && ['C.A.P', 'ENCHAÎNEMENT'].includes(classifyDiscipline(p.type)))));
    if (!conflict) continue;
    const curList = getList(cur.week);
    const idx = curList.findIndex((w) => w.id === conflict.id);
    if (idx === -1) continue;
    curList[idx] = easeSession(curList[idx], profile, `séance dure la veille (${prev.day}), récupération insuffisante`);
  }
 ...
 */
export function enforceDoubleThresholdEligibility(weekWorkouts, fitnessLevel, hoursPerWeek, trainingExperience, profile = null) {
  const expRank = EXPERIENCE_RANK[trainingExperience] || 3;
  ...
  return weekWorkouts.map((w) => (secondHardIds.has(w.id)
    ? easeSession(w, profile, 'deux séances de qualité le même jour (double seuil) non adaptées à ce profil')
    : w));
}
...
export function enforceTaperVolume(weekWorkouts, phaseKey, hoursPerWeek) {
  ...
  return weekWorkouts.map((w) => {
    ...
    const newMin = Math.max(20, Math.round((currentMin * ratio) / 5) * 5);
    if (newMin >= currentMin) return w;
    // CORRECTIF : la durée était réduite sans que la description change. La consigne de
    // réduction est désormais explicite pour l'athlète (note séparée de la description).
    return withAutoNote({ ...w, duration: formatDurationMinutes(newMin), modified: true },
      `Affûtage : séance raccourcie de ${w.duration} à ${formatDurationMinutes(newMin)} — garde l'échauffement, les accélérations et le retour au calme, réduis le corps de séance d'environ ${Math.round((1 - newMin / currentMin) * 100)} %.`);
  });
}
 ...
 */
export function applyFatigueAutoRegulation(weekWorkouts, { trendHarder = false, hrvLow = false, profile = null } = {}) {
  ...
  const reason = trendHarder && hrvLow
    ? 'ressenti récent plus dur que prévu et VFC en baisse'
    : trendHarder ? 'ressenti récent plus dur que prévu' : 'VFC en baisse par rapport à ta moyenne';
  return weekWorkouts.map((w) => (w.id === toEase.id ? easeSession(w, profile, reason) : w));
}
...
// Plancher de volume natation (m) par FORMAT visé et par expérience. CORRECTIF : l'ancien
// plancher unique (3200 m pour un expert) s'appliquait à toutes les séances — une natation
// de récupération de 2000 m pour un expert visant un format S devenait 3200 m / 1h16.
// Un format court se prépare avec des séances plus courtes et plus qualitatives.
export const SWIM_VOLUME_FLOOR_BY_FORMAT = {
  XS: { debutant: 1000, novice: 1200, intermediaire: 1500, confirme: 1800, expert: 2200 },
  S: { debutant: 1000, novice: 1300, intermediaire: 1700, confirme: 2000, expert: 2400 },
  M: { debutant: 1200, novice: 1500, intermediaire: 2000, confirme: 2500, expert: 2800 },
  L: { debutant: 1200, novice: 1500, intermediaire: 2200, confirme: 2800, expert: 3200 },
  XL: { debutant: 1400, novice: 1700, intermediaire: 2400, confirme: 3000, expert: 3400 },
};

export function swimVolumeFloor(trainingExperience, triathlonFormat) {
  const table = SWIM_VOLUME_FLOOR_BY_FORMAT[triathlonFormat] || SWIM_VOLUME_FLOOR_BY_FORMAT.M;
  return table[trainingExperience] || table.intermediaire;
}
 ...
 */
export function enforceSwimVolumeFloor(weekWorkouts, fitnessLevel, trainingExperience, phaseKey, triathlonFormat = 'M') {
  if (!Array.isArray(weekWorkouts)) return weekWorkouts;
  if (phaseKey === 'taper' || phaseKey === 'recovery') return weekWorkouts; // volume réduit voulu
  const floor = swimVolumeFloor(trainingExperience, triathlonFormat);

  return weekWorkouts.map((w) => {
    if (classifyDiscipline(w.type) !== 'NATATION') return w;
    // Séances volontairement légères : récupération, technique, test → jamais gonflées.
    if (/récup|recup|souple|technique|éducatif|educatif|test|allégée|recovery|drill/i.test(`${w.title || ''} ${w.structure || ''}`)) return w;
    const total = parseSwimTotalMeters(w.desc);
    // Tolérance de 10 % : on ne rajoute pas 2 longueurs pour atteindre un chiffre rond.
    if (total === null || total >= floor * 0.9) return w;
    ...
    const extraLine = `${extraSets}*200 NC Z2 R : 20''`;
    let newDesc;
    if (/\n---\n/.test(desc)) newDesc = desc.replace(/\n---\n/, `\n${extraLine}\n---\n`);
    else if (/Total\s*:/i.test(desc)) newDesc = desc.replace(/(\n?)(Total\s*:\s*~?\d[\d\s.,]*\s*m\b.*)/i, `\n${extraLine}\n$2`);
    else newDesc = `${desc}\n${extraLine}`;
    ...
    const newDurMin = durMin + Math.round(extraM / 33);
    return withAutoNote(
      { ...w, duration: formatDurationMinutes(newDurMin), desc: finalDesc, modified: true },
      `Volume complété de ${total} à ${newTotal} m (bloc aérobie ajouté) pour atteindre le minimum attendu à ton niveau.`
    );
  });
}
...
export function applyBeginnerFirstPlanRamp(weekWorkouts, fitnessLevel, trainingExperience, feedbackHistory, weekKey, hasExistingTrainingBase) {
  ...
  return weekWorkouts.map((w) => {
    ...
    const newMin = Math.max(15, Math.round((currentMin * factor) / 5) * 5);
    if (newMin >= currentMin) return w;
    return withAutoNote({ ...w, duration: formatDurationMinutes(newMin), modified: true },
      `Premier plan : durée ramenée de ${w.duration} à ${formatDurationMinutes(newMin)} pour une reprise progressive — raccourcis le corps de séance en proportion.`);
  });
}
...
export function applyEasierTrendProgression(weekWorkouts, trendDirection) {
  ...
  // CORRECTIF : l'ancienne version ajoutait "(densifiée)" au TITRE sans rien changer au contenu.
  // On densifie réellement une sortie d'endurance course ou vélo (la plus longue) en insérant
  // un bloc tempo dans son corps de séance, sans changer sa durée totale.
  const eligible = weekWorkouts.filter((w) => ['C.A.P', 'CYCLISME'].includes(classifyDiscipline(w.type))
    && !isHardSession(w) && !/\((allégée|densifiée)\)/i.test(w.title || '') && parseDurationMinutes(w.duration) >= 50);
  if (eligible.length === 0) return weekWorkouts;
  const target = [...eligible].sort((a, b) => parseDurationMinutes(b.duration) - parseDurationMinutes(a.duration))[0];
  const isBike = classifyDiscipline(target.type) === 'CYCLISME';
  const block = isBike ? "dont 3*(10' Z3 tempo - 5' Z2)" : "dont 3*(6' Z3 tempo - 3' Z2)";
  const desc = String(target.desc || '');
  const newDesc = /(corps de s[ée]ance|main set|parte principal)\s*:\s*\n/i.test(desc)
    ? desc.replace(/((corps de s[ée]ance|main set|parte principal)\s*:\s*\n[^\n]*)/i, `$1\n${block}`)
    : `${desc}\n${block}`.trim();
  return weekWorkouts.map((w) => (w.id === target.id ? withAutoNote({
    ...w,
    title: `${w.title} (densifiée)`,
    desc: newDesc,
    effortZone: 'Z2-Z3',
    structure: `${w.structure || ''} Avec ${isBike ? '3 blocs de 10' : '3 blocs de 6'}' en tempo (Z3).`.trim(),
    modified: true,
  }, 'Densifiée automatiquement : tes dernières séances ont été ressenties nettement plus faciles que prévu.') : w));
}
...
export function enforceBeginnerProgression(weekWorkouts, fitnessLevel, phaseKey, trainingExperience, hasExistingTrainingBase) {
  ...
  return weekWorkouts.map((w) => {
    ...
    return withAutoNote({ ...w, duration: formatDurationMinutes(capMin), modified: true },
      `Durée plafonnée à ${formatDurationMinutes(capMin)} (au lieu de ${w.duration}) pour ton niveau actuel — raccourcis le corps de séance en proportion.`);
  });
}
...
// CORRECTIF : ces planchers (issus des plans longue distance) s'appliquaient quel que soit
// l'objectif — un expert visant un triathlon S se voyait imposer 3 h de vélo et 2 h de course
// "longue". Ils dépendent maintenant de l'épreuve : longue distance = tableau complet,
// distance olympique / semi = plancher réduit, format court / 5-10 km = aucun plancher.
...
const LONG_SESSION_FLOOR_MIN_MID = {
  'C.A.P': { debutant: 40, novice: 50, intermediaire: 65, confirme: 75, expert: 85 },
  CYCLISME: { debutant: 60, novice: 75, intermediaire: 90, confirme: 110, expert: 120 },
};

function longFloorTableFor(goal = {}) {
  if (goal.sportType === 'running') {
    if (/marathon|semi/i.test(String(goal.distance || '')) || Number(goal.trailKm) >= 20) return LONG_SESSION_FLOOR_MIN;
    return null;
  }
  const fmt = goal.triathlonFormat;
  if (fmt === 'L' || fmt === 'XL') return LONG_SESSION_FLOOR_MIN;
  if (fmt === 'M') return LONG_SESSION_FLOOR_MIN_MID;
  return null; // XS / S
}
 ...
 */
export function enforceLongSessionFloor(weekWorkouts, trainingExperience, phaseKey, goal = { triathlonFormat: 'L' }) {
  if (!Array.isArray(weekWorkouts)) return weekWorkouts;
  if (phaseKey === 'taper' || phaseKey === 'recovery') return weekWorkouts;
  const table = longFloorTableFor(goal);
  if (!table) return weekWorkouts;
  const expKey = EXPERIENCE_RANK[trainingExperience] ? trainingExperience : 'intermediaire';

  return weekWorkouts.map((w) => {
    const discipline = classifyDiscipline(w.type);
    const floors = table[discipline];
    if (!floors || !isLongSession(w)) return w;
    ...
    const extraLine = `+${addedMin}' en endurance fondamentale (Z2), allure conversationnelle`;
    const lines = String(w.desc || '').split('\n').filter(Boolean);
    const newDesc = lines.length > 0
      ? [...lines.slice(0, -1), extraLine, lines[lines.length - 1]].join('\n')
      : extraLine;
    return withAutoNote(
      { ...w, duration: formatDurationMinutes(floorMin), desc: newDesc, modified: true },
      `Sortie longue allongée de ${w.duration} à ${formatDurationMinutes(floorMin)} (minimum attendu pour ton niveau et ton objectif).`
    );
  });
}
