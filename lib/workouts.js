...
export function enrichWorkoutMetrics(workout, profile) {
  ...
  if (type.includes('C.A.P') || type.includes('RUN')) {
    ...
    if (!profile?.vma && !alreadyRpeBased) {
  ...
  DAYS_OF_WEEK.forEach((day, idx) => {
    if (!presentDays.has(day)) {
      list.push({
        ...
        modified: false,
        desc: "Jour ajouté automatiquement : la réponse de l'IA ne contenait aucune entrée pour ce jour (réponse incomplète).",
      });
    }
  });
  return list;
...
function buildComplementaryWorkout(targetDay, list, nextType, seed) {
  ...
  return {
    ...
    rpe: chosen.rpe,
    modified: false,
    // Note sur sa propre ligne (jamais accolée à la ligne "Total :" / au retour au
    // calme) pour ne pas casser le formalisme en blocs du desc généré ci-dessus.
    desc: `${chosen.desc}\n\n(Séance ajoutée automatiquement pour atteindre le nombre de séances/semaine déclaré au questionnaire — jour double.)`,
  };
}
 ...
 */
export function enforceMaxSessionsPerDay(weekWorkouts, offDays, sportType) {
  ...
  const disciplineOf = (w) => classifyDiscipline(w.type);
  const sessionsOn = (day) => list.filter((w) => w.day === day && w.type !== 'REPOS');
  const dayCap = (day) => (sessionsOn(day).some((w) => disciplineOf(w) === 'ENCHAÎNEMENT') ? 1 : 2);
  ...
  while (guard < 30) {
    ...
    if (candidates.length === 0) break; // aucune destination possible : best effort, on laisse tel quel.
...
export function enforceNoConsecutiveHardDays(weekN, weekNPlus1) {
  ...
  for (let i = 1; i < orderedDays.length; i += 1) {
    ...
    curList[idx] = {
      ...
      cardio: 'Z2',
      modified: true,
      desc: `${curList[idx].desc || ''} (Intensité réduite automatiquement : deux séances difficiles consécutives détectées, récupération nécessaire entre deux séances à haute intensité.)`.trim(),
    };
  }
...
export function applyBeginnerFirstPlanRamp(weekWorkouts, fitnessLevel, trainingExperience, feedbackHistory, weekKey) {
  ...
  return weekWorkouts.map((w) => {
    ...
    return {
      ...
      duration: newDuration,
      modified: true,
      desc: `${w.desc || ''}${w.desc ? ' ' : ''}(Durée réduite automatiquement : premier plan pour un profil débutant, montée en charge progressive sur les premières semaines plutôt que le volume cible complet dès le départ.)`.trim(),
    };
  });
}
...
export function applyEasierTrendProgression(weekWorkouts, trendDirection) {
  ...
  return weekWorkouts.map((w) => (w.id === toIntensify.id ? {
    ...
    modified: true,
    desc: `${w.desc || ''}${w.desc ? ' ' : ''}(Intensité légèrement augmentée automatiquement : ressenti récent nettement plus facile que prévu sur plusieurs séances de suite, signe de progression — ajoute un bloc à allure soutenue dans cette séance.)`.trim(),
  } : w));
}
...
const PHYSIO_TEST_TEMPLATES = {
  ...
  CYCLISME: {
    title: 'Test terrain — FTP (20 minutes)',
    duration: '55 min',
 ...
 */
export function injectPhysioTestSessions(weekWorkouts, missingMetrics) {
  ...
  missingMetrics.forEach((discipline) => {
    const template = PHYSIO_TEST_TEMPLATES[discipline];
    if (!template) return;
