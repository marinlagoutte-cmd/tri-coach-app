...
export function checkWorkoutCoherence(workout, profile) {
  ...
  if (isInterval && (!workout.restTime || workout.restTime === '-')) {
    issues.push('Séance par intervalles sans temps de repos précisé');
  }
  if (isInterval && (!workout.structure || workout.structure.length < 15)) {
    issues.push('Structure de séance manquante ou trop vague');
  }
...
export function sanitizeWorkout(workout, profile) {
  ...
  return enrichWorkoutMetrics({ ...enriched, intensity: null, avgBpm: null, cardio: null, restTime: null, structure: null }, profile);
}
