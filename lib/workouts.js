...
/**
 * Construit une nouvelle séance "double" cohérente pour `targetDay`, en évitant
 * activement toute répétition : on préfère une discipline pas encore présente ce
 * jour-là (vrai enchaînement type brick) ; si une seule discipline est possible
 * (mono-discipline, ex: plan course à pied uniquement), on choisit un type de
 * séance (récupération / endurance / côtes / technique / fractionné) différent de
 * ceux déjà présents ce jour-là, jamais un doublon du même contenu.
 */
function buildComplementaryWorkout(targetDay, list, disciplineCycle, countByDiscipline, seed) {
  const sessionsOnDay = list.filter((w) => w.day === targetDay && w.type !== 'REPOS');
  const disciplinesOnDay = new Set(sessionsOnDay.map((w) => classifyDiscipline(w.type)));

  let candidateDisciplines = disciplineCycle.filter((d) => !disciplinesOnDay.has(d));
  if (candidateDisciplines.length === 0) candidateDisciplines = disciplineCycle;
  const nextType = [...candidateDisciplines].sort((a, b) => countByDiscipline(a) - countByDiscipline(b))[0];
...
export function enforceSessionCount(weekWorkouts, maxSessionsPerWeek, offDays, profile, sportType) {
  ...
  if (trainingDays.length < target) {
    ...
    while (missing > 0 && guard < 30) {
      ...
      // le choix précis (discipline + type de séance, jamais un doublon) est fait
      // dans buildComplementaryWorkout, en tenant compte de ce qui existe déjà ce
      // jour-là pour ne jamais générer deux fois la même séance.
      const restIdx = list.findIndex((w) => w.day === targetDay && w.type === 'REPOS');
      const newWorkout = buildComplementaryWorkout(targetDay, list, disciplineCycle, countByDiscipline, guard);
  ...
  // si aucune correction de quota n'a été nécessaire) : jamais deux séances
  // identiques sur le même jour dans le calendrier final.
  return dedupeIdenticalSameDaySessions(list).map((w) => sanitizeWorkout(w, profile));
}
