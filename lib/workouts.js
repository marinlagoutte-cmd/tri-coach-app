...
function isHardSession(workout) {
  const cardio = String(workout.cardio || '').toUpperCase();
 ...
 */
export function enforceBeginnerProgression(weekWorkouts, fitnessLevel, phaseKey) {
  const level = Number(fitnessLevel) || 3;
  if (level > 2 || !Array.isArray(weekWorkouts)) return weekWorkouts;
  ...
  return weekWorkouts.map((w) => {
    ...
    return {
      ...
      modified: true,
      desc: `${w.desc || ''}${w.desc ? ' ' : ''}(Durée réduite automatiquement : niveau de forme déclaré ${level}/5, progressivité de la phase "${phaseKey}" respectée.)`.trim(),
    };
  });
}

// --- GARDE-FOU DÉTERMINISTE : DEUX JOURS Z4/Z5 CONSÉCUTIFS ---
// Le prompt interdit déjà "deux séances à haute intensité (Z4/Z5) consécutives sur deux
// jours de suite" (voir buildWorkoutSchema dans lib/gemini.js), mais — comme pour le
// nombre de séances ou les unités d'allure — rien ne vérifiait le JSON réellement reçu
// si l'IA ignorait cette règle. On downgrade ici, de façon déterministe, la/les séance(s)
// intense(s) en trop du second jour vers un contenu endurance fondamentale équivalent,
// jamais une suppression pure — pour ne jamais faire disparaître une séance du calendrier.
...
function downgradeToEndurance(workout, reasonNote) {
  const discipline = classifyDiscipline(workout.type);
  const templates = COMPLEMENTARY_TEMPLATES[discipline] || COMPLEMENTARY_TEMPLATES['C.A.P'];
  const enduranceTemplate = templates.find((t) => t.cardio === 'Z2') || templates.find((t) => t.cardio === 'Z1-Z2') || templates[0];
  return {
    ...workout,
    title: enduranceTemplate.title,
    cardio: enduranceTemplate.cardio,
    rpe: enduranceTemplate.rpe,
    intensity: null, // recalculé par sanitizeWorkout/enrichWorkoutMetrics à partir du profil, jamais une valeur générique figée ici
    structure: null,
    restTime: null,
    modified: true,
    desc: `${enduranceTemplate.desc} (${reasonNote})`,
  };
}

/**
 * Downgrade la/les séance(s) haute intensité (Z4/Z5, seuil/VMA/sweetspot/fractionné) d'un
 * jour donné vers un contenu endurance fondamentale, si le jour PRÉCÉDENT (dans l'ordre
 * DAYS_OF_WEEK) contenait déjà au moins une séance haute intensité. Ne touche jamais au
 * premier jour d'une paire consécutive (on corrige toujours le second, jamais les deux),
 * et ne recule jamais dans le temps.
 */
export function enforceNoConsecutiveHardDays(weekWorkouts) {
  if (!Array.isArray(weekWorkouts)) return weekWorkouts;
  const list = weekWorkouts.map((w) => ({ ...w }));

  for (let i = 1; i < DAYS_OF_WEEK.length; i += 1) {
    const prevDay = DAYS_OF_WEEK[i - 1];
    const currDay = DAYS_OF_WEEK[i];
    const prevWasHard = list.some((w) => w.day === prevDay && w.type !== 'REPOS' && isHardSession(w));
    if (!prevWasHard) continue;

    list.forEach((w, idx) => {
      if (w.day === currDay && w.type !== 'REPOS' && isHardSession(w)) {
        list[idx] = downgradeToEndurance(
          w,
          `intensité réduite automatiquement : deux séances haute intensité (Z4/Z5) ne peuvent pas s'enchaîner sur deux jours consécutifs (le ${prevDay} en contenait déjà une)`
        );
      }
    });
  }
...
/**
 * Même règle que ci-dessus, mais à la CHARNIÈRE entre deux semaines consécutives (Dimanche
 * de la semaine N / Lundi de la semaine N+1) — un cas que enforceNoConsecutiveHardDays ne
 * peut pas voir puisqu'il ne traite qu'une semaine à la fois.
 */
export function enforceNoConsecutiveHardDaysAcrossWeeks(weekN, weekN1) {
  if (!Array.isArray(weekN) || !Array.isArray(weekN1)) return weekN1;
  const sundayWasHard = weekN.some((w) => w.day === 'Dimanche' && w.type !== 'REPOS' && isHardSession(w));
  if (!sundayWasHard) return weekN1;

  return weekN1.map((w) => {
    if (w.day === 'Lundi' && w.type !== 'REPOS' && isHardSession(w)) {
      return downgradeToEndurance(
        w,
        "intensité réduite automatiquement : la semaine précédente se terminait déjà par une séance haute intensité (Z4/Z5) le dimanche"
      );
    }
    return w;
  });
...
// --- GARDE-FOU DÉTERMINISTE : DOUBLE SEUIL HORS CRITÈRES ---
// Le prompt (buildEnduranceExpertRules) n'autorise le "double seuil" (deux blocs de
// seuil/intensité dans la MÊME journée, méthode norvégienne) que pour un niveau confirmé
// (>=4/5) ET un volume déclaré >=8h/semaine. Pour tout autre profil, on garantit ici,
// de façon déterministe, qu'un même jour ne contient jamais plus d'UNE séance haute
// intensité — en downgradant les séances intenses en trop du jour vers de l'endurance.
export function enforceDoubleThresholdRule(weekWorkouts, fitnessLevel, hoursPerWeek) {
  if (!Array.isArray(weekWorkouts)) return weekWorkouts;
  const level = Number(fitnessLevel) || 3;
  const advancedOk = level >= 4 && Number(hoursPerWeek) >= 8;
  if (advancedOk) return weekWorkouts; // double seuil autorisé pour ce profil, rien à corriger

  const list = weekWorkouts.map((w) => ({ ...w }));
  DAYS_OF_WEEK.forEach((day) => {
    const hardIdxOnDay = list
      .map((w, idx) => ({ w, idx }))
      .filter(({ w }) => w.day === day && w.type !== 'REPOS' && isHardSession(w))
      .map(({ idx }) => idx);

    // On garde la première séance intense du jour telle quelle, on downgrade les suivantes.
    hardIdxOnDay.slice(1).forEach((idx) => {
      list[idx] = downgradeToEndurance(
        list[idx],
        `double seuil non autorisé pour ce profil (niveau ${level}/5 et/ou volume déclaré < 8h/semaine) : une seule séance haute intensité par jour maximum`
      );
    });
  });
...
// --- GARDE-FOU DÉTERMINISTE : AFFÛTAGE (RÉDUCTION DE VOLUME FORCÉE) ---
// Le prompt demande déjà (buildEnduranceExpertRules) de réduire le volume de 40 à 60% en
// phase d'affûtage tout en conservant quelques touches d'intensité — mais, comme pour la
// progressivité débutant, cette baisse de charge est trop importante pour dépendre
// uniquement de l'obéissance de l'IA. Si le volume total de la semaine ne descend pas au
// moins à ~55% du volume horaire habituel de l'athlète, on réduit ici proportionnellement
// la durée de chaque séance (jamais leur intensité/contenu, conformément à la règle du
// prompt : "ne jamais couper l'intensité totalement, seulement le volume").
export function enforceTaperVolumeCap(weekWorkouts, phaseKey, hoursPerWeek) {
  if (!Array.isArray(weekWorkouts) || phaseKey !== 'taper') return weekWorkouts;
  const declaredMin = Number(hoursPerWeek) * 60;
  if (!declaredMin) return weekWorkouts;

  const capMin = declaredMin * 0.55; // milieu de la fourchette "réduction de 40 à 60%" demandée au prompt
  const totalMin = weekWorkouts
    .filter((w) => w.type !== 'REPOS')
    .reduce((sum, w) => sum + parseDurationMinutes(w.duration), 0);
  if (totalMin === 0 || totalMin <= capMin) return weekWorkouts;

  const ratio = capMin / totalMin;
  return weekWorkouts.map((w) => {
    if (w.type === 'REPOS') return w;
    const currentMin = parseDurationMinutes(w.duration);
    const reducedMin = Math.max(15, Math.round((currentMin * ratio) / 5) * 5); // jamais sous 15min, arrondi au 5min
    if (reducedMin >= currentMin) return w;
    return {
      ...w,
      duration: formatMinutesDuration(reducedMin),
      modified: true,
      desc: `${w.desc || ''}${w.desc ? ' ' : ''}(Durée réduite automatiquement : phase d'affûtage, volume ramené vers ~55% du volume habituel, intensité conservée.)`.trim(),
    };
  });
}

// --- GARDE-FOU DÉTERMINISTE : VOLUME HEBDOMADAIRE VS HEURES DÉCLARÉES (±15%) ---
// Le prompt demande déjà que "le volume hebdomadaire total... corresponde au volume
// horaire disponible déclaré... à ±15% près", mais rien ne le vérifiait après coup. On ne
// corrige QUE si l'écart dépasse la tolérance, en répartissant l'ajustement proportionnellement
// sur toutes les séances de la semaine (jamais sur une seule) pour rester cohérent avec le
// contenu généré par l'IA.
export function enforceWeeklyVolumeTolerance(weekWorkouts, hoursPerWeek) {
  if (!Array.isArray(weekWorkouts)) return weekWorkouts;
  const declaredMin = Number(hoursPerWeek) * 60;
  if (!declaredMin) return weekWorkouts;
  ...
  const totalMin = weekWorkouts
    .filter((w) => w.type !== 'REPOS')
    .reduce((sum, w) => sum + parseDurationMinutes(w.duration), 0);
  if (totalMin === 0) return weekWorkouts;

  const lowerBound = declaredMin * 0.85;
  const upperBound = declaredMin * 1.15;
  if (totalMin >= lowerBound && totalMin <= upperBound) return weekWorkouts;

  const target = totalMin < lowerBound ? lowerBound : upperBound;
  const ratio = target / totalMin;
  const direction = totalMin < lowerBound ? 'augmentée' : 'réduite';

  return weekWorkouts.map((w) => {
    if (w.type === 'REPOS') return w;
    const currentMin = parseDurationMinutes(w.duration);
    const adjustedMin = Math.max(15, Math.round((currentMin * ratio) / 5) * 5);
    if (adjustedMin === currentMin) return w;
    return {
      ...w,
      duration: formatMinutesDuration(adjustedMin),
      modified: true,
      desc: `${w.desc || ''}${w.desc ? ' ' : ''}(Durée ${direction} automatiquement : le volume total de la semaine devait rester à ±15% des ${hoursPerWeek}h/semaine déclarées.)`.trim(),
    };
  });
}

// --- GARDE-FOU DÉTERMINISTE : SEMAINE N+1 = COPIE DE LA SEMAINE N ---
// Le prompt demande déjà que "la semaine N+1 soit une PROGRESSION ou une VARIATION réelle
// par rapport à N", mais dedupeIdenticalSameDaySessions ne compare que DANS un même jour,
// jamais semaine contre semaine. Si un trop grand nombre de séances de N+1 sont identiques
// (même jour + même titre + même durée) à celles de N, on applique une légère surcharge
// progressive (+10% de durée, plafonnée) aux séances d'endurance concernées pour garantir
// une vraie progression, sans jamais toucher aux séances déjà différentes de N.
export function enforceWeekProgression(weekN, weekN1) {
  if (!Array.isArray(weekN) || !Array.isArray(weekN1) || weekN1.length === 0) return weekN1;

  const keyOf = (w) => `${w.day}::${(w.title || '').trim().toLowerCase()}::${w.duration}`;
  const weekNKeys = new Set(weekN.filter((w) => w.type !== 'REPOS').map(keyOf));
  const trainingN1 = weekN1.filter((w) => w.type !== 'REPOS');
  if (trainingN1.length === 0) return weekN1;

  const identicalCount = trainingN1.filter((w) => weekNKeys.has(keyOf(w))).length;
  const identicalRatio = identicalCount / trainingN1.length;
  if (identicalRatio < 0.6) return weekN1; // assez de variation réelle, rien à corriger

  return weekN1.map((w) => {
    if (w.type === 'REPOS' || !weekNKeys.has(keyOf(w))) return w;
    const currentMin = parseDurationMinutes(w.duration);
    const increasedMin = Math.round((currentMin * 1.1) / 5) * 5;
    if (increasedMin <= currentMin) return w;
    return {
      ...w,
      duration: formatMinutesDuration(increasedMin),
      modified: true,
      desc: `${w.desc || ''}${w.desc ? ' ' : ''}(Volume légèrement augmenté automatiquement : la semaine N+1 était quasi identique à la semaine N, une vraie progression a été appliquée.)`.trim(),
    };
  });
}
