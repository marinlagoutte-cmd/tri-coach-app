...
import { getIncompleteWorkouts, sanitizeWorkout, checkSessionCountCoherence, enforceSessionCount, mergeWeekFix } from './workouts';
 ...
 */
export async function generateNutritionAdvice({ profile, trainingPlan, sportType, raceProfile }) {
  const targetsBlock = buildRaceTargetsBlock(raceProfile);

  const trainingPrompt = `${NUTRITION_GUARDRAILS}
Profil athlète : poids ${profile?.weight || '?'}kg, discipline : ${sportType === 'triathlon' ? 'triathlon' : 'course à pied'}.
...
Profil athlète : poids ${profile?.weight || '?'}kg.
${targetsBlock}
...
export async function answerNutritionQuestion({ profile, trainingPlan, question, raceProfile, planSummary }) {
