...
import { getIncompleteWorkouts, sanitizeWorkout, checkSessionCountCoherence, enforceSessionCount, mergeWeekFix, dedupeIdenticalSameDaySessions, rebalanceSameDisciplineDoubles, enforceBeginnerProgression, enforceNoConsecutiveHardDays, enforceNoConsecutiveHardDaysAcrossWeeks, enforceDoubleThresholdRule, enforceTaperVolumeCap, enforceWeeklyVolumeTolerance, enforceWeekProgression } from './workouts';
 ...
 */
function buildEnduranceExpertRules({ fitnessLevel, sportType, runningSubtype, phaseKey, hoursPerWeek } = {}) {
  const level = Number(fitnessLevel) || 3;
  const advancedOk = level >= 4 && Number(hoursPerWeek) >= 8;

  const doubleThresholdRule = advancedOk
    ? `- DOUBLE SEUIL AUTORISÉ (niveau confirmé + volume ≥8h/sem) : tu PEUX proposer une séance de double
  seuil (2 blocs de seuil courts dans la même journée, ex: 6x6min Z3 le matin + 5x5min Z3 le soir,
  méthode norvégienne) UNIQUEMENT en phase développement/spécifique, jamais plus d'1x/semaine, jamais
  la semaine d'une sortie longue le lendemain, et jamais 2 semaines de suite sans semaine allégée entre les deux.`
    : `- Double seuil INTERDIT pour ce profil (niveau ou volume insuffisant) : une seule séance de qualité
  à dominante seuil/intensité par jour maximum.`;
...
function buildWorkoutSchema({ maxSessionsPerWeek, offDays, sportType, fitnessLevel, runningSubtype, phaseKey, hoursPerWeek } = {}) {
  const restCount = maxSessionsPerWeek ? 7 - Number(maxSessionsPerWeek) : null;
  const expertRules = buildEnduranceExpertRules({ fitnessLevel, sportType, runningSubtype, phaseKey, hoursPerWeek });
...
{ "workouts": { "N": [...], "N+1": [...] } }`;
  ...
  for (const weekKey of ['N', 'N+1']) {
    ...
    // ROBUSTESSE : mêmes principes que ci-dessus, pour les garde-fous d'expertise
    // physiologique (buildEnduranceExpertRules) qui n'étaient jusqu'ici demandés qu'au
    // niveau du prompt — donc jamais garantis si l'IA les ignorait.
    // 1) Double seuil (2 blocs d'intensité le même jour) réservé au niveau confirmé + ≥8h/sem.
    sanitized[weekKey] = enforceDoubleThresholdRule(sanitized[weekKey], wizardData.fitnessLevel, wizardData.hoursPerWeek);
    // 2) Jamais deux jours Z4/Z5 consécutifs DANS la même semaine (la charnière entre N et
    // N+1 est traitée séparément juste après la boucle, une fois les deux semaines prêtes).
    sanitized[weekKey] = enforceNoConsecutiveHardDays(sanitized[weekKey]);
    // 3) Affûtage : réduction de volume forcée (~55% du volume habituel) si l'IA ne l'a pas
    // fait elle-même — ne s'applique qu'à la phase "taper", donc jamais en base/développement.
    sanitized[weekKey] = enforceTaperVolumeCap(sanitized[weekKey], currentPhaseKey, wizardData.hoursPerWeek);
    // 4) Volume hebdo total vs heures déclarées, tolérance ±15% — sauf en affûtage, où une
    // baisse de volume plus marquée est volontaire et ne doit pas être "corrigée" à la hausse.
    if (currentPhaseKey !== 'taper') {
      sanitized[weekKey] = enforceWeeklyVolumeTolerance(sanitized[weekKey], wizardData.hoursPerWeek);
    }
  }

  // ROBUSTESSE : deux vérifications qui ne peuvent se faire qu'une fois les DEUX semaines
  // prêtes (comparaison inter-semaines, donc impossible dans la boucle ci-dessus).
  // 1) Jamais deux jours Z4/Z5 consécutifs à la CHARNIÈRE entre N (dimanche) et N+1 (lundi).
  sanitized['N+1'] = enforceNoConsecutiveHardDaysAcrossWeeks(sanitized.N, sanitized['N+1']);
  // 2) La semaine N+1 ne doit jamais être une simple copie de la semaine N (progression réelle).
  sanitized['N+1'] = enforceWeekProgression(sanitized.N, sanitized['N+1']);
