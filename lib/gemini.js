...
import { getIncompleteWorkouts, sanitizeWorkout, checkSessionCountCoherence, enforceSessionCount, mergeWeekFix, ensureAllDaysPresent, dedupeIdenticalSameDaySessions, rebalanceSameDisciplineDoubles, enforceMaxSessionsPerDay, enforceBeginnerProgression, enforceNoConsecutiveHardDays, enforceDoubleThresholdEligibility, enforceTaperVolume, applyFatigueAutoRegulation, checkWeeklyVolumeWarning, checkWeekSimilarityWarning, checkMonotonyWarning, checkPolarizationWarning, checkTrailElevationWarning, injectPhysioTestSessions, enforceSwimVolumeFloor, applyBeginnerFirstPlanRamp, applyEasierTrendProgression } from './workouts';
...
function runPaceSanityBoundsMinPerKm(vma) {
  if (!vma) return null;
...
function checkZoneRangeWarnings(weekWorkouts, physio, weekLabel) {
  ...
  weekWorkouts.forEach((w) => {
    ...
    if (w.type === 'C.A.P') {
      const bounds = runPaceSanityBoundsMinPerKm(physio.vma);
...
function buildEnduranceExpertRules({ fitnessLevel, trainingExperience, sportType, runningSubtype, phaseKey, hoursPerWeek } = {}) {
  const level = Number(fitnessLevel) || 3;
  const expRank = EXPERIENCE_RANK[trainingExperience] || 3;
...
function buildWorkoutSchema({ maxSessionsPerWeek, offDays, sportType, fitnessLevel, trainingExperience, runningSubtype, phaseKey, hoursPerWeek } = {}) {
  const restCount = maxSessionsPerWeek ? 7 - Number(maxSessionsPerWeek) : null;
  const expertRules = buildEnduranceExpertRules({ fitnessLevel, trainingExperience, sportType, runningSubtype, phaseKey, hoursPerWeek });
...
- MAXIMUM ABSOLU 2 séances réelles sur un même jour, JAMAIS 3 ni 4. Un jour "double" ne comporte QUE
  2 entrées, pas plus.
- Un jour qui contient une séance de type "ENCHAÎNEMENT" (brick) ne doit contenir AUCUNE autre séance
  ce jour-là : le brick combine déjà 2 disciplines en une seule séance structurée (ex: vélo→course),
  donc ajouter en plus une sortie longue séparée de l'une de ces disciplines le même jour est une
  INCOHÉRENCE (double emploi) — jamais "sortie longue course + vélo + natation PUIS brick course" le
...
- NC = nage complète (4 nages ou crawl continu) ; PULL = avec pull-buoy ; PLAQ = avec plaquettes ;
  ...
  maximal court.
- all [FORMAT] = allure course cible (ex "all HALF" = allure Half/70.3, "all XL" = allure Ironman,
  "all s" ou "all sprint" = allure format sprint) — n'utilise ces repères que si l'objectif de l'athlète
  correspond à ce format, sinon utilise directement la zone d'allure CSS précalculée plus bas.
 ...
 *      annoncé est réellement celui du contenu décrit).
 ...
 */
async function reviewPlanCoherenceWithAI({ sanitized, wizardData, resolvedProfile, phase, currentPhaseKey, isTriathlon }) {
  const contextSummary = `Contexte de l'athlète pour cette relecture :
- Objectif : ${wizardData.sportType}${wizardData.raceFormat ? ` (${wizardData.raceFormat})` : ''}, niveau déclaré ${wizardData.fitnessLevel}/5, expérience ${wizardData.trainingExperience}.
...
Vérifie ces 3 niveaux, dans cet ordre :
1. COHÉRENCE INTERNE de chaque séance : la durée annoncée correspond-elle vraiment à la structure décrite (échauffement + corps de séance + retour au calme) ? L'intensité/allure correspond-elle au type et au titre de la séance (une séance "récupération" ne doit pas contenir un bloc à allure seuil, une séance "fractionné" ne doit pas être quasi entièrement en endurance de base) ?
...
Réponds UNIQUEMENT avec ce JSON :
{
  ...
  try {
    const reviewData = await callGeminiJSON(prompt);
...
{ "workouts": { "N": [...], "N+1": [...] } }`;
  ...
  let sanitized = {
    ...
    N: ensureAllDaysPresent(data.workouts?.N || []).map((w) => sanitizeWorkout(w, resolvedProfile)),
    'N+1': ensureAllDaysPresent(data.workouts?.['N+1'] || []).map((w) => sanitizeWorkout(w, resolvedProfile)),
  };
  ...
  for (const weekKey of ['N', 'N+1']) {
    ...
    sanitized[weekKey] = rebalanceSameDisciplineDoubles(sanitized[weekKey], wizardData.sportType, wizardData.offDays);
    ...
    // 2 disciplines. On replafonne à 2 séances/jour (1 si le jour contient un brick),
    // puis on relance le rebalance ci-dessus au cas où le déplacement recréerait un
    // doublon de discipline sur le jour de destination.
    sanitized[weekKey] = enforceMaxSessionsPerDay(sanitized[weekKey], wizardData.offDays, wizardData.sportType);
    sanitized[weekKey] = rebalanceSameDisciplineDoubles(sanitized[weekKey], wizardData.sportType, wizardData.offDays);
    // Filet de sécurité INDÉPENDANT du nombre total de séances : même quand le total
    ...
    // -> on n'attend pas la prochaine génération pour réagir, on allège dès cette semaine.
    sanitized[weekKey] = applyFatigueAutoRegulation(sanitized[weekKey], trend.direction);
  ...
  sanitized.N = injectPhysioTestSessions(sanitized.N, missingMetrics);
  ...
  const reviewed = await reviewPlanCoherenceWithAI({ sanitized, wizardData, resolvedProfile, phase, currentPhaseKey, isTriathlon });
  ...
  // pour objectiver un doute plutôt que silencieusement ignorés.
  const qualityWarnings = [
    ...
    ...checkZoneRangeWarnings(sanitized.N, resolvedProfile, 'N'),
    ...checkZoneRangeWarnings(sanitized['N+1'], resolvedProfile, 'N+1'),
    // Incohérences repérées par la relecture IA (jamais bloquantes non plus, même logique
    // que les autres avertissements ci-dessus — visibles par l'athlète, jamais silencieuses).
    ...reviewed.reviewIssues.map((i) => `Relecture — ${i.week ? `semaine ${i.week} : ` : ''}${i.problem}`),
  ].filter(Boolean);

  return { trainingPlan: data.trainingPlan, workouts: sanitized, resolvedProfile, qualityWarnings };
}
...
// (voir pages/api/strava/webhook.js). `plannedWorkout` est null si aucune
// séance du plan n'a pu être associée automatiquement (voir lib/stravaMatch.js) :
// dans ce cas l'IA commente uniquement la séance réalisée, sans comparaison.
export async function analyzeStravaActivity({ activity, plannedWorkout, profile, language = 'fr' }) {
  const langNote = language !== 'fr' ? `Rédige ta réponse entièrement en ${AI_LANGUAGE_NAMES[language] || 'français'}.\n` : '';
...
Rédige une analyse COURTE (5-8 lignes MAXIMUM) de cette séance, ton motivant mais honnête :
${plannedWorkout ? "1) La séance réalisée correspond-elle à ce qui était prévu (allure/puissance/FC/durée) ? Sois précis et chiffré si les données le permettent, sans inventer de valeur absente.\n2) Un point positif concret.\n3) Un point de vigilance ou conseil pour la suite, seulement s'il y a une vraie raison de le mentionner (jamais de remarque inventée juste pour en avoir une)." : "Commente la séance réalisée (effort, régularité perçue via FC/puissance si dispo) et donne un conseil de récupération ou d'enchaînement adapté."}
N'utilise JAMAIS de markdown (pas de **gras**, pas de listes à puces) : du texte simple, en phrases.`;

  try {
    const text = await callGeminiText(prompt);
    return { analysis: text, status: 'ok' };
  } catch (err) {
    console.error('[gemini] analyzeStravaActivity error:', { code: err?.code, message: err?.message });
    return { analysis: null, status: 'error', errorCode: err?.code || 'UNKNOWN' };
  }
}
