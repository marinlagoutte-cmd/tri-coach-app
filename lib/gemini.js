...
const DEFAULT_CANDIDATES = ['gemini-2.5-flash', 'gemini-2.0-flash'];
const MAX_RETRIES = 2;
...
function getClient() {
  ...
  if (!apiKey) {
    throw new Error('Clé API Gemini manquante. Définis GEMINI_API_KEY dans .env.local (ou sur Vercel).');
  }
...
async function callGeminiJSON(prompt) {
  ...
  for (const model of candidates) {
    try {
      ...
      lastError = err;
      console.warn(`Modèle ${model} indisponible :`, err?.message || err);
    }
  }
  const label = candidates.join(', ');
  throw new Error(`Aucun modèle disponible parmi les candidats: ${label}. Détails: ${lastError?.message || 'inconnu'}`);
}
