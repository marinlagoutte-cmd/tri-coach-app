...
import { getIncompleteWorkouts, sanitizeWorkout, checkSessionCountCoherence, enforceSessionCount } from './workouts';
...
export async function generatePlanWithAI({ wizardData, profile, feedbackHistory }) {
  ...
  // Physiologie réellement adaptée à CET athlète (mesurée > estimée depuis un chrono > niveau
  // déclaré) au lieu des constantes génériques fixes utilisées auparavant pour tout le monde.
...
- VMA : ${physio.vma} km/h — fiabilité : ${physio.vmaSource}
- FTP : ${physio.ftp}W — fiabilité : ${physio.ftpSource}
- Allure natation CSS : ${physio.nat100}/100m — fiabilité : ${physio.nat100Source}
- FC max : ${physio.fcMax} bpm — fiabilité : ${physio.fcMaxSource}
- FC repos : ${physio.fcRepos} bpm — fiabilité : ${physio.fcReposSource}
${(physio.vmaSource.includes('générique') || physio.ftpSource.includes('générique') || physio.nat100Source.includes('générique')) ? "⚠️ Certaines valeurs ci-dessus ne sont PAS mesurées — reste légèrement plus prudent sur l'intensité tant qu'elles ne sont pas confirmées par l'athlète, et tu peux le lui suggérer dans le titre/desc d'une séance clé (ex: test VMA / test FTP)." : ''}`
    : `Profil physiologique de référence pour CET athlète :
- VMA : ${physio.vma} km/h — fiabilité : ${physio.vmaSource}
- FC max : ${physio.fcMax} bpm — fiabilité : ${physio.fcMaxSource}
- FC repos : ${physio.fcRepos} bpm — fiabilité : ${physio.fcReposSource}
(FTP et allure natation non pertinents : objectif course à pied uniquement.)
${physio.vmaSource.includes('générique') ? "⚠️ VMA non mesurée — reste légèrement plus prudent sur l'intensité, et tu peux suggérer un test VMA (ex: demi-Cooper) dans une séance de la semaine." : ''}`;
...
Réponds avec : { "workouts": { "N": [...], "N+1": [...] } }`;
  ...
  // fréquemment ignorée par l'IA malgré la règle absolue n°0 du prompt. On vérifie
  // programmatiquement et on tente UNE correction ciblée via prompt avant de
  // recourir à une correction déterministe garantie (jamais de plan livré incohérent).
  for (const weekKey of ['N', 'N+1']) {
    let issues = checkSessionCountCoherence(sanitized[weekKey], wizardData.maxSessionsPerWeek, wizardData.offDays);
    if (issues.length === 0) continue;

    const fixPrompt = `La semaine ci-dessous ne respecte pas le nombre de séances déclaré par l'athlète.
Problèmes détectés : ${JSON.stringify(issues.map((i) => i.message))}
Semaine actuelle : ${JSON.stringify(sanitized[weekKey])}
Corrige EXACTEMENT ${wizardData.maxSessionsPerWeek} séances d'entraînement (type ≠ REPOS) sur les 7 jours,
en gardant obligatoirement le ${wizardData.offDays} en REPOS, sans changer inutilement les autres séances déjà correctes.
${workoutSchema}
Réponds avec : { "workouts": [...7 entrées...] }`;
    try {
      const fixData = await callGeminiJSON(fixPrompt);
      const fixedWeek = fixData.workouts?.[weekKey] || fixData.workouts;
      if (Array.isArray(fixedWeek) && fixedWeek.length) {
        sanitized[weekKey] = fixedWeek.map((w) => sanitizeWorkout(w, resolvedProfile));
      }
    } catch (e) {
      console.warn('[gemini] Correction du nombre de séances via prompt échouée, correction déterministe appliquée:', e?.message);
    }

    issues = checkSessionCountCoherence(sanitized[weekKey], wizardData.maxSessionsPerWeek, wizardData.offDays);
    if (issues.length > 0) {
      // Dernier recours garanti : ne dépend plus de l'IA, corrige localement.
      sanitized[weekKey] = enforceSessionCount(sanitized[weekKey], wizardData.maxSessionsPerWeek, wizardData.offDays, resolvedProfile);
    }
  }
...
export async function chatWithCoach({ message, profile, workouts, trainingPlan, intent, sportType, constraints, feedbackHistory }) {
...
${constraintsBlock}${trendInstruction}
