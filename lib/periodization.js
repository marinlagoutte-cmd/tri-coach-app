...
/**
 * Numéro de semaine ISO 8601 (lundi = premier jour, semaine 1 = celle contenant le premier
 * jeudi de l'année) — AJOUTÉ pour remplacer l'affichage "Semaine N / Semaine N+1" du calendrier
 * par le vrai numéro de semaine calendaire (ex: "Semaine 35"), bien plus lisible pour l'athlète
 * que la notation interne N/N+1 (qui reste utilisée uniquement comme clé de données, jamais
 * affichée telle quelle désormais — voir pages/index.js).
 */
export function getISOWeekNumber(date) {
  const d = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()));
  const dayNum = d.getUTCDay() || 7; // dimanche -> 7 (ISO : lundi = 1 ... dimanche = 7)
...
/**
 * Libellé "Semaine XX" pour une semaine démarrant à `offsetWeeks` semaines de `referenceDate`
 * (0 = semaine en cours = "N", 1 = semaine suivante = "N+1", 2/3 = l'aperçu N+2/N+3, etc.) —
 * calcule le numéro ISO du LUNDI de cette semaine-là (jamais celui d'aujourd'hui), pour rester
 * correct même à cheval sur un changement d'année (semaine 52/53 -> semaine 1).
 */
export function getWeekLabel(offsetWeeks, referenceDate = new Date()) {
  const now = new Date(referenceDate);
  const dayNum = now.getDay() || 7;
  const monday = addDays(now, 1 - dayNum + offsetWeeks * 7);
...
const PHASE_GUIDANCE = {
  ...
  // Point 7 — calendrier multi-courses : courte coupure après une échéance B/C
  // intermédiaire, avant de reprendre le développement vers l'échéance suivante.
  recovery: "Récupération post-course : volume très réduit, intensité quasi nulle (footing/spin très léger ou repos complet), le temps que la fatigue neuromusculaire et la charge mentale de la course intermédiaire s'estompent avant de reprendre la progression vers l'échéance suivante.",
};
...
export function getWeeksOutlook(targetDate, constraints, offsets = [2, 3], referenceDate = new Date()) {
  ...
  return offsets.map((offset) => {
    ...
    return {
      offset,
      // Numéro de semaine calendaire réel (voir getWeekLabel ci-dessus) plutôt que la
      // notation interne "N+2"/"N+3" — jamais affichée telle quelle à l'athlète désormais.
      label: String(getWeekLabel(offset, referenceDate)),
...
// STORAGE_KEYS.raceCalendar). Beaucoup d'athlètes enchaînent plusieurs échéances (une
// course B de préparation, puis l'objectif A) plutôt qu'un unique objectif — ce que
// buildPeriodizationPlan ci-dessus ne gère pas (un seul `targetDate`). Les fonctions
// suivantes réutilisent les MÊMES briques (segmentWeeks, PHASE_GUIDANCE, dates réelles)
// sans dupliquer leur logique, pour articuler plusieurs échéances avec priorités A/B/C :
// une échéance A reçoit un vrai mésocycle d'affûtage complet (comme avant), une échéance
// B/C reçoit un mini-affûtage court suivi d'une courte récupération, SANS remettre à zéro
// la trajectoire de fond vers l'objectif A suivant.
// ---------------------------------------------------------------------------------------

/** Mésocycles pour un segment menant à une échéance INTERMÉDIAIRE (B/C) : pas de vraie
 * phase "base" séparée (elle a déjà eu lieu au tout début de la préparation globale, avant
 * la toute première échéance) — juste du développement continu, puis un mini-affûtage
 * court (en jours, pas en semaines complètes) avant la course. */
function buildTuneUpSegments(totalWeeks, priority) {
  const taperDays = priority === 'B' ? 5 : 3;
  const taperWeeks = Math.min(1, taperDays / 7);
  const buildWeeks = Math.max(0.5, Math.round((totalWeeks - taperWeeks) * 10) / 10);
  return [
    { key: 'build', label: 'Développement (vers échéance intermédiaire)', weeks: buildWeeks },
    { key: 'taper', label: `Affûtage court avant course ${priority}`, weeks: Math.round(taperWeeks * 10) / 10 },
  ];
}

/**
 * Périodisation sur PLUSIEURS échéances (races: [{ id, name, date, priority: 'A'|'B'|'C',
 * sportType, distanceLabel }]), du jour J jusqu'à la DERNIÈRE échéance de la liste. Chaque
 * phase renvoyée porte `forRaceId`/`forRaceName`/`forRacePriority` pour savoir à quelle
 * échéance elle prépare. Les échéances passées (déjà courues) sont ignorées. Avec une
 * seule échéance à venir, se comporte comme un simple appel à buildPeriodizationPlan
 * (mêmes phases, même forme) — donc toujours utilisable même si l'athlète n'a saisi qu'un
 * seul objectif dans le calendrier.
 */
export function computeMultiRacePeriodization(races, referenceDate = new Date()) {
  const now = new Date(referenceDate);
  ...
  if (upcoming.length === 0) return { phases: [], races: [] };

  if (upcoming.length === 1) {
    const { phases } = buildPeriodizationPlan(upcoming[0].date, now);
    return {
      races: upcoming,
      phases: phases.map((p) => ({ ...p, forRaceId: upcoming[0].id, forRaceName: upcoming[0].name, forRacePriority: upcoming[0].priority || 'A' })),
    };
  }
  ...
  upcoming.forEach((race, idx) => {
    const segmentWeeksCount = Math.max(1, Math.ceil((race.dateObj.getTime() - cursor.getTime()) / (7 * 24 * 3600 * 1000)));
    const isMainGoal = (race.priority || 'A') === 'A';
    const segments = (isMainGoal ? segmentWeeks(segmentWeeksCount) : buildTuneUpSegments(segmentWeeksCount, race.priority || 'B'))
      .filter((s) => s.weeks > 0);

    segments.forEach((seg, segIdx) => {
      const start = new Date(cursor);
      const end = addDays(start, Math.max(1, Math.round(seg.weeks * 7)) - 1);
      cursor = addDays(end, 1);
      allPhases.push({
        id: `race-${race.id}-phase-${segIdx + 1}`,
        key: seg.key,
        name: seg.label,
        weeks: seg.weeks,
        startDate: toISODate(start),
        endDate: toISODate(end),
        dates: `${formatShort(start)} → ${formatShort(end)} (${seg.weeks < 1 ? `${Math.round(seg.weeks * 7)}j` : `${seg.weeks} sem.`})`,
        status: start <= now ? 'En cours' : 'À venir',
        forRaceId: race.id,
        forRaceName: race.name,
        forRacePriority: race.priority || (isMainGoal ? 'A' : 'B'),
      });
    });

    // Courte récupération après une course B/C avant d'attaquer le bloc suivant — inutile
    // après la toute dernière échéance de la liste (rien à préparer après).
    if (!isMainGoal && idx < upcoming.length - 1) {
      const recoveryDays = race.priority === 'B' ? 5 : 3;
      const start = new Date(cursor);
      const end = addDays(start, recoveryDays - 1);
      cursor = addDays(end, 1);
      allPhases.push({
        id: `race-${race.id}-recovery`,
        key: 'recovery',
        name: `Récupération post-course (${race.name})`,
        weeks: Math.round((recoveryDays / 7) * 10) / 10,
        startDate: toISODate(start),
        endDate: toISODate(end),
        dates: `${formatShort(start)} → ${formatShort(end)} (${recoveryDays}j)`,
        status: start <= now ? 'En cours' : 'À venir',
        forRaceId: race.id,
        forRaceName: race.name,
        forRacePriority: race.priority || 'B',
      });
    }
  });

  return { phases: allPhases, races: upcoming };
}

/**
 * Résumé textuel des échéances À VENIR (hors objectif principal déjà géré par
 * `constraints.targetDate`/`targetTime`) — pensé pour être injecté dans le prompt IA
 * (voir lib/gemini.js:buildRaceCalendarBlock) même quand la génération se limite encore
 * aux semaines N/N+1 : le coach doit savoir qu'une course B arrive dans 3 semaines pour
 * commencer à y penser AVANT d'être dans la semaine de l'affûtage court correspondant.
 */
export function summarizeUpcomingRaces(races, referenceDate = new Date()) {
  const now = new Date(referenceDate);
  return [...(races || [])]
    .filter((r) => r?.date)
    .map((r) => ({ ...r, dateObj: new Date(r.date) }))
    ...
    .sort((a, b) => a.dateObj - b.dateObj)
    .map((r) => {
      const daysAway = Math.round((r.dateObj.getTime() - now.getTime()) / (24 * 3600 * 1000));
      return { ...r, daysAway };
    });
}
