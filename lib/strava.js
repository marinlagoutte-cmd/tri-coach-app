...
export function toActivityRow(raw, userId) {
  return {
    ...
    // Id du matériel Strava utilisé pour cette sortie ("b..." vélo, "g..." chaussures) —
    // voir lib/equipment.js pour le rattachement au total de kilométrage par matériel.
    gear_id: raw.gear_id || null,
  };
}

/**
 * Normalise `athlete.bikes` / `athlete.shoes` (retournés par fetchStravaAthlete, endpoint
 * /athlete) en une liste plate de matériel : [{ stravaGearId, kind, name, distanceM }, ...].
 * `distance` Strava est déjà un TOTAL À VIE en mètres pour ce matériel (pas une activité) —
 * c'est la source de vérité utilisée par lib/equipment.js pour le kilométrage des pièces,
 * plutôt qu'une somme des activités locales (qui ne couvrent que depuis la liaison du compte).
 */
export function extractStravaGear(athlete) {
  const bikes = Array.isArray(athlete?.bikes) ? athlete.bikes : [];
  const shoes = Array.isArray(athlete?.shoes) ? athlete.shoes : [];
  return [
    ...bikes.map((b) => ({ stravaGearId: b.id, kind: 'bike', name: b.name || 'Vélo', distanceM: b.distance ?? 0, retired: Boolean(b.retired) })),
    ...shoes.map((s) => ({ stravaGearId: s.id, kind: 'shoe', name: s.name || 'Chaussures', distanceM: s.distance ?? 0, retired: Boolean(s.retired) })),
  ];
}

/**
 * Laps (tours) d'une activité — endpoint dédié, séparé du détail d'activité
 * (fetchStravaActivity ci-dessus ne les inclut pas). Chaque lap contient ses
 * propres moyennes (FC, puissance, cadence, vitesse, dénivelé) et sa durée :
 * c'est la donnée qui permet de décortiquer une séance (répétitions, effort vs
 * récup) au lieu de se limiter aux moyennes globales de l'activité — voir
 * lib/lapsAnalysis.js:describeLaps, utilisée par analyzeStravaActivity
 * (lib/gemini.js) pour construire une analyse IA détaillée lap par lap.
 * Un seul appel (léger), fait UNIQUEMENT sur événement webhook temps réel
 * (une activité à la fois) — jamais lors de l'import en masse (sync.js), qui
 * utilise volontairement les résumés de liste pour ne pas exploser le quota
 * Strava sur des dizaines/centaines d'activités d'un coup.
 */
export async function fetchStravaActivityLaps(accessToken, activityId) {
  return stravaFetch(`${STRAVA_API_BASE}/activities/${activityId}/laps`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
}
...
/**
 * Segments Strava (créés par la communauté) dans une zone rectangulaire — utilisé par le
 * planificateur de parcours (lib/routePlanning.js) comme signal de "route fréquentée par
 * de vrais cyclistes" : demande explicite de l'athlète ("tracer un parcours... en prenant
 * en compte les outils de Strava pour... les routes les plus populaires"). ATTENTION :
 * Strava n'expose PAS sa vraie heatmap globale (fonctionnalité Summit réservée à l'app
 * Strava elle-même) via l'API publique tierce — `/segments/explore` est le seul signal de
 * fréquentation réellement accessible ici, un proxy imparfait mais réel (voir
 * lib/osmCycleRoutes.js pour la seconde source, complémentaire, qui compense son biais
 * "segments de performance" plutôt que "routes agréables").
 * @param {[number,number,number,number]} bbox [minLat, minLon, maxLat, maxLon]
 * @param {'riding'|'running'} activityType
 */
export async function exploreSegments(accessToken, bbox, activityType = 'riding') {
  const [minLat, minLon, maxLat, maxLon] = bbox;
  const params = new URLSearchParams({
    bounds: `${minLat},${minLon},${maxLat},${maxLon}`,
    activity_type: activityType,
  });
  const data = await stravaFetch(`${STRAVA_API_BASE}/segments/explore?${params.toString()}`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
...
/**
 * Détail d'un segment — SEULE façon d'obtenir un vrai compteur de fréquentation
 * (`effort_count`/`athlete_count`) : `/segments/explore` ne les renvoie pas directement.
 * Volontairement appelé seulement sur une poignée de segments déjà présélectionnés (voir
 * lib/routePlanning.js) — un appel par segment, pas question d'interroger le détail de
 * tous les segments d'une zone entière.
 */
export async function fetchSegmentDetail(accessToken, segmentId) {
  return stravaFetch(`${STRAVA_API_BASE}/segments/${segmentId}`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
}
