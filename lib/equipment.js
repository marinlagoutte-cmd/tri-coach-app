...
export async function syncEquipmentFromStrava(admin, userId, accessToken) {
  try {
    ...
    // Diagnostic temporaire (26/08) : à retirer une fois le blocage résolu. Montre
    // précisément ce que /athlete a renvoyé, pour distinguer "Strava ne renvoie aucun
    // matériel" (scope encore insuffisant / rien enregistré côté Strava) de "le matériel
    // est bien renvoyé mais quelque chose échoue après" (DB, backfill...).
