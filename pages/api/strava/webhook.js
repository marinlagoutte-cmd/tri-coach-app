...
import { ensureValidStravaToken, fetchStravaActivity, fetchStravaActivityLaps, toActivityRow } from '../../../lib/strava';
import { findAutoMatch } from '../../../lib/stravaMatch';
import { coAnalyzeStravaActivity } from '../../../lib/coGeneration';
import { createDeadline } from '../../../lib/aiConfig';
...
async function handleActivityUpsert({ admin, athleteId, activityId, isUpdate }) {
  ...
  // Laps (tours) — best-effort : une activité sans découpage particulier (Strava en crée
  // toujours au moins un) ou un rare échec ne doit jamais faire échouer tout le traitement
  // de l'activité, l'analyse IA sait fonctionner sans (voir describeLaps, lib/lapsAnalysis.js).
  let laps = null;
  try {
    laps = await fetchStravaActivityLaps(accessToken, activityId);
  } catch (err) {
    console.error('[webhook] fetchStravaActivityLaps error:', { activityId, message: err?.message });
  }
  ...
  const { weekKey, workoutId } = findAutoMatch(row, workouts, claimedWorkoutIds);
  ...
  try {
    // Budget de 50 s : la fonction est limitée à 60 s (vercel.json) et le matching/l'upsert
    // doivent encore s'exécuter après l'analyse.
    const result = await coAnalyzeStravaActivity({ activity: row, plannedWorkout, profile, language, laps: row.laps, deadline: createDeadline(50_000) });
  ...
  await admin.from('strava_activities').upsert(
    {
      ...
      // Demande explicite de l'athlète (03/09) : plus AUCUNE association automatique, même
      // 'exact' (même jour réel que prévu + même discipline) — matched_workout_id/weekKey
      // restent posés comme SUGGESTION (affichée dans ActivityDetail.js), mais
      // match_confirmed reste toujours false tant que l'athlète ne l'a pas validée
      // lui-même à la main (menu déroulant → 'Confirmer', qui repasse match_source à
      // 'manual' via pages/api/strava/match.js). Remplace l'ancienne règle qui confirmait
      // seule une correspondance 'exact'.
