...
export default async function handler(req, res) {
  ...
  const { wizardData, profile, feedbackHistory, healthHistory, manualPaceZones, injuryLog, raceCalendar, menstrualCycle, language, clientDate } = req.body || {};
  ...
  try {
    ...
    const { trainingPlan, workouts, resolvedProfile, qualityWarnings, autoFixNotes, weekSummary } = await coGeneratePlan({ wizardData, profile, feedbackHistory, healthHistory, manualPaceZones, injuryLog, raceCalendar, menstrualCycle, language: lang, clientDate });
    ...
    return res.status(200).json({
      ...
      // Explication courte de la logique des 2 semaines, rédigée par l'IA pour l'athlète.
