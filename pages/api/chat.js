...
export default async function handler(req, res) {
  ...
  const { message, history, clientDate, profile, workouts, trainingPlan, intent, sportType, constraints, feedbackHistory, healthHistory, manualPaceZones, injuryLog, raceCalendar, menstrualCycle, language } = req.body || {};
  ...
  try {
    ...
    // `history` : derniers messages de la conversation (le coach n'avait jusqu'ici aucune
    // mémoire du fil) ; `clientDate` : date locale de l'athlète ("demain", "mardi prochain").
    const safeHistory = Array.isArray(history)
      ? history.slice(-8).filter((m) => m && typeof m.text === 'string').map((m) => ({ sender: m.sender === 'user' ? 'user' : 'coach', text: m.text.slice(0, 1500) }))
      : [];
    const { reply, patches } = await coChatWithCoach({ message, history: safeHistory, clientDate, profile, workouts, trainingPlan, intent, sportType, constraints, feedbackHistory, healthHistory, manualPaceZones, injuryLog, raceCalendar, menstrualCycle, language: lang });
    ...
    // Garde-fous structurels appliqués UNIQUEMENT aux semaines réellement modifiées (avant :
    // les deux semaines étaient re-réparties à chaque ajustement, même celle non touchée).
    const touchedWeeks = [...new Set((patches || []).map((p) => (p.week === 'N+1' ? 'N+1' : 'N')))];
    if (patches?.length) {
      touchedWeeks.forEach((weekKey) => {
        ...
        week = rebalanceSameDisciplineDoubles(week, sportType, constraints?.offDays, constraints?.maxSessionsPerWeek);
        week = enforceThirdSessionLowIntensity(week, profileForSanitize);
    ...
    const code = ERROR_MESSAGES[lang][error?.code] ? error.code : (['BUDGET', 'TRUNCATED', 'SERVER'].includes(error?.code) ? 'NETWORK' : 'UNKNOWN');
    const reply = ERROR_MESSAGES[lang][code];
