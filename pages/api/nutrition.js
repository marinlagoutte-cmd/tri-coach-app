...
export default async function handler(req, res) {
  ...
  const { profile, trainingPlan, sportType, constraints, question, planSummary } = req.body || {};
  const raceProfile = deriveRaceProfile({ constraints, trainingPlan, sportType });

  try {
    if (question && String(question).trim()) {
      const { answer, verified } = await answerNutritionQuestion({ profile, trainingPlan, question, raceProfile, planSummary });
      return res.status(200).json({ answer, verified });
    }
    const result = await generateNutritionAdvice({ profile, trainingPlan, sportType, raceProfile });
