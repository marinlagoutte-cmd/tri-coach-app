...
const TOOLS_SUB_TABS = ['nutrition', 'weather', 'route', 'tirePressure', 'equipment'];
  ...
  const handleWizardComplete = async (wizardData) => {
    ...
    try {
      ...
      const res = await fetch('/api/generate-plan', {
        ...
        body: JSON.stringify({ wizardData, profile, feedbackHistory, healthHistory, manualPaceZones: paceZones, injuryLog, raceCalendar, menstrualCycle, language: lang, clientDate: localISODate() }),
      });
  ...
  const sendCoachMessage = async (userText, { intent = null, displayText = null } = {}) => {
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ message: userText, history: messages.slice(-8), clientDate: localISODate(), profile, workouts, trainingPlan, intent, sportType, constraints, feedbackHistory, healthHistory, manualPaceZones: paceZones, injuryLog, raceCalendar, menstrualCycle, language: lang }),
      });
      const data = await res.json();
    ...
    <div className="min-h-screen bg-ink-950 text-ink-100 font-body flex flex-col pb-[calc(4.75rem+env(safe-area-inset-bottom))] antialiased">
      ...
      <header className="sticky top-0 z-30 bg-ink-900/90 backdrop-blur-md border-b border-ink-800 px-4 pt-[calc(0.75rem+env(safe-area-inset-top))] pb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {isSupabaseConfigured && session && <NotificationBell accessToken={session?.access_token} />}
          ...
          {/* Bouton réglages toujours visible : AccountMenu (qui contenait aussi l'accès
              aux réglages) ne s'affiche que connecté — sans ce bouton, un athlète non
              connecté (ou Supabase non configuré) n'aurait plus aucun moyen d'ouvrir les
              réglages, donc plus aucun moyen de lancer "Nouveau plan" une fois onboardé. */}
          {!(isSupabaseConfigured && session) && (
            <button
              onClick={() => setShowSettings(true)}
              title={t('settings.title')}
              className="w-8 h-8 rounded-xl bg-ink-950 border border-ink-800 flex items-center justify-center text-sm shrink-0"
            >
              ⚙️
            </button>
          )}
      ...
      <main key={activeTab} className="flex-1 max-w-md w-full mx-auto p-4 space-y-4 animate-fadeIn">
        ...
        {activeTab === 'tools' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-5 gap-1.5 bg-ink-950 border border-ink-800 rounded-2xl p-1.5">
              {TOOLS_SUB_TABS.map((sub) => (
                <button
                  ...
                  className={`py-2 rounded-xl text-[10px] font-bold uppercase tracking-tight transition-all border ${
                    toolsSubTab === sub
                      ? 'bg-volt-500 border-volt-500 text-white shadow-md shadow-volt-500/20'
                      : 'bg-ink-900 border-ink-800 text-ink-400 hover:border-ink-700 hover:text-ink-200'
                  }`}
                >
        ...
        {activeTab === 'profile' && (
          <div className="flex flex-col h-[calc(100vh-170px)]">
            <div className="shrink-0 grid grid-cols-3 gap-1.5 bg-ink-950 border border-ink-800 rounded-2xl p-1.5 mb-3">
