...
export default function Home() {
  ...
  // Sous-onglet de l'onglet Outils (ex-Profil) — Profil & données santé, ou le nouvel
  // outil Pression pneus. Séparé de `activeTab` : ce n'est pas un onglet de la barre
  // principale, juste une sous-navigation locale à cet onglet.
  const [toolsSubTab, setToolsSubTab] = useState('profile');
  ...
  return (
    <div className="min-h-screen bg-ink-950 text-ink-100 font-body flex flex-col pb-[calc(4.75rem+env(safe-area-inset-bottom))] antialiased">
      ...
      <main key={activeTab} className="flex-1 max-w-md w-full mx-auto p-4 space-y-4 animate-fadeIn">
        ...
        {/* ONGLET OUTILS (ex-Profil) — sous-onglets : Profil & données santé / Pression pneus */}
        {activeTab === 'profile' && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2 bg-ink-900 border border-ink-800 rounded-2xl p-1">
              {[
                { id: 'profile', label: t('tools.profileSubTab') },
                { id: 'tirePressure', label: t('tools.tirePressureSubTab') },
              ].map((sub) => (
                <button
                  key={sub.id}
                  onClick={() => setToolsSubTab(sub.id)}
                  className={`py-2 rounded-xl text-xs font-bold transition-colors ${
                    toolsSubTab === sub.id ? 'bg-volt-500 text-white' : 'text-ink-400'
                  }`}
                >
                  {sub.label}
                </button>
