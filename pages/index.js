...
import { setCloudUser, fetchAndMergeCloudData, queueCloudPush, pushCloudDataNow } from '../lib/cloudSync';
...
export default function Home() {
  ...
  useEffect(() => {
    setStorageSaveHook(() => queueCloudPush());
  }, []);
  ...
  const handleWizardComplete = async (wizardData) => {
    ...
    try {
      const res = await fetch('/api/generate-plan', {
        ...
        body: JSON.stringify({ wizardData, profile, feedbackHistory }),
      });
  ...
  const sendCoachMessage = async (userText, { intent = null, displayText = null } = {}) => {
    ...
    try {
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: userText, profile, workouts, trainingPlan, intent, sportType, constraints, feedbackHistory }),
      });
      const data = await res.json();
  ...
  return (
    <div className="min-h-screen bg-ink-950 text-ink-100 font-body flex flex-col pb-[calc(5rem+env(safe-area-inset-bottom))] md:pb-6 antialiased">

      {/* HEADER — pt- supplémentaire pour ne pas passer sous l'encoche/la barre de statut
          une fois l'app lancée en plein écran (mode "standalone" installé) */}
        ...
        <div className="flex items-center gap-2">
          {isSupabaseConfigured && session && (
            <button
              ...
              title={session.user?.email ? `Connecté : ${session.user.email} — toucher pour se déconnecter` : 'Se déconnecter'}
          ...
          <button
            onClick={() => {
              ...
              if (hasExistingPlan && !window.confirm('Créer un nouveau plan remplacera et effacera ton plan actuel. Continuer ?')) return;
              setWizardError(null);
              setShowWizard(true);
            }}
          ...
          <button
            onClick={() => {
              if (!window.confirm('Supprimer ton plan actuel ? Tu pourras toujours utiliser le reste de l\'app (profil, nutrition, météo…), mais aucun nouveau plan ne sera généré automatiquement.')) return;
            ...
            title="Supprimer le plan actuel"
          >
            🗑
          </button>
        </div>
      ...
      {/* ONGLETS — décalage aligné sur la hauteur réelle du header (qui varie selon
          l'inset de sécurité en haut sur les appareils à encoche en mode standalone) */}
      <nav className="bg-ink-900 border-b border-ink-800 sticky top-[calc(56px+env(safe-area-inset-top))] z-20 px-2 py-2">
        <div className="max-w-md mx-auto flex gap-1 bg-ink-950 p-1 rounded-2xl border border-ink-800/80 overflow-x-auto scrollbar-none">
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center py-2 px-2.5 rounded-xl text-[11px] font-bold transition-all shrink-0 min-w-[64px] ${
                  isActive
                    ? 'bg-ink-800 text-volt-400 border border-ink-700 shadow-sm'
                      ...
                      <div key={c.id} className="relative flex gap-3 pb-4 last:pb-0">
                        ...
                        <div className={`flex-1 rounded-xl p-2.5 border ${isCurrent ? 'bg-volt-500/10 border-volt-500/40' : 'bg-ink-950 border-ink-800'}`}>
                          <div className="flex items-center justify-between gap-2">
                            <p className={`font-bold text-xs ${isCurrent ? 'text-white' : 'text-ink-200'}`}>{c.name}</p>
          ...
          <div className="space-y-3 flex flex-col h-[calc(100vh-170px)]">
            ...
            <form onSubmit={handleSendMessage} className="flex space-x-2 pt-2">
              <input
                ...
                className="flex-1 bg-ink-900 border border-ink-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-ink-500 focus:border-volt-500 focus:outline-none"
              />
              <button
                type="submit"
