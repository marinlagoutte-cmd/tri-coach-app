  ...
  const handleWizardComplete = async (wizardData) => {
    ...
    try {
      ...
      const res = await fetch('/api/generate-plan', {
        ...
        body: JSON.stringify({ wizardData, profile, feedbackHistory, healthHistory, language: lang }),
      });
  ...
  const sendCoachMessage = async (userText, { intent = null, displayText = null } = {}) => {
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: userText, profile, workouts, trainingPlan, intent, sportType, constraints, feedbackHistory, healthHistory, language: lang }),
      });
      const data = await res.json();
      ...
      <main key={activeTab} className="flex-1 max-w-md w-full mx-auto p-4 space-y-4 animate-fadeIn">
        ...
        {activeTab === 'profile' && (
          <div className="space-y-4">
            ...
            <PerformanceDashboard profile={profile} workouts={workouts} feedbackHistory={feedbackHistory} sportType={sportType} stravaActivities={stravaActivities} />
          </div>
        )}
