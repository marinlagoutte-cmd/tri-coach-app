...
describe('lib/weather utilities', () => {
  ...
  it('reverseGeocode should return single place or null', async () => {
    // Format de réponse du service de géocodage inverse utilisé par lib/weather.js
    // (city/locality/principalSubdivision/countryName) — l'ancien mock simulait encore
    // le format Open-Meteo ({ results: [...] }), d'où l'échec du test.
    const fake = { city: 'Local', locality: 'Local', principalSubdivision: 'X', countryName: 'France' };
    global.fetch = mockFetch(fake);
