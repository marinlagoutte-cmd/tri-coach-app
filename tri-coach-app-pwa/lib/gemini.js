...
import { getIncompleteWorkouts, sanitizeWorkout } from './workouts';
...
Disponibilités : ~${wizardData.hoursPerWeek}h/semaine réparties sur environ ${wizardData.maxSessionsPerWeek} séances/semaine.
Repos obligatoire le ${wizardData.offDays}.
${Number(wizardData.maxSessionsPerWeek) < 5 && wizardData.sportType === 'triathlon' ? `⚠️ Avec seulement ${wizardData.maxSessionsPerWeek} séances/semaine pour un triathlon, tu ne peux PAS traiter les 3 disciplines séparément chaque semaine à volume égal : priorise les disciplines les plus déterminantes pour CE format (voir répartition ci-dessus), envisage des séances d'enchaînement (brick) pour combiner deux disciplines dans une même séance, et sois transparent dans le titre/desc de chaque séance sur le compromis fait (ex: "cette semaine, moins de natation pour prioriser le vélo, discipline la plus longue de l'épreuve").` : ''}
