  ...
  return (
    <div className="bg-ink-900 border border-ink-800 rounded-2xl p-4 space-y-2">
      <div className="flex items-center justify-between">
        ...
        {verified && <span className="text-[9px] text-emerald-400 font-bold shrink-0">✓ {t('nutrition.verified')}</span>}
      </div>
...
export default function NutritionPanel({ profile, trainingPlan, sportType, constraints }) {
  ...
  const loadAiAdvice = async () => {
    ...
    try {
      const res = await fetch('/api/nutrition', {
        ...
        body: JSON.stringify({ profile, trainingPlan, sportType, constraints, language: lang }),
      });
  ...
  const askQuestion = async (e) => {
    ...
    try {
      const res = await fetch('/api/nutrition', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ profile, trainingPlan, constraints, question: q, planSummary, language: lang }),
      });
      const data = await res.json();
      setQaHistory((prev) => [...prev, { role: 'coach', text: data.error || data.answer, verified: data.verified }]);
    } catch (e) {
      setQaHistory((prev) => [...prev, { role: 'coach', text: lang === 'en' ? '⚠️ Error while answering.' : lang === 'es' ? '⚠️ Error al responder.' : "⚠️ Erreur lors de la réponse." }]);
    } finally {
      setAsking(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-sm font-black uppercase text-ink-50">{t('nutrition.title')}</h2>
      ...
      <AdviceBlock title={t('nutrition.training')} text={training} loading={loading} verified={verified} />
      <AdviceBlock title={t('nutrition.race')} text={race} loading={loading} verified={verified} />
      ...
      <div className="bg-ink-900 border border-ink-800 rounded-2xl p-4 space-y-3">
        <span className="text-[10px] font-mono text-volt-400 uppercase tracking-widest block">{t('nutrition.question')}</span>
        <p className="text-[10px] text-ink-500">{t('nutrition.questionExample')}</p>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
            {qaHistory.map((m, i) => (
              <div key={i} className={`text-xs p-2.5 rounded-xl ${m.role === 'user' ? 'bg-volt-500/10 border border-volt-500/20 text-volt-200' : 'bg-ink-950 border border-ink-800 text-ink-200'}`}>
                {m.role === 'coach' && m.verified && <span className="block text-[9px] text-emerald-400 font-bold mb-1">✓ {t('nutrition.verified')}</span>}
