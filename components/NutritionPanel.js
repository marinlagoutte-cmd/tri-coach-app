...
function AdviceBlock({ title, text, loading, verified }) {
  return (
    <div className="bg-ink-900 border border-ink-800 rounded-2xl p-4 space-y-2">
      ...
      {loading ? (
        <p className="text-xs text-ink-500 animate-pulse">Génération du conseil...</p>
      ) : (
        <p className="text-xs text-ink-200 leading-relaxed whitespace-pre-line">{text || "Pas encore de conseil généré."}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-ink-900 border border-ink-800 rounded-2xl p-4 space-y-3">
        ...
        <p className="text-[10px] text-ink-500">Ex : "Je n'arrive pas à manger du solide pendant les efforts intenses, que faire ?"</p>
        ...
        <form onSubmit={askQuestion} className="flex gap-2">
          <input
            ...
            className="flex-1 bg-ink-950 border border-ink-800 rounded-xl px-3 py-2 text-xs text-white placeholder-ink-500 focus:border-volt-500 focus:outline-none"
          />
          <button
            type="submit"
            disabled={asking}
