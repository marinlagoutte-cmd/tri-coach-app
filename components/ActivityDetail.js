...
export default function ActivityDetail({ activity, session, workouts, onClose, onActivityUpdated }) {
  ...
  const handleMatchChange = async (workoutId) => {
    if (!supabase) return;
    setMatchSaving(true);
    try {
      const patch = workoutId
        ? { matched_week_key: 'N', matched_workout_id: workoutId, match_source: 'manual' }
        : { matched_week_key: null, matched_workout_id: null, match_source: 'none' };
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-end sm:items-center justify-center animate-sheetBackdrop" onClick={onClose}>
      ...
      >
        ...
        {/* Correspondance avec le plan — auto par défaut, corrigeable ici (voir
            lib/stravaMatch.js pour l'hypothèse "Semaine N = semaine réelle en cours"
            qui explique pourquoi le matching auto peut parfois se tromper). */}
        <div className="space-y-1.5">
          <span className="text-[10px] font-mono text-ink-500 uppercase tracking-widest block">Séance associée</span>
          <select
            value={activity.matched_workout_id || ''}
            onChange={(e) => handleMatchChange(e.target.value || null)}
          ...
          {activity.match_source === 'auto' && matchedWorkout && (
            <p className="text-[9px] text-ink-600">Association automatique (jour + discipline) — modifiable ci-dessus si besoin.</p>
          )}
        </div>
