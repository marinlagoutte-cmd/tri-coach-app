...
export default function PerformanceDashboard({ profile, workouts, feedbackHistory, sportType = 'triathlon' }) {
  ...
  const trendData = {
    labels: trend.labels,
    datasets: [
      { label: 'Difficulté ressentie /10', data: trend.difficulty, borderColor: '#FB7185', backgroundColor: '#FB718533', tension: 0.3, spanGaps: true },
      { label: 'Forme ressentie /10', data: trend.capacity, borderColor: '#34D399', backgroundColor: '#34D39933', tension: 0.3, spanGaps: true },
    ],
  };
  const trendOptions = {
    responsive: true,
    plugins: { legend: { labels: { color: AXIS_COLOR, font: { size: 10 } } } },
    scales: {
      y: { min: 0, max: 10, ticks: { color: AXIS_COLOR, stepSize: 2 } },
      x: { ticks: { color: AXIS_COLOR } },
    },
  };
  ...
  const volumeOptions = {
    ...
    plugins: { legend: { labels: { color: AXIS_COLOR, font: { size: 10 } } } },
    scales: {
      y: { beginAtZero: true, ticks: { color: AXIS_COLOR, callback: (v) => `${v}h` } },
      x: { ticks: { color: AXIS_COLOR } },
    },
  };
  ...
  return (
    <div className="bg-ink-900 border border-ink-800 rounded-2xl p-4 space-y-4 text-ink-100">
      <div>
        ...
        <h2 className="text-lg font-black text-white font-display">Performance & progression</h2>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="bg-ink-950 border border-ink-800 rounded-xl p-3">
          <p className="text-xs font-bold text-white">Charge & forme ressenties</p>
        ...
        <div className="bg-ink-950 border border-ink-800 rounded-xl p-3">
          <p className="text-xs font-bold text-white">Volume prévu</p>
        ...
        <div className="bg-ink-950 border border-ink-800 rounded-xl p-3">
          <p className="text-xs font-bold text-white">Distribution des zones</p>
            ...
            <div className="space-y-2">
              {zones.map((z) => (
                <div key={z.zone} className="flex items-center gap-2 text-[11px]">
                  <span className="w-6 font-bold" style={{ color: z.color }}>{z.zone}</span>
                  <span className="w-20 text-ink-400 truncate">{z.label}</span>
                  ...
                  <span className="w-14 text-right text-ink-300 font-mono">{z.minutes}min</span>
                  <span className="w-9 text-right font-mono" style={{ color: z.color }}>{z.pct}%</span>
                </div>
              ))}
            </div>
          ) : (
        ...
        <div className="bg-ink-950 border border-ink-800 rounded-xl p-3">
          <p className="text-xs font-bold text-white mb-2">Métriques clés</p>
          {metrics.length > 0 ? (
              ...
              {metrics.map((m) => (
                <div key={m.label} className="flex items-center justify-between py-2 first:pt-0 last:pb-0">
                  ...
                  <span className="text-right">
                    ...
                    {m.delta && (
                      <span className={`ml-1.5 text-[10px] font-mono ${m.positive ? 'text-emerald-400' : 'text-ink-500'}`}>{m.delta}</span>
                    )}
                  </span>
                </div>
              ))}
