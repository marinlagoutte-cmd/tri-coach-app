...
// Cliquer un groupe sur la photo d'ensemble bascule directement (sans animation, sur
// retour utilisateur du 26/08 — l'effet de zoom précédent gênait plus qu'autre chose)
// vers la photo en gros plan de ce groupe, où les points cliquables (Hotspot) ouvrent
// le suivi de la pièce.
//
...
const PHOTOS = {
  ...
  pillPos: {
    ...
    roues: { x: 80.1, y: 59.0 },
    cockpit: { x: 78.4, y: 39.6 },
  },
  zones: {
    'transmission-avant': {
      src: '/equipment/canyon-aeroad/transmission-avant.jpg', ratio: 730 / 630,
      parts: { manivelles: { x: 79, y: 46 }, pedales: { x: 43, y: 89 }, chaine: { x: 20.5, y: 41.3 } },
    },
    ...
    roues: {
      ...
      parts: { 'pneu-av': { x: 58, y: 14 }, disques: { x: 48, y: 60 }, roues: { x: 20, y: 60 }, plaquettes: { x: 42, y: 70 } },
    },
    cockpit: {
      src: '/equipment/canyon-aeroad/cockpit.jpg', ratio: 1000 / 1333,
      parts: { cintre: { x: 50, y: 58 }, ruban: { x: 69.1, y: 65.6 }, durites: { x: 45.1, y: 58.7 } },
    },
  },
};
...
function GroupPill({ x, y, label, tone, onClick }) {
  ...
  return (
    <div style={{ position: 'absolute', left: `${cx}%`, top: `${cy}%`, transform: `translate(${translateX},-50%)`, display: 'flex', justifyContent: justify, maxWidth: '92%' }}>
      <button onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 6, background: C.card, border: `1px solid ${C.border}`, borderRadius: 999,
        padding: '6px 12px 6px 8px', cursor: 'pointer', boxShadow: '0 1px 4px rgba(20,22,28,0.25)', whiteSpace: 'nowrap' }}>
...
function Hotspot({ x, y, tone, label, onClick }) {
  const cx = Math.min(95, Math.max(5, x));
  const cy = Math.min(95, Math.max(5, y));
  return (
    <button onClick={onClick} title={label} style={{ position: 'absolute', left: `${cx}%`, top: `${cy}%`, transform: 'translate(-50%,-50%)',
          ...
          {zoneComponents.map((c) => {
            ...
            return (
              <div key={c.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
                ...
                {open && (
                  <div style={{ borderTop: `1px solid ${C.border}`, padding: '10px 14px', background: C.page, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <DetailsField value={c.details} onSave={(v) => saveDetails(c, v)} />
