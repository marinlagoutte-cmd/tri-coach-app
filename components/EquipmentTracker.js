...
export default function EquipmentTracker({ session }) {
  ...
  // Diagnostic brut de la dernière synchro Strava (voir lib/equipment.js:syncEquipmentFromStrava)
  // — distance renvoyée par Strava vs somme locale vs valeur retenue, PAR matériel. Affiché
  // tel quel sous le bouton "Actualiser" (demande explicite de l'athlète suite à un km de
  // chaussures resté bloqué à 0 malgré plusieurs correctifs) : sans ça, impossible de savoir
  // depuis l'extérieur si le blocage vient de Strava (scope/`distance` non renvoyé) ou du
  // repli local (aucune activité locale avec ce gear_id) — même symptôme, causes différentes.
  ...
  async function handleSync() {
    ...
    try {
      ...
      setSyncDetails(Array.isArray(data?.details) ? data.details : null);
      await load();
    } catch {
  ...
  /**
   * Action manuelle groupée : remet TOUTES les pièces usables d'un matériel (vélo OU
   * chaussure, demande explicite de l'athlète — le bouton n'était visible que pour les
   * vélos jusqu'ici) au kilométrage TOTAL du matériel lui-même (baseline = 0 → km affiché =
   * totalKm), en un clic — même principe que le comportement appliqué par défaut à un vélo
   * NOUVELLEMENT synchronisé (voir lib/equipment.js:backfillComponents), mais applicable ici
   * rétroactivement à un matériel déjà suivi dont les pièces sont restées bloquées sur un
   * ancien total (ex. baseline figée à un moment où le kilométrage Strava remonté était
   * encore incorrect/à 0 — voir syncDetails plus haut pour vérifier que le total Strava
   * lui-même est bien à jour AVANT de réinitialiser, sinon ça ne fait que remettre à 0 aussi).
   * Direction inverse de "Marquer comme changé", qui remet une pièce précise à 0%.
   * Confirmation demandée avant d'écraser les corrections déjà saisies manuellement.
   *
   ...
   * kilométrage du matériel, ce qui peut être en dessous, à, ou au-dessus de 100% de la durée
   * de vie de chaque pièce selon son lifespan_km propre.
   */
  async function resetAllComponentsToMaxWear(equipment) {
    const usable = equipment.components.filter((c) => c.lifespan_km > 0);
    if (usable.length === 0) return;
    if (!window.confirm(`Remettre les ${usable.length} pièce(s) usables de "${equipment.name}" au kilométrage total (${equipment.kind === 'bike' ? 'du vélo' : 'de la paire'}) ? Toute correction déjà saisie sur ces pièces sera écrasée.`)) return;
    await Promise.all(
  ...
  if (view.level === 'list') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {syncDetails && (
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: '8px 10px', fontSize: 11, color: C.textSecondary, fontFamily: 'monospace' }}>
            {syncDetails.length === 0 ? (
              <div>Strava n'a renvoyé aucun matériel (bikes/shoes vides côté /athlete).</div>
            ) : syncDetails.map((d, i) => (
              <div key={i} style={{ marginBottom: i < syncDetails.length - 1 ? 4 : 0 }}>
                {d.name} ({d.kind}) — Strava&nbsp;: {Math.round(d.stravaDistanceM / 1000)} km · local&nbsp;: {Math.round(d.localSumM / 1000)} km · retenu&nbsp;: {Math.round(d.appliedDistanceM / 1000)} km
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {comp && (
          <button
            ...
            style={{ alignSelf: 'flex-start', fontSize: 12, fontWeight: 500, color: C.textSecondary, background: 'none', border: `1px solid ${C.border}`, borderRadius: 8, padding: '6px 12px', cursor: 'pointer' }}
          >
            Réinitialiser au kilométrage Strava
          </button>
        )}
