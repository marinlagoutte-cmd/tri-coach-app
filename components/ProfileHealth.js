...
const METRICS = [
  { key: 'weight', label: 'Poids', unit: 'kg', color: '#8358FF' },
  { key: 'fcRepos', label: 'FC repos', unit: 'bpm', color: '#FF4D80' },
...
export default function ProfileHealth({ profile, onProfileChange, sportType = 'triathlon' }) {
  ...
  const chartOptions = {
    responsive: true,
    plugins: { legend: { display: activeMetrics.length > 1, labels: { color: '#8177AC', font: { size: 10 } } } },
    scales: {
      y: { position: 'left', ticks: { color: activeMetrics[0]?.color || '#8177AC' } },
  ...
  return (
    <div className="bg-ink-900 border border-ink-800 rounded-2xl p-4 space-y-4 text-ink-100">
      ...
      <div className="flex gap-2 items-end">
        ...
        <button
          onClick={addEntry}
          className="bg-volt-500 hover:bg-volt-400 text-ink-950 font-bold text-xs px-4 py-2 rounded-xl min-h-tap"
        >
          Ajouter
        </button>
      </div>
