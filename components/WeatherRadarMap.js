...
export default function WeatherRadarMap() {
  ...
  // BUG RÉEL CORRIGÉ : les 3 paliers utilisaient la MÊME teinte (bleu ciel) avec seulement
  // l'opacité qui variait (0.35/0.55/0.8) — sur la carte (fond clair OU sombre selon la zone/le
  // thème), ça rendait les 3 niveaux quasi indiscernables entre eux, et le remplissage
  // translucide sans contour se fondait dans les tuiles de la carte : illisible en pratique,
  // en particulier sur petit écran. On passe à une échelle de 3 TEINTES distinctes (au lieu
  // d'une seule teinte à 3 opacités), avec un contour sombre semi-opaque sur chaque cercle
  // pour qu'il se détache nettement du fond quelle que soit la carte en dessous.
  ...
  function rainDivIcon(L, { color, tier, uid, size = 30 }) {
  // TEXTURE AJOUTÉE (demande explicite) : un simple disque de couleur plate ne "ressemble"
  // à rien de reconnaissable — on superpose un dégradé radial (effet "flaque"/goutte
  // concentrée au centre) ET des stries diagonales façon pluie, dont le nombre/la longueur
  // augmentent avec l'intensité — encodage redondant avec la couleur (accessible même sans
  // bien distinguer les teintes), et surtout ça se LIT comme de la pluie, pas comme un point.
  // `uid` doit être unique par marqueur : un <radialGradient id="..."> est résolu dans TOUT
  // le document, un id dupliqué entre les dizaines de divIcon affichés simultanément ferait
  // pointer certains marqueurs vers le mauvais dégradé.
  const streakCount = { bruine: 3, moderee: 5, soutenue: 7 }[tier] || 3;
  const streakLen = { bruine: 5, moderee: 8, soutenue: 11 }[tier] || 5;
  const streakOpacity = { bruine: 0.35, moderee: 0.5, soutenue: 0.65 }[tier] || 0.4;
  const c = size / 2;
  const streaks = Array.from({ length: streakCount }, (_, i) => {
    // Positions pseudo-aléatoires mais déterministes (même uid+i => même rendu à chaque
    // redraw, pas de scintillement au changement d'heure sur un point qui reste affiché).
    const seed = (uid * 31 + i * 17) % 100;
    const angle = 200 + (seed % 20); // stries toutes inclinées dans le même sens (pluie qui tombe), léger jitter
    const rad = (angle * Math.PI) / 180;
    const cx = c + ((seed % 13) - 6);
    const cy = c + (((seed * 3) % 13) - 6);
    const x2 = cx + Math.cos(rad) * streakLen;
    const y2 = cy + Math.sin(rad) * streakLen;
    return `<line x1="${cx}" y1="${cy}" x2="${x2}" y2="${y2}" stroke="rgba(15,23,42,${streakOpacity})" stroke-width="1.6" stroke-linecap="round"/>`;
  }).join('');
  const gradId = `rain-grad-${uid}`;
  const html = `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" style="overflow:visible;">
    <defs>
      <radialGradient id="${gradId}" cx="50%" cy="45%" r="55%">
        ...
        <stop offset="70%" stop-color="${color}" stop-opacity="0.75"/>
        <stop offset="100%" stop-color="${color}" stop-opacity="0.4"/>
      </radialGradient>
    </defs>
    <circle cx="${c}" cy="${c}" r="${c - 2}" fill="url(#${gradId})" stroke="rgba(15,23,42,0.65)" stroke-width="1.5"/>
    ${streaks}
  </svg>`;
  ...
  useEffect(() => {
    ...
    gridWindData.forEach((pt, idx) => {
      ...
      if (!color) return;
      const tier = mm < 1 ? 'bruine' : mm < 4 ? 'moderee' : 'soutenue';
      // Marqueur texturé (dégradé + stries de pluie) plutôt qu'un simple cercle plat — voir
      // rainDivIcon ci-dessus pour le détail. `idx` sert de graine déterministe pour les stries.
      const marker = L.marker([pt.lat, pt.lon], {
        icon: rainDivIcon(L, { color, tier, uid: idx, size: 30 }),
        interactive: true,
      });
      marker.bindTooltip(`🌧️ ${mm.toFixed(1)}mm/h${Number.isFinite(proba) ? ` · ${proba}% de proba` : ''}`, { direction: 'top' });
      marker.addTo(group);
    });
      ...
      <div className="bg-ink-900 border border-ink-800 rounded-2xl p-3 space-y-2">
        ...
        {showRadar && showRainForecast && (
          <p className="text-[9px] text-ink-600 text-center">
            ⓘ Le radar (tuiles) montre la pluie déjà observée · les cercles montrent la prévision heure par heure — deux sources différentes affichées ensemble.
          </p>
        )}
