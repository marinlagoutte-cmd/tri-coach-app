...
export default function WeatherRadarMap() {
  ...
  // On ajoute donc ici une vraie prévision horaire de précipitations sur 24h (Open-Meteo,
  ...
  // Échelle de 3 TEINTES distinctes (pas juste une opacité qui varie) pour que les 3
  // paliers d'intensité restent nettement discernables une fois fondus dans la nappe
  // continue de pluie (voir le rendu canvas plus bas).
  function rainColor(mm) {
    if (mm < 0.2) return null; // zone sèche à cette heure/ce point : rien à peindre
    if (mm < 1) return '#7DD3FC'; // sky-300 — bruine
    if (mm < 4) return '#2563EB'; // blue-600 — pluie modérée
    return '#7C3AED'; // violet-600 — pluie soutenue
  }
  const RAIN_LEGEND = [
    { label: 'Bruine', range: '< 1mm/h', color: '#7DD3FC' },
    { label: 'Modérée', range: '1-4mm/h', color: '#2563EB' },
    { label: 'Soutenue', range: '> 4mm/h', color: '#7C3AED' },
  ];
  ...
  // ---------------------------------------------------------------------
  // Pluie — NAPPE ANIMÉE (demande explicite : remplacer les points isolés par un
  // rendu de zones continues, comme sur une carte météo pro type "screen" fourni :
  // aplats bleutés qui couvrent la zone concernée, pas des puces ponctuelles).
  // Même principe que le champ de particules de vent ci-dessous : on interpole
  // (IDW) la pluie de la grille chargée en tout point du canvas, on peint des
  // cellules dont le flou fusionne en nappe continue, puis on fait défiler des
  // stries obliques par-dessus pour donner une vraie sensation de pluie qui tombe.
  // ---------------------------------------------------------------------
  const rainCanvasRef = useRef(null);
  const rainAnimRef = useRef(null);
  const rainLastPatchRef = useRef(0);
  const showRainForecastRef = useRef(true);
  useEffect(() => { showRainForecastRef.current = showRainForecast; }, [showRainForecast]);

  const sampleRainAt = useCallback((lat, lon) => {
    ...
    for (const pt of grid) {
      const mm = pt.hourly?.precipitation?.[hIdx];
  ...
  useEffect(() => {
    ...
    // Nappe peinte sur un canvas hors-écran, recalculée seulement toutes les
    // PATCH_INTERVAL ms (l'interpolation + le flou coûtent) ; les stries, elles,
    // sont redessinées à CHAQUE frame par-dessus pour un vrai mouvement de pluie.
    const patchCanvas = document.createElement('canvas');
    const patchCtx = patchCanvas.getContext('2d');
    const CELL = 24; // px écran — taille de cellule de la nappe
    const PATCH_INTERVAL = 220; // ms
    let streakPhase = 0;
    let cellsCache = [];
    ...
    const tick = (ts) => {
      ...
      const map = mapRef.current;
      if (!showRainForecastRef.current || !map || !gridWindDataRef.current.length) return;

      if (patchCanvas.width !== canvas.width || patchCanvas.height !== canvas.height) {
        patchCanvas.width = canvas.width;
        patchCanvas.height = canvas.height;
        patchCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
      }

      // 1) Nappe continue (recalcul throttlé) : cellules floutées dont le flou
      // chevauche celui des voisines de même teinte -> fusionne en aplat continu,
      // comme les zones bleutées de la carte fournie en référence.
      if (ts - rainLastPatchRef.current >= PATCH_INTERVAL) {
        rainLastPatchRef.current = ts;
        patchCtx.clearRect(0, 0, w, h);
        const cols = Math.ceil(w / CELL) + 1;
        const rows = Math.ceil(h / CELL) + 1;
        const cells = [];
        patchCtx.filter = 'blur(9px)';
        for (let cy = 0; cy < rows; cy++) {
          for (let cx = 0; cx < cols; cx++) {
            const px = cx * CELL + CELL / 2;
            const py = cy * CELL + CELL / 2;
            const latlng = map.containerPointToLatLng([px, py]);
            const mm = sampleRainAt(latlng.lat, latlng.lng);
            if (!Number.isFinite(mm) || mm < 0.15) continue;
            const color = rainColor(mm);
            if (!color) continue;
            cells.push({ px, py, mm });
            patchCtx.globalAlpha = Math.min(0.75, 0.3 + mm / 9);
            patchCtx.fillStyle = color;
            patchCtx.beginPath();
            patchCtx.arc(px, py, CELL * 0.72, 0, Math.PI * 2);
            patchCtx.fill();
          }
        }
      ...
      ctx.drawImage(patchCanvas, 0, 0, w, h);

      // 2) Stries de pluie qui défilent en continu au-dessus des cellules mouillées —
      // c'est ce qui donne l'impression de pluie qui TOMBE plutôt qu'une tache statique.
      streakPhase = (streakPhase + 0.6) % (CELL * 2);
      ctx.strokeStyle = 'rgba(226,240,255,0.5)';
      ctx.lineWidth = 1.3;
      for (const c of cellsCache) {
        const density = c.mm < 1 ? 1 : c.mm < 4 ? 2 : 3;
        for (let i = 0; i < density; i++) {
          const offset = ((i * CELL) / density + streakPhase) % CELL;
          const sx = c.px - CELL / 2 + offset;
          const sy = c.py - CELL / 2;
          ctx.beginPath();
          ctx.moveTo(sx, sy);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-ink-900 border border-ink-800 rounded-2xl p-3 space-y-2">
        ...
        {showRadar && showRainForecast && (
          <p className="text-[9px] text-ink-600 text-center">
            ⓘ Le radar (tuiles) montre la pluie déjà observée · la nappe bleutée animée montre la prévision heure par heure — deux sources différentes affichées ensemble.
          </p>
        )}
        {(showWind || showRainForecast) && (
          <div className="space-y-1.5">
            ...
            <p className="text-[9px] text-ink-600 text-center">Curseur : prévision heure par heure sur les 24 prochaines heures.</p>
          </div>
        )}
      </div>
      ...
      <p className="text-[9px] text-ink-600 text-center">
        Radar pluie observé : RainViewer (temps réel + ~30min). Prévision pluie &amp; vent heure par heure : Open-Meteo (24h à venir).
      </p>
    </div>
  );
}
