...
export default function WeatherRadarMap() {
  ...
  // --- Prévision pluie (24h, même grille/horaire que le vent) ---
  // SEULE source de pluie affichée (demande explicite : plus de radar temps réel séparé,
  // qui doublonnait la timeline avec sa propre barre "passé 2h" — cette prévision horaire
  // Open-Meteo sur 24h, pilotée par le même curseur que le vent, est la seule qui reste).
  ...
  // Échelle RADAR MÉTÉO (dBZ) à 5 paliers — mêmes teintes que les radars pluie grand public
  // (Météo-France, RainViewer, Windy) : vert → jaune → orange → rouge → violet, du plus
  // léger au plus intense. Remplace l'ancienne échelle 3 teintes bleutées, plus proche
  // d'un pictogramme que d'un vrai radar.
  function rainColor(mm) {
    if (mm < 0.2) return null; // zone sèche à cette heure/ce point : rien à peindre
    if (mm < 1) return '#34D399'; // vert — pluie légère
    if (mm < 2.5) return '#A3E635'; // vert-jaune — pluie faible à modérée
    if (mm < 5) return '#FACC15'; // jaune — pluie modérée
    if (mm < 10) return '#FB923C'; // orange — pluie forte
    return '#E11D48'; // rouge — pluie très forte
  }
  const RAIN_LEGEND = [
  ...
  // Échelle de vitesse du vent — mêmes paliers que speedColor() (lib/windMap.js),
  // utilisée pour colorer les traînées de particules sur la carte.
  const WIND_LEGEND = [
    { label: 'Brise légère', range: '< 10 km/h', color: '#67E8F9' },
    { label: 'Modéré', range: '10-20 km/h', color: '#9A78FF' },
    { label: 'Soutenu', range: '20-35 km/h', color: '#FBBF24' },
    { label: 'Fort', range: '> 35 km/h', color: '#FF4D80' },
  ];

  // ---------------------------------------------------------------------
  // Pluie — NAPPE RADAR ANIMÉE, façon vrai site radar météo (Météo-France, RainViewer,
  // Windy) : palette 5 teintes vert→jaune→orange→rouge (voir rainColor plus haut), aplats
  // flous fusionnés en zones continues, et surtout un FONDU ENCHAÎNÉ (cross-fade) entre
  // l'heure précédente et l'heure sélectionnée quand le curseur avance — c'est exactement
  // l'effet "loop" d'un radar animé (les frames s'enchaînent en fondu progressif). On a
  // volontairement RETIRÉ les stries obliques défilantes de la version précédente : aucun
  // vrai radar pluie ne rend la pluie comme des traits qui tombent, ça ressemblait plus à
  // un effet de jeu vidéo qu'à un radar — un vrai radar ne montre QUE des aplats de couleur
  // qui bougent/évoluent d'une frame à l'autre, jamais de mouvement "de chute" superposé.
  // ---------------------------------------------------------------------
  const rainCanvasRef = useRef(null);
  const rainAnimRef = useRef(null);
  const showRainForecastRef = useRef(true);
  useEffect(() => { showRainForecastRef.current = showRainForecast; }, [showRainForecast]);

  const sampleRainAt = useCallback((lat, lon, hourIdx, grid) => {
    if (!grid.length) return null;
    let wSum = 0;
    let mmSum = 0;
    for (const pt of grid) {
      const mm = pt.hourly?.precipitation?.[hourIdx];
  ...
  useEffect(() => {
    ...
    // Deux canvas hors-écran ("A"/"B") : l'un affiche l'heure actuellement visible, l'autre
    // reçoit la nouvelle heure pendant qu'on fait le fondu de l'un vers l'autre — exactement
    // le mécanisme utilisé par les lecteurs de boucle radar (RainViewer, Windy...).
    const canvasA = document.createElement('canvas');
    const canvasB = document.createElement('canvas');
    const ctxA = canvasA.getContext('2d');
    const ctxB = canvasB.getContext('2d');
    const CELL = 24; // px écran — taille de cellule de la nappe
    const FADE_MS = 550; // durée du fondu enchaîné entre deux heures

    let active = 'A'; // canvas actuellement affiché à pleine opacité
    let renderedHourIdx = null; // heure "bakée" dans le canvas actif
    let renderedGrid = null; // référence de grille "bakée" dans le canvas actif (détecte un déplacement de carte)
    let transition = null; // { to, start, hourIdx, grid } pendant un fondu en cours

    const syncOffscreenSize = (cv) => {
    ...
    // Peint la nappe radar pour UNE heure/une grille donnée dans le contexte fourni — même
    // interpolation IDW + flou qu'avant, mais paramétrée explicitement (hourIdx, grid) plutôt
    // que de toujours lire "l'heure courante" par ref, pour pouvoir peindre à la fois
    // l'ancienne et la nouvelle heure lors d'un fondu.
    const paintField = (targetCtx, w, h, dpr, hourIdx, grid) => {
      ...
      for (let cy = 0; cy < rows; cy++) {
        for (let cx = 0; cx < cols; cx++) {
          ...
          const mm = sampleRainAt(latlng.lat, latlng.lng, hourIdx, grid);
          if (!Number.isFinite(mm) || mm < 0.15) continue;
          const color = rainColor(mm);
          if (!color) continue;
          targetCtx.globalAlpha = Math.min(0.72, 0.28 + mm / 12);
          targetCtx.fillStyle = color;
          targetCtx.beginPath();
          targetCtx.arc(px, py, CELL * 0.75, 0, Math.PI * 2);
          targetCtx.fill();
        }
      }
    ...
    const tick = (ts) => {
      rainAnimRef.current = requestAnimationFrame(tick);
      ...
      if (!showRainForecastRef.current || !map || !grid.length) return;

      syncOffscreenSize(canvasA);
      syncOffscreenSize(canvasB);

      const hourIdx = windHourIndexRef.current;
      const gridChanged = grid !== renderedGrid;
      const hourChanged = hourIdx !== renderedHourIdx;

      if (!transition && (hourChanged || gridChanged)) {
        if (renderedHourIdx === null || gridChanged) {
          // Premier rendu, OU la carte a bougé/zoomé (nouvelle zone) : repaint immédiat
          // sans fondu — un fondu n'a de sens que pour un changement d'HEURE, pas de zone.
          const activeCtx = active === 'A' ? ctxA : ctxB;
          paintField(activeCtx, w, h, dpr, hourIdx, grid);
          renderedHourIdx = hourIdx;
          renderedGrid = grid;
        } else {
          // Nouvelle heure sélectionnée (curseur déplacé, ou lecture automatique) : peint
          // la nouvelle heure dans le canvas INACTIF puis lance le fondu enchaîné vers lui.
          const toKey = active === 'A' ? 'B' : 'A';
          const toCtx = toKey === 'A' ? ctxA : ctxB;
          paintField(toCtx, w, h, dpr, hourIdx, grid);
          transition = { to: toKey, start: ts, hourIdx, grid };
        }
      }

      if (transition) {
        const t = Math.min(1, (ts - transition.start) / FADE_MS);
        const fromCanvas = active === 'A' ? canvasA : canvasB;
        const toCanvas = transition.to === 'A' ? canvasA : canvasB;
        ctx.globalAlpha = 1 - t;
        ...
        if (t >= 1) {
          ...
          renderedHourIdx = transition.hourIdx;
          renderedGrid = transition.grid;
          transition = null;
        }
      } else {
        const activeCanvas = active === 'A' ? canvasA : canvasB;
        ctx.drawImage(activeCanvas, 0, 0, w, h);
      }
    };
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (rainCanvas) {
        ...
        rainCanvas.getContext('2d').setTransform(dpr, 0, 0, dpr, 0, 0);
      }
    };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative rounded-2xl overflow-hidden border border-ink-800" style={{ height: '380px' }}>
        ...
        <canvas ref={rainCanvasRef} className="absolute inset-0 z-[6] pointer-events-none" />
      ...
      {/* Contrôles vent + prévision pluie (grille sur la zone visible, 24h, même curseur horaire) */}
      <div className="bg-ink-900 border border-ink-800 rounded-2xl p-3 space-y-2">
        <div className="flex items-center justify-between gap-2">
          <span className="text-[11px] font-black uppercase text-ink-50">💨 Vent &amp; 🌧️ Pluie (prévision 24h)</span>
        </div>
