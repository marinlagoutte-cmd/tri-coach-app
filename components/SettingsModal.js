...
import { getLocalDayStartEpochSec } from '../lib/stravaMatch';
...
export default function SettingsModal({ isOpen, onClose, session, onSignOut, onStravaSynced, onNewPlan, onDeletePlan }) {
  ...
  // true si le token Strava lié n'a PAS le scope profile:read_all (voir pages/api/strava/status.js) —
  // dans ce cas athlete.bikes/athlete.shoes (donc le kilométrage matériel, Outils > Matériel)
  // reste bloqué à 0 même avec un compte "connecté" : seule une reconnexion complète le corrige,
  // Strava n'accorde jamais ce scope rétroactivement à un token déjà autorisé.
  const [stravaMissingScope, setStravaMissingScope] = useState(false);
  ...
  useEffect(() => {
    ...
    fetch('/api/strava/status', {
      ...
      .then((data) => {
        if (cancelled) return;
        setStravaStatus(data?.connected ? 'connected' : 'disconnected');
        setStravaMissingScope(Boolean(data?.missingProfileScope));
      })
  ...
  const handleStravaSync = async () => {
    ...
    try {
      const res = await fetch('/api/strava/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // sinceEpochSec (minuit il y a 12 semaines = 84 jours, DANS LE FUSEAU RÉEL de
        // l'athlète) borne l'import côté serveur aux 12 dernières semaines glissantes —
        // voir pages/api/strava/sync.js. Aligné sur la fenêtre du graphe "Progression"
        // (components/WeeklyProgressChart.js, computeWeeklySportSeries { weeks: 12 }) pour
        // qu'il ait toujours assez d'historique importé, pas seulement le dernier mois.
        // Calculé ici plutôt que côté serveur (Vercel tourne en UTC).
        body: JSON.stringify({ accessToken: session.access_token, sinceEpochSec: getLocalDayStartEpochSec(84) }),
      });
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(data.error || 'sync failed');
      setStravaSyncMessage(t('settings.stravaSyncSuccess', { imported: data.imported || 0, skipped: data.skipped || 0, limited: data.limitedToRecentWindow || false }));
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* --- Plan d'entraînement --- */}
        {(onNewPlan || onDeletePlan) && (
          <div className="space-y-2">
            <span className="text-[10px] font-mono text-volt-400 uppercase tracking-widest block">{t('header.newPlanFull')}</span>
            <div className="flex gap-2">
              {onNewPlan && (
                <button
                  onClick={onNewPlan}
                  className="flex-1 text-xs font-bold bg-gradient-to-r from-volt-500 to-flare-500 hover:from-volt-400 hover:to-flare-400 text-white px-3 py-2 rounded-xl shadow-glow-sm transition-all flex items-center justify-center gap-1 active:scale-95"
                >
                  <span>+</span> {t('header.newPlanFull')}
                </button>
              )}
              {onDeletePlan && (
                <button
                  onClick={() => { onDeletePlan(); onClose(); }}
                  title={t('header.deletePlanTitle')}
                  className="shrink-0 text-xs font-bold text-ink-400 border border-ink-800 bg-ink-950 hover:bg-ink-900 px-3 py-2 rounded-xl transition-all active:scale-95"
                >
                  🗑
                </button>
              )}
            ...
            <div className="bg-ink-950 border border-ink-800 rounded-xl p-3 space-y-2">
              {stravaStatus === 'loading' ? (
                ...
                <>
                  ...
                  {stravaMissingScope && (
                    <div className="bg-amber-950/40 border border-amber-800 rounded-lg p-2 space-y-1.5">
                      <p className="text-[10px] text-amber-300 leading-relaxed">{t('settings.stravaMissingScopeHint')}</p>
                      <button
                        onClick={handleStravaConnect}
                        className="w-full text-[10px] font-bold text-white px-2.5 py-1.5 rounded-lg"
                        style={{ backgroundColor: '#FC4C02' }}
                      >
                        {t('settings.stravaReconnect')}
                      </button>
                    </div>
                  )}
