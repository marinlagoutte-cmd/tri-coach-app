    ...
    <div className="space-y-1.5">
      <div className="flex gap-1.5">
        <select
          ...
          className="flex-1 min-w-0 bg-ink-950 border border-ink-800 rounded-lg px-2 py-1.5 text-[11px] text-white"
        >
        ...
        <input
          ...
          className="w-14 shrink-0 bg-ink-950 border border-ink-800 rounded-lg px-1.5 py-1.5 text-[11px] text-white text-center"
        />
      ...
      {isCustom && (
        <div className="grid grid-cols-3 gap-1 bg-ink-950/60 border border-ink-800 rounded-lg p-1.5">
          <input
            ...
            className="col-span-3 bg-ink-900 border border-ink-800 rounded-lg px-2 py-1 text-[10px] text-white placeholder-ink-600"
          />
          <input
            placeholder="Glucides (g)" type="number" value={custom.carbs}
            onChange={(e) => setCustom({ ...custom, carbs: e.target.value })}
            className="bg-ink-900 border border-ink-800 rounded-lg px-1.5 py-1 text-[10px] text-white placeholder-ink-600"
          />
          <input
  ...
  return (
    <div className={`rounded-xl p-3 border ${marker.fixed ? 'bg-ink-950/60 border-ink-800' : 'bg-volt-500/5 border-volt-500/30'}`}>
      <div className="flex items-center gap-2">
        ...
        {marker.fixed ? (
          <span className="text-xs font-bold text-white flex-1">{marker.name}</span>
        ) : (
          <input
            value={marker.name}
            onChange={(e) => onUpdate({ name: e.target.value })}
            className="flex-1 min-w-0 bg-ink-950 border border-ink-800 rounded-lg px-2 py-1 text-xs text-white"
          />
        )}
        {!marker.fixed && (
          <>
            <input
              ...
              className="w-16 shrink-0 bg-ink-950 border border-ink-800 rounded-lg px-1.5 py-1 text-[11px] text-white text-center font-mono"
            />
...
export default function NutritionPlanner({ profile, sportType, constraints, trainingPlan, onSummaryChange }) {
  ...
  return (
    <div className="bg-ink-900 border border-ink-800 rounded-2xl p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xs font-black uppercase text-white">🧮 Stratégie nutrition course</h3>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <input
              ...
              className="w-full bg-ink-900 border border-ink-800 rounded-lg px-2 py-1.5 text-xs text-white font-mono"
            />
          </div>
          <div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 text-[10px] text-ink-300">
          <div className="bg-ink-950 border border-ink-800 rounded-lg px-2.5 py-1.5">Glucides : <strong className="text-white">{carbRange.min}-{carbRange.max}g/h</strong></div>
          <div className="bg-ink-950 border border-ink-800 rounded-lg px-2.5 py-1.5">Liquide : <strong className="text-white">{fluidRange.min}-{fluidRange.max}ml/h</strong></div>
          <div className="bg-ink-950 border border-ink-800 rounded-lg px-2.5 py-1.5">Sodium : <strong className="text-white">{sodiumRange.min}-{sodiumRange.max}mg/h</strong></div>
          <div className="bg-ink-950 border border-ink-800 rounded-lg px-2.5 py-1.5">Potassium : <strong className="text-white">{potassiumRange.min}-{potassiumRange.max}mg/h</strong></div>
        </div>
