...
// Statut → couleur du tracé de la jauge (repris de STATUS_STYLE, en version trait SVG).
const GAUGE_STROKE = {
  low: '#fbbf24', // amber-400
  ok: '#34d399', // emerald-400
  high: '#fb923c', // orange-400
  neutral: '#7b818a', // ink-500
};

function MetricTile({ label, value, unit, range, rangeUnit }) {
  const status = rangeStatus(value, range);
  // Jauge circulaire : progression vers le haut de la cible plutôt qu'un simple
  // chiffre brut. Sans cible connue, la jauge se remplit à 50% (état neutre).
  const pct = range && range.max > 0 ? Math.min(1, value / range.max) : 0.5;
  const radius = 15;
  const circumference = 2 * Math.PI * radius;
  const offset = circumference * (1 - pct);
  return (
    <div className={`rounded-xl p-2.5 border flex items-center gap-2.5 ${STATUS_STYLE[status]}`}>
      <svg width="36" height="36" viewBox="0 0 36 36" className="shrink-0 -rotate-90">
        ...
        <circle
          ...
          stroke={GAUGE_STROKE[status]} strokeWidth="4" strokeLinecap="round"
          strokeDasharray={circumference} strokeDashoffset={offset}
        />
      </svg>
      <div className="min-w-0">
        <span className="text-[9px] uppercase tracking-wide block opacity-80">{label}</span>
        <span className="text-base font-black font-mono block leading-tight">{Math.round(value)}{unit}</span>
        {range && (
