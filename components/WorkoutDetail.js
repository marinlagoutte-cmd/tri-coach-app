    ...
    return (
      <div className="bg-amber-500/10 border border-amber-500/30 p-3 rounded-xl space-y-2">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={onKeep} className="flex-1 bg-ink-800 hover:bg-ink-700 text-white font-bold px-3 py-2 rounded-xl text-[11px] min-h-tap">
            Garder comme ça
          </button>
        </div>
      </div>
  ...
  if (!showForm) {
    return (
      <button
        ...
        className="w-full bg-ink-800 hover:bg-ink-700 text-white font-bold px-3 py-2.5 rounded-xl text-xs uppercase min-h-tap"
      >
        ✔️ Valider la séance
      </button>
    );
...
function FieldsGrid({ workout, title, dimmed }) {
  ...
  return (
    <div className={dimmed ? 'opacity-60' : ''}>
      ...
      <p className="text-xs font-bold text-white mb-1.5">{workout.title} · <span className="font-mono text-ink-400">{workout.duration}</span></p>
      {workout.structure && (
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div className="bg-ink-900 border border-ink-800 w-full max-w-sm rounded-3xl p-5 space-y-4 shadow-2xl text-ink-100 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-start border-b border-ink-800 pb-3">
          ...
          <button onClick={onClose} className="text-ink-400 hover:text-white font-bold p-1 min-h-tap min-w-[44px]">✕</button>
        </div>
        ...
        <div className="flex justify-end pt-2 border-t border-ink-800 text-xs">
          <button
            onClick={onClose}
            className="bg-ink-800 hover:bg-ink-700 text-white font-bold px-4 py-2 rounded-xl text-xs uppercase min-h-tap"
          >
            Fermer
          </button>
        </div>
