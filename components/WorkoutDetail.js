...
function FieldsGrid({ workout, title, dimmed }) {
  ...
  return (
    <div className={dimmed ? 'opacity-60' : ''}>
      ...
      {/* Note des garde-fous automatiques (séance allégée, durée réduite, volume complété…) :
          séparée de la description pour ne jamais polluer la feuille de séance. */}
      {!dimmed && typeof workout.autoNote === 'string' && workout.autoNote && (
        <p className="text-[10px] text-ink-400 italic leading-snug mb-2">ⓘ {workout.autoNote}</p>
      )}
      {workout.structure && (
        <div className="bg-volt-500/5 border border-volt-500/20 p-2.5 rounded-lg mb-2">
