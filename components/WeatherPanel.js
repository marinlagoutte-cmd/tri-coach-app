...
export default function WeatherPanel() {
  const [view, setView] = useState('summary'); // 'summary' | 'radar'
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-black uppercase text-ink-50">🌦️ Météo d'entraînement</h2>
        {view === 'summary' && (
          <button
            onClick={loadWeather}
            disabled={loading}
      ...
      {/* Bascule Résumé / Carte radar — la carte radar est le vrai outil de préparation
          des sorties (pluie heure par heure + vent), le résumé reste utile pour un
          coup d'œil rapide sur la semaine. */}
      <div className="flex bg-ink-950 border border-ink-800 rounded-xl p-1 gap-1">
        <button
          ...
          className={`flex-1 text-[11px] font-bold py-1.5 rounded-lg transition-colors ${
            view === 'summary' ? 'bg-ink-800 text-volt-400 border border-ink-700' : 'text-ink-400'
          }`}
        >
          📋 Résumé
        </button>
        <button
          ...
          className={`flex-1 text-[11px] font-bold py-1.5 rounded-lg transition-colors ${
            view === 'radar' ? 'bg-ink-800 text-volt-400 border border-ink-700' : 'text-ink-400'
          }`}
        >
          🗺️ Carte radar &amp; vent
        </button>
      ...
      {cw && (
        <div className="bg-ink-900 border border-ink-800 rounded-2xl p-4 flex items-center justify-between">
          <div>
            ...
            <span className="text-3xl font-black text-ink-50">{Math.round(cw.temperature)}°C</span>
          </div>
        ...
        <div className="grid grid-flow-col auto-cols-[minmax(88px,1fr)] gap-2 overflow-x-auto pb-1">
          {daily.time.map((date, i) => (
            <div key={date} className="bg-ink-900 border border-ink-800 rounded-xl p-2.5 text-center flex flex-col items-center gap-1">
              ...
              <span className="text-[10px] font-mono text-ink-50 font-bold">{Math.round(daily.temperature_2m_max[i])}°</span>
