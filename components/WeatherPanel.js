...
export default function WeatherPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading && !weather && <p className="text-xs text-slate-500 animate-pulse">Localisation et récupération de la météo...</p>}

      {cw && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between">
          <div>
            <span className="text-[10px] font-mono text-orange-400 uppercase tracking-widest block">{placeName}</span>
      ...
      {daily && (
        <div className="grid grid-flow-col auto-cols-[minmax(88px,1fr)] gap-2 overflow-x-auto pb-1">
          {daily.time.map((date, i) => (
            <div key={date} className="bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-center flex flex-col items-center gap-1">
              ...
              <span className="text-[9px] font-mono text-slate-500">{Math.round(daily.temperature_2m_min[i])}°</span>
