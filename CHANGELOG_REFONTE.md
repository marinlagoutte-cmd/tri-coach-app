...
## -2. Co-génération Gemini + Groq (gratuit) — remplace la première version Claude (payante)

**Contexte :** la première implémentation de la co-génération (double-check réel de chaque
séance) utilisait Claude comme 2e IA. L'API Claude étant payante à l'usage, remplacée par
**Groq** (gratuit, aucune carte bancaire requise — https://console.groq.com/keys), modèle
`llama-3.3-70b-versatile` par défaut.

**Ce qui n'a PAS changé** (voir section précédente pour le détail complet de la logique) :
- Gemini et Groq reçoivent exactement le même prompt et passent par la même chaîne de
  garde-fous déterministes (`provider` propagé dans `generatePlanWithAI` /
  `regenerateWeekWithAI` / `reviewPlanCoherenceWithAI`, lib/gemini.js).
- Comparaison 100% déterministe en JS (structure, durée ±25%, volume natation ±30%),
  aucun appel IA supplémentaire pour arbitrer.
- Désaccord → 1 round complet relancé → toujours désaccord → compromis déterministe
  (moyenne durée/volume, structure Gemini conservée si non fusionnable). Convergence
  garantie en 2 rounds maximum.
- Une seule IA disponible → l'autre prend le relais seule, note transparente dans
  `autoFixNotes`/`qualityWarnings`.

**Fichiers :**
- `lib/groq.js` (nouveau) : appelant Groq, même contrat d'interface que `callGeminiJSON`
  (candidats de modèles, retries, extraction JSON, classification d'erreur).
- `lib/gemini.js` : ajout du dispatcher `callAIJSON(prompt, provider)` et du paramètre
  `provider` sur `generatePlanWithAI`, `regenerateWeekWithAI`, `reviewPlanCoherenceWithAI`.
  `chatWithCoach` et les routes nutrition restent Gemini seul (hors périmètre demandé).
- `lib/coGeneration.js` : renommé conceptuellement provider Claude → Groq, logique de
  comparaison/retry/compromis inchangée.
- `pages/api/generate-plan.js`, `pages/api/regenerate-week.js` : messages d'erreur mis à
  jour pour mentionner `GEMINI_API_KEY` + `GROQ_API_KEY` (plus de mention de Claude).

**À faire côté déploiement :** ajouter `GROQ_API_KEY` dans Vercel → Settings → Environment
Variables (clé gratuite sur console.groq.com). `ANTHROPIC_API_KEY` n'est plus utilisé et
peut être retiré si présent depuis la version précédente.
