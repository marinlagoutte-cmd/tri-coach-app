...
## 6. Protocole de vérification (double/triple-check) et prompts-cadres par domaine

**Obligatoire pour toute tâche** : avant de considérer une tâche terminée, suivre le
protocole détaillé dans `AI_DOMAIN_PROMPTS.md` — cadrage, implémentation,
double-check (auto-vérification à froid), triple-check (posture indépendante :
angle athlète, angle casse silencieuse, angle domaine). Ce fichier contient aussi un
prompt-cadre par domaine métier (génération de plan, physiologie/zones, nutrition,
pression de pneu, matériel, Strava, sync/auth, i18n, météo, UI) avec les règles non
négociables et les sources qui font autorité pour chacun — à lire avant de travailler
dans ce domaine, pas seulement `AI_GUIDE.md` §3.

## 7. Journal des interventions IA
...
- 2026-08-28 — Claude (Sonnet 5, claude.ai) — Corrigé 2 bugs signalés : (1)
  `pages/index.js` (régénération d'une semaine seule) affichait encore
  `data.qualityWarnings` dans le chat malgré la demande de retrait déjà appliquée sur
  la génération d'un nouveau plan — même correction appliquée ici (warnings toujours
  disponibles côté API, plus affichés dans le chat) ; (2) `lib/groq.js` pointait vers
  `llama-3.3-70b-versatile`/`llama-3.1-8b-instant`, dépréciés et retirés par Groq
  (arrêt le 16/08/2026, d'où l'erreur "model_not_found") — remplacés par
  `openai/gpt-oss-120b`/`openai/gpt-oss-20b` (migration recommandée par Groq).
- 2026-08-28 — Claude (Sonnet 5, claude.ai) — Créé `AI_DOMAIN_PROMPTS.md` : protocole
  obligatoire de double/triple vérification pour toute tâche IA sur ce projet, +
  prompt-cadre expert par domaine métier (génération de plan, physio/zones,
  nutrition, pression de pneu, matériel, Strava, sync/auth, i18n, météo, UI) avec
  règles non négociables et sources faisant autorité pour chacun. Référencé depuis ce
  fichier (§6). Demande explicite de l'athlète suite à l'incident du même jour (fix
  appliqué à un seul des deux endroits nécessaires).
- 2026-08-28 — Claude (Sonnet 5, claude.ai) — Précision de l'athlète : la demande de
  double/triple vérification portait sur les IA APPELÉES PAR L'APP (Gemini/Groq via
  `lib/gemini.js`/`lib/coGeneration.js`), pas sur les IA qui codent dessus. Audit
  complet effectué : toutes les routes API passent déjà exclusivement par
  `lib/coGeneration.js` (aucun bypass direct vers `lib/gemini.js` trouvé), donc le
  double-check Gemini+Groq est déjà systématique sur chaque fonctionnalité IA
  (génération de plan, régénération de semaine, chat, nutrition, validation de
  zones, analyse Strava). Point faible identifié et corrigé : la relecture de
  cohérence d'un plan généré (`reviewPlanCoherenceWithAI`) était faite par le MÊME
  modèle que celui qui avait généré (auto-relecture, angle mort structurel) —
  bascule vers une relecture CROISÉE (`oppositeProvider()` dans `lib/gemini.js` :
  Gemini génère → Groq relit, et inversement) pour `generatePlanWithAI` et
  `regenerateWeekWithAI`, avec note de transparence (`autoFixNotes`) si la
  relecture croisée échoue. Coût accepté : double les appels IA pour cette passe.
