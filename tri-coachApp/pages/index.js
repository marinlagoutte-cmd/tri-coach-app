...
export default function Home() {
  ...
  const handleForceRegenerateWeek = async (weekKey) => {
    ...
    try {
      ...
      // NB : data.qualityWarnings n'est volontairement plus affiché dans le chat (retiré
      // à la demande — même choix que pour data.coherenceWarnings/autoFixNotes dans
      // handleWizardComplete ci-dessus, voir le commentaire associé). Ce fix couvrait la
      // génération d'un nouveau plan mais avait été oublié ici, sur la régénération d'une
      // semaine seule — d'où le message qui continuait à apparaître dans ce cas précis.
      // Les données restent disponibles dans data.qualityWarnings côté API si besoin de
      // les réafficher ailleurs plus tard (ex. un badge discret sur la semaine concernée).
    } catch (err) {
