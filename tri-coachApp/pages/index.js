  ...
  const handleWizardComplete = async (wizardData) => {
    ...
    try {
      ...
      const res = await fetch('/api/generate-plan', {
        ...
        body: JSON.stringify({ wizardData, profile, feedbackHistory, healthHistory, manualPaceZones: paceZones, injuryLog, raceCalendar, menstrualCycle, language: lang }),
      });
  ...
  const sendCoachMessage = async (userText, { intent = null, displayText = null } = {}) => {
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: userText, profile, workouts, trainingPlan, intent, sportType, constraints, feedbackHistory, healthHistory, manualPaceZones: paceZones, injuryLog, raceCalendar, menstrualCycle, language: lang }),
      });
      const data = await res.json();
