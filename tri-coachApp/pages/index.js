...
const TOOLS_SUB_TABS = ['nutrition', 'weather', 'route', 'tirePressure', 'equipment', 'records'];
      ...
      <main key={activeTab} className="flex-1 max-w-md w-full mx-auto p-4 space-y-4 animate-fadeIn">
        ...
        {activeTab === 'tools' && (
          <div className="space-y-4">
            {/* grid-cols-6 : CORRIGÉ (était grid-cols-5, laissant le 6e sous-onglet
                "records" retomber sur une ligne implicite à part depuis son ajout — les
                6 sous-onglets tiennent maintenant sur une seule ligne). */}
            <div className="grid grid-cols-6 gap-1 bg-ink-900 border border-ink-800 rounded-2xl p-1">
              {TOOLS_SUB_TABS.map((sub) => (
                ...
                >
                  {sub === 'tirePressure' ? t('tools.tirePressureSubTab')
                    ...
                    : sub === 'records' ? t('tools.recordsSubTab')
                    : t(`tabs.${sub}`)}
                </button>
              ))}
            </div>
