...
export default function Home() {
  ...
  const handleForceRegenerateWeek = async (weekKey) => {
    ...
    try {
      ...
      const coachMsg = `🔁 **Semaine ${weekLabel} régénérée** à ta demande, en gardant l'autre semaine et le reste du plan inchangés.`;
      // NB : data.qualityWarnings (statut interne du double-check Gemini+Groq, ex. "Double-check
      // indisponible cette fois...") n'est volontairement plus affiché dans le chat — même
      // raison que data.coherenceWarnings/autoFixNotes plus bas : ces récaps techniques n'ont
      // rien à faire dans l'app. Toujours renvoyé par l'API si besoin de le ré-exposer ailleurs,
      // ex. le panneau de diagnostic IA (Réglages → IA).
