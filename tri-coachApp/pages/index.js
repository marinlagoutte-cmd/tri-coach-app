...
function getSmartSuggestions({ lastCoachText, nextWorkout, lang, t }) {
  const suggestions = [];
  const mentionsAutoFix = /Ajustements automatiques/i.test(lastCoachText || '');
  const mentionsWarnings = /Points de vigilance/i.test(lastCoachText || '');

  if (mentionsAutoFix || mentionsWarnings) {
    suggestions.push({
      label: t('chat.suggestExplain'),
      text: mentionsAutoFix
        ? "Peux-tu m'expliquer pourquoi tu as fait ces ajustements automatiques ?"
        : 'Peux-tu m\'expliquer les points de vigilance que tu as soulevés ?',
      intent: null,
    });
  }
