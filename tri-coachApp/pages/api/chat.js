...
import { mergeWorkoutPatches, checkSessionCountCoherence, rebalanceSameDisciplineDoubles, enforceMaxSessionsPerDay, enforceThirdSessionLowIntensity, dedupeIdenticalSameDaySessions, sanitizeWorkout, findPaceZoneMismatches } from '../../lib/workouts';
...
export default async function handler(req, res) {
  ...
  try {
    // co-génération Gemini + Groq (voir lib/coGeneration.js:coChatWithCoach) : un
    // ajustement de séance demandé via le chat est désormais double-checké par les
    // deux IA, comme la génération de plan initiale et la régénération de semaine —
    // demande explicite de l'athlète (auparavant ce endpoint n'appelait QUE Gemini).
    const { reply, patches } = await coChatWithCoach({ message, profile, workouts, trainingPlan, intent, sportType, constraints, feedbackHistory, healthHistory, manualPaceZones, language: lang });
    ...
    // GARDE-FOU AJOUTÉ : signale explicitement à l'athlète toute séance dont l'allure
    // chiffrée ne correspond pas à la zone qu'elle prétend cibler (voir lib/workouts.js:
    // findPaceZoneMismatches) — plutôt que de laisser le coach affirmer "j'ai recalé sur ta
    // Z2" alors que la valeur réellement renvoyée est ailleurs. Non-bloquant (on ne réécrit
    // rien automatiquement, l'athlète garde la main) mais toujours visible.
    let finalReply = reply;
    if (patches?.length) {
      const allMismatches = ['N', 'N+1'].flatMap((weekKey) => findPaceZoneMismatches(updatedWorkouts[weekKey], profileForSanitize));
      if (allMismatches.length) {
        const lines = allMismatches.map((m) => `- ${m.day} (${m.title}) : annoncée en ${m.announcedZone} mais ${m.intensity} correspond en fait à ta ${m.actualZone}`);
        finalReply = `${reply}\n\n⚠️ **Allure à revérifier** — décalage détecté entre la zone annoncée et l'allure chiffrée :\n${lines.join('\n')}`;
      }
    }

    return res.status(200).json({ reply: finalReply, updatedWorkouts });
  } catch (error) {
