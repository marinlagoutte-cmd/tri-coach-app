import { coGenerateNutritionAdvice, coAnswerNutritionQuestion } from '../../lib/coGeneration';
...
export default async function handler(req, res) {
  ...
  // Rate limit léger par IP (voir lib/rateLimit.js).
  const { allowed, retryAfterSec } = checkRateLimit(req, { id: 'nutrition', limit: 12, windowMs: 60_000 });
  if (!allowed) {
    return res.status(200).json({ error: RATE_LIMIT_MESSAGES[lang](retryAfterSec) });
  }
  ...
  try {
    if (question && String(question).trim()) {
      // co-génération Gemini + Groq (voir lib/coGeneration.js:coAnswerNutritionQuestion) —
      // demande explicite de l'athlète : même protocole double-IA que la génération de plan.
      const { answer, verified } = await coAnswerNutritionQuestion({ profile, trainingPlan, question, raceProfile, planSummary, language: lang });
      return res.status(200).json({ answer, verified });
    }
    const result = await coGenerateNutritionAdvice({ profile, trainingPlan, sportType, raceProfile, language: lang });
