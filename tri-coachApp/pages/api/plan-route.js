...
export default async function handler(req, res) {
  ...
  try {
    ...
    const winner = topCandidates[aiResult.pickedIndex] || topCandidates[0];
    const gpx = buildGPX(winner.points, `Parcours vélo ${Math.round(winner.distanceKm)}km`);

    return res.status(200).json({
      winner: {
        points: winner.points.map((p) => ({ lat: p.lat, lon: p.lon, ele: p.ele ?? null })),
        distanceKm: Math.round(winner.distanceKm * 10) / 10,
        ascentM: winner.ascentM,
        wind: { distTailKm: winner.wind.distTailKm, distHeadKm: winner.wind.distHeadKm, distCrossKm: winner.wind.distCrossKm, netScore: winner.wind.netScore },
        popularityScore: winner.popularityScore,
        compositeScore: winner.compositeScore,
      },
      alternatives: topCandidates.filter((c) => c !== winner).map((c) => ({
        distanceKm: Math.round(c.distanceKm * 10) / 10,
        wind: { netScore: c.wind.netScore },
    ...
    const message = e?.code === 'NO_KEY'
      ? e.message
      : "La génération du parcours a échoué. Vérifie le point de départ (accessible en vélo) et réessaie dans un instant.";
    return res.status(e?.code === 'NO_KEY' ? 503 : 500).json({ error: message });
  }
}
