...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data } = await admin.from('strava_tokens').select('athlete_id').eq('user_id', userData.user.id).maybeSingle();

    return res.status(200).json({ connected: Boolean(data), athleteId: data?.athlete_id || null });
  } catch (e) {
