...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Toutes les activités (vélo + course + natation) : les records course/natation n'ont pas
    // besoin de streams (juste distance/temps de la table déjà synchronisée), seul le vélo a
    // besoin du détail seconde par seconde. Limite relevée à 3000 (au lieu de 500) — depuis que
    // pages/api/strava/auto-sync.js importe l'historique COMPLET en arrière-plan (et plus
    // seulement les 12 dernières semaines), un vieux record encore valable pouvait tomber en
    // dehors des 500 activités les plus récentes et donc ne jamais apparaître ici. NB : selon
    // la config Supabase (db.max_rows), une limite serveur peut plafonner ceci plus bas — voir
    // SUPABASE_SETUP.md si un athlète très ancien/actif semble encore tronqué.
    ...
    // Info affichée dans l'onglet Records pour que l'athlète comprenne que la fraîcheur des
    // données ne dépend plus du bouton "Actualiser" seul (voir components/PerformanceRecords.js).
    const { data: tokenMeta } = await admin
      .from('strava_tokens')
      .select('full_history_synced_at, last_synced_at')
      .eq('user_id', userId)
      .maybeSingle();
    ...
    return res.status(200).json({
      ...
      autoSync: {
        fullHistorySyncedAt: tokenMeta?.full_history_synced_at || null,
        lastSyncedAt: tokenMeta?.last_synced_at || null,
      },
