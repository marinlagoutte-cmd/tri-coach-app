...
//   texte d'analyse. ai_analysis reste donc null pour les activités importées ici — SAUF
//   pour celles de la semaine calendaire en cours (voir analyzeCurrentWeekWithAI dans
//   lib/stravaSync.js), volontairement analysées ici aussi pour permettre de vérifier le
//   rendu de l'analyse IA détaillée (laps) sans attendre un vrai événement webhook.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const first = await importStravaActivities({
      ...
      // Demande explicite de l'athlète : sur cet import manuel, analyse IA détaillée (laps +
      // double-check Gemini/Groq) pour les activités de la semaine en cours, pour vérifier le
      // rendu sans attendre un vrai événement webhook (voir lib/stravaSync.js).
      analyzeCurrentWeekWithAI: true,
    });
