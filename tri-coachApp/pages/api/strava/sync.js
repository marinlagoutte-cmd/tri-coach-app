...
// les 30 DERNIERS JOURS (et non plus 2 semaines calendaires) — un mois complet est le
// minimum pour que les graphes de l'onglet Profil (charge, distribution des zones,
// tendance des métriques auto-détectées, voir lib/zones.js) aient assez de séances à
// afficher pour un athlète qui vient tout juste de lier son compte. `sinceEpochSec`
// (minuit il y a 30 jours) est calculé CÔTÉ CLIENT par SettingsModal.js (voir
// getLocalDayStartEpochSec dans lib/stravaMatch.js) — le serveur Vercel tourne en UTC et
// n'a aucune notion fiable du fuseau horaire réel de l'athlète, donc ce repère doit venir
// du navigateur.
// GARDE-FOU VOLUME : si malgré cette fenêtre d'un mois le nombre d'activités récupérées
// dépasse MAX_ACTIVITIES_ONE_MONTH (cas d'un athlète multi-sports très actif, ou d'une
...
// La fenêtre (1 mois) ne nécessite normalement qu'une seule page même pour un athlète
...
// Seuil arbitraire mais généreux (bien plus qu'un mois d'entraînement normal, même pour
// un triathlète en grosse charge — jusqu'à ~4-5 séances/jour tous sports confondus) : au-
// delà, on suppose un cas hors norme et on se recentre sur les 7 derniers jours plutôt
// que de tout importer d'un coup.
const MAX_ACTIVITIES_ONE_MONTH = 150;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // GARDE-FOU VOLUME (voir commentaire d'en-tête) : si même sur 1 mois le volume est
    ...
    if (allActivities.length > MAX_ACTIVITIES_ONE_MONTH) {
