...
export default function TrainingLogChart({ activities = [], profile }) {
  ...
  return (
    <div className="bg-ink-950 border border-ink-800 rounded-xl p-3">
      ...
      {/* BUG RÉEL CORRIGÉ : ces boutons étaient DANS la branche "hasAnyActivity" ci-dessus —
          en remontant assez loin dans l'historique pour tomber sur une fenêtre de semaines
          sans la moindre activité synchronisée (page vide), tout le bloc de pagination
          disparaissait avec elle, y compris "Revenir à aujourd'hui" : plus aucun moyen de
          revenir en avant, l'athlète restait bloqué dans le passé. Ces boutons sont
          maintenant TOUJOURS affichés dès qu'il y a un minimum d'activités Strava connues
          (indépendamment du contenu de la page actuellement affichée), pour rester
          navigables quelle que soit la fenêtre de semaines en cours. */}
      {activities?.length > 0 && (
