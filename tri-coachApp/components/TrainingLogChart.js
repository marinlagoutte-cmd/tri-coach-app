...
const MIN_RADIUS = 5;
const MAX_RADIUS = 30;
const EMPTY_RADIUS = 4; // bulle "témoin" pour une activité sans valeur sur la métrique choisie (ex: renfo en Distance)
              ...
              <div key={week.key} className="flex items-stretch gap-1">
                ...
                <div className="grid flex-1 rounded-lg bg-ink-900/60" style={{ gridTemplateColumns: 'repeat(7, minmax(0, 1fr))' }}>
                  {week.days.map((dayActs, dayIdx) => (
                    <div key={dayIdx} className="flex flex-wrap items-center justify-center gap-0.5 py-1.5 min-h-[2.75rem]">
                      {dayActs.map((a) => {
                        const value = a[metricKey];
                        const r = radiusFor(value, maxValue);
                        const showLabel = r >= 14 && Number.isFinite(value) && value > 0;
                        return (
                          <div
                            key={a.id}
