...
export default function ZoneCharts({ profile, activities, onPaceZonesChange }) {
  ...
  useEffect(() => {
    const hrRunSeed = hasStoredValue(STORAGE_KEYS.hrZones)
      ? { zones: loadFromStorage(STORAGE_KEYS.hrZones, null), source: 'manual' }
      : resolveSeedZones(profile?.fcMax, defaultHrZones, autoRunEstimate.hrZones);
    setHrZonesRun(hrRunSeed.zones);

    const hrBikeSeed = hasStoredValue(STORAGE_KEYS.hrZonesBike)
      ? { zones: loadFromStorage(STORAGE_KEYS.hrZonesBike, null), source: 'manual' }
      : resolveSeedZones(profile?.fcMax, defaultHrZones, autoBikeEstimate.hrZones);
    setHrZonesBike(hrBikeSeed.zones);

    const powerRunSeed = hasStoredValue(STORAGE_KEYS.powerZones)
      ? { zones: loadFromStorage(STORAGE_KEYS.powerZones, null), source: 'manual' }
      : resolveSeedZones(profile?.ftp, defaultPowerZones, autoRunEstimate.powerZones);
    setPowerZonesRun(powerRunSeed.zones);

    const powerBikeSeed = hasStoredValue(STORAGE_KEYS.powerZonesBike)
      ? { zones: loadFromStorage(STORAGE_KEYS.powerZonesBike, null), source: 'manual' }
          ...
          >
            ...
            {metricTab === 'pace' ? (
              <input
                ...
                className={`w-16 text-right bg-ink-900 border border-ink-800 rounded-lg px-2 py-1 font-mono text-[11px] ${
