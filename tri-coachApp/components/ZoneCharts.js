    ...
    <div className="bg-ink-950 border border-ink-800 rounded-xl p-3 space-y-3">
      ...
      <div className="space-y-1.5">
        {activeZones.map((z, i) => {
          ...
          return (
            // BUG RÉEL CORRIGÉ (v1) : la clé doit inclure `discipline` ET `metricTab`. Les
            // champs ci-dessous sont non contrôlés (`defaultValue`, pas `value`) — React ne
            // relit `defaultValue` qu'au MONTAGE d'un nœud. Avec une clé partagée entre
            // onglets/disciplines (juste `z.zone`, identique pour Z1..Z5 partout), changer
            // d'onglet OU de discipline réutilisait le MÊME <input> DOM plutôt que d'en créer
            // un nouveau : le champ restait figé sur la valeur brute affichée précédemment au
            ...
            //
            // BUG RÉEL CORRIGÉ (v2, signalé par l'athlète — écart entre l'affichage normal
            // et l'affichage une fois le crayon cliqué) : en mode 'manual', la partie
            // "valeur" de la clé restait le texte STATIQUE `'manual'` — donc IDENTIQUE avant
            // et après une modification des bornes (la clé ne dépendait alors que de
            // `discipline`/`metricTab`/`editing`, jamais de la valeur elle-même). Conséquence
            // concrète : dès qu'un champ changeait de valeur SANS changement d'onglet/
            // discipline/mode édition dans le même rendu (ex: éditer la borne basse d'une
            // zone fait recalculer `highValue` affiché par la zone précédente, qui dépend de
            // `nextZone.min` — voir plus haut), React réutilisait le même <input> DOM et son
            // `defaultValue` ne se mettait PAS à jour : le champ affichait encore l'ancienne
            // valeur. C'est exactement l'écart rapporté ("normal" vs "après clic crayon").
            // On inclut donc maintenant la valeur RÉELLEMENT affichée (bornes basse ET haute,
            // déjà formatées via `formatVal` pour englober aussi bien allure que FC/Puissance)
            // dans la clé : tout changement de valeur — édition manuelle, ou répercussion sur
            // une zone voisine — force désormais un remount avec la valeur fraîche.
            <div
              key={`${discipline}-${metricTab}-${z.zone}-${editing}-${formatVal(z.min)}-${formatVal(highValue)}`}
