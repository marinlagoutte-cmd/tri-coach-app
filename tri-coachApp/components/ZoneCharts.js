...
export default function ZoneCharts({ profile, activities, onPaceZonesChange }) {
  ...
  // Réinitialise le brouillon (+ ferme l'édition, + efface une éventuelle erreur d'un
  // onglet précédent) UNIQUEMENT à un vrai changement de discipline/onglet — jamais une
  // saisie abandonnée d'un onglet ou d'une discipline précédente ne doit survivre à ce
  // changement.
  // BUG RÉEL CORRIGÉ : cet effet dépendait auparavant AUSSI de hrZonesRun/hrZonesBike/
  // powerZonesRun/powerZonesBike/paceZones — donc du résultat de "Valider" lui-même
  // (handleValidate appelle le setter correspondant, ex. setPaceZones). Juste après
  // qu'un enregistrement réussi affichait "✅ Zones enregistrées" (setValidationResult
  // dans handleValidate), CET effet se redéclenchait aussitôt à cause du changement de
  // paceZones/hrZonesRun/etc. et appelait setValidationResult(null) — effaçant le
  // message de confirmation avant même que l'athlète ait pu le voir, donnant
  // l'impression trompeuse qu'enregistrer "ne faisait rien".
  useEffect(() => {
  ...
  // Pendant de l'effet ci-dessus, mais pour un changement des zones "committées" venant
  // d'une cause EXTERNE à une action explicite de l'athlète sur CET onglet (chargement
  // localStorage au montage, arrivée async du profil — voir l'effet plus haut) : on
  // resynchronise seulement le brouillon affiché, sans toucher à validationResult/
  // editing (déjà gérés explicitement par handleValidate juste après un "Valider").
  useEffect(() => {
    setDraftZones(committedZones);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hrZonesRun, hrZonesBike, powerZonesRun, powerZonesBike, paceZones]);
  ...
  async function handleValidate() {
    ...
    setValidationResult({
      type: 'success',
      // aiResult.doubleCheckNote (statut interne du double-check Gemini+Groq) n'est
      // volontairement plus concaténé au message affiché à l'athlète — demande
      // explicite : ces détails techniques (ex. panne d'un des deux fournisseurs)
      // n'ont rien à faire dans l'app. Toujours calculé côté serveur si besoin de le
      // ré-exposer ailleurs, ex. le panneau de diagnostic IA (Réglages → IA).
      message:
        metricTab === 'pace'
  ...
  return (
    <div className="bg-ink-950 border border-ink-800 rounded-xl p-3 space-y-3">
      ...
      {/* Bouton crayon "en haut" (demande explicite de l'athlète) : point d'entrée UNIQUE
          ...
          BUG RÉEL CORRIGÉ : cette rangée passait auparavant de "affichée" (hauteur ~20px,
          bouton crayon) à "totalement absente du DOM" dès `editing=true` — le contenu
          juste en dessous (liste des zones) remontait donc d'un coup à l'ouverture de
          l'édition, avant de redescendre à la fermeture ("Annuler"/"Valider"). La rangée
          est maintenant TOUJOURS rendue (même hauteur dans les deux états), seul son
          contenu change — plus de saut visuel entre affichage normal et affichage édition. */}
      {zonesMode === 'manual' && (
        <div className="flex items-center justify-end h-5">
          {!editing && (
            <button
              ...
              title="Modifier"
              className="shrink-0 w-5 h-5 flex items-center justify-center rounded-md bg-ink-900 border border-ink-800 text-ink-300 hover:text-volt-400 hover:border-volt-500/40 transition-colors"
            >
