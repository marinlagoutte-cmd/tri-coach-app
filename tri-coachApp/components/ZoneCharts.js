...
export default function ZoneCharts({ profile, activities, onPaceZonesChange }) {
  ...
  const formatAutoDate = (iso) => new Intl.DateTimeFormat('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(iso));
  const autoExplanation = autoIsFallback || !autoMeta
    ...
    : metricTab === 'pace'
      ? `VMA estimée à ${autoMeta.value} km/h d'après ta meilleure sortie course (${autoMeta.distanceKm} km en ${autoMeta.durationMin} min, le ${formatAutoDate(autoMeta.date)}).`
      : `${metricTab === 'hr' ? 'FC de seuil' : 'FTP'} estimée à ${autoMeta.value} ${activeTab.unit} (95% de ta meilleure moyenne sur un effort continu) d'après ta meilleure sortie ${activeDiscipline.label.toLowerCase()} (${autoMeta.durationMin} min, ${
        ...
        } de moyenne, le ${formatAutoDate(autoMeta.date)}).`;
