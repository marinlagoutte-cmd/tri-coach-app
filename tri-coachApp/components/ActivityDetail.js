  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-end sm:items-center justify-center animate-sheetBackdrop" onClick={onClose}>
      ...
      >
        ...
        {/* Analyse IA — générée automatiquement à la réception de l'activité (voir
            pages/api/strava/webhook.js:coAnalyzeStravaActivity), jamais recalculée
            ici. `ai_seance_detectee` (demande explicite de l'athlète, analyse PAR
            INTERVALLES — voir lib/intervals.js) résume en une phrase ce que l'IA a
            compris de la structure réelle de la séance (ex: "3x10min au seuil,
            récup 3min") avant même l'analyse détaillée qui suit — mise en avant
            séparément pour que ce diagnostic saute aux yeux, plutôt que noyé au
            milieu du texte libre. `ai_respecte_plan` (booléen, null si aucune
            séance prévue associée) donne un badge conforme/à ajuster immédiat. */}
        <div className="bg-volt-500/5 border border-volt-500/20 p-3 rounded-xl space-y-2">
          <span className="text-[9px] text-volt-400 uppercase font-bold block">🤖 Analyse du coach</span>
          {activity.ai_analysis_status === 'ok' && activity.ai_seance_detectee && (
            <div className="flex items-start gap-1.5">
              <span
                className={`shrink-0 text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-md border ${
                  activity.ai_respecte_plan === true
                    ? 'text-emerald-300 border-emerald-500/30 bg-emerald-500/10'
                    : activity.ai_respecte_plan === false
                      ? 'text-amber-300 border-amber-500/30 bg-amber-500/10'
                      : 'text-ink-400 border-ink-700 bg-ink-900'
                }`}
              >
                {activity.ai_respecte_plan === true ? '✓ Conforme' : activity.ai_respecte_plan === false ? '⚠ À ajuster' : 'Séance'}
              </span>
              <p className="text-xs font-bold text-ink-50 leading-snug">{activity.ai_seance_detectee}</p>
            </div>
          )}
