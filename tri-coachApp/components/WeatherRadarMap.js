...
import { fetchWindForPoints, classifyWindImpact, nearestHourIndex, speedColor, impactColor, IMPACT_LABEL, windArrowDivIcon } from '../lib/windMap';
import { DARK_TILES, TILES_ATTRIBUTION, CARTO_KEY_CONFIGURED } from '../lib/mapTiles';
...
// windArrowDivIcon déplacée dans lib/windMap.js (partagée avec components/RoutePlannerMap.js).
...
export default function WeatherRadarMap() {
  ...
  return (
    <div className="space-y-3">
      {!CARTO_KEY_CONFIGURED && (
        <p className="text-[10px] text-amber-400 bg-amber-500/10 border border-amber-500/30 rounded-lg p-2 leading-relaxed">
          ⚠️ Fond de carte affiché avec un filigrane "API KEY REQUIRED" — CARTO exige
          désormais une clé gratuite pour ses tuiles. Voir CARTO_SETUP.md pour la créer
          (2 min) et l'ajouter en variable d'environnement <code>NEXT_PUBLIC_CARTO_API_KEY</code>.
          Les données vent/pluie elles-mêmes (Open-Meteo, RainViewer) ne sont pas concernées.
        </p>
      )}
