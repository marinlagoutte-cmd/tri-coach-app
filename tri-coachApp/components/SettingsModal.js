  ...
  const handleStravaSync = async () => {
    ...
    try {
      const res = await fetch('/api/strava/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // sinceEpochSec (minuit il y a 30 jours, DANS LE FUSEAU RÉEL de l'athlète) borne
        // l'import côté serveur au dernier mois glissant — voir pages/api/strava/sync.js.
        // Calculé ici plutôt que côté serveur (Vercel tourne en UTC).
        body: JSON.stringify({ accessToken: session.access_token, sinceEpochSec: getLocalDayStartEpochSec(30) }),
      });
      const data = await res.json();
