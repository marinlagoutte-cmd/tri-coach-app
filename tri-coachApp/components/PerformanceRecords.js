...
function fmtDateTime(d) {
  if (!d) return '';
  try { return new Date(d).toLocaleString('fr-FR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }); } catch { return ''; }
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <button onClick={refresh} disabled={loading} style={{ fontSize: 12.5, fontWeight: 500, color: C.volt, background: C.voltLight, border: 'none', borderRadius: 8, padding: '7px 12px', cursor: 'pointer', opacity: loading ? 0.6 : 1 }}>
          {loading ? 'Calcul…' : '↻ Recalculer maintenant'}
        </button>
        {/* Depuis pages/api/strava/auto-sync.js (cron horaire) : les activités se synchronisent
            désormais aussi en arrière-plan, ce bouton ne fait que relire/recalculer depuis les
            données déjà en base — il ne consomme donc pas de quota Strava supplémentaire tant
            que l'historique complet est déjà importé. */}
        {data?.autoSync?.fullHistorySyncedAt ? (
          <span style={{ fontSize: 11, color: C.textMuted }}>
            Synchro auto active — dernière mise à jour : {fmtDateTime(data.autoSync.lastSyncedAt) || 'à l’instant'}
          </span>
        ) : data?.autoSync ? (
          <span style={{ fontSize: 11, color: C.textMuted }}>
            Import de l'historique complet en cours en arrière-plan (par petits blocs, toutes les heures) — les records les plus anciens peuvent encore manquer temporairement.
          </span>
        ) : null}
      </div>
