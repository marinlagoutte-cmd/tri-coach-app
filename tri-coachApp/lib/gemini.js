...
import { buildPeriodizationPlan, describePhaseGuidance, formatMacrocyclesForPrompt, phasesToCycles, getProgressionFactor, interpolateTowardTarget } from './periodization';
...
// RELECTURE CROISÉE (demande explicite de l'athlète, 2026-08-28) : la relecture de
// cohérence d'un plan (reviewPlanCoherenceWithAI) doit être faite par l'AUTRE IA que
// celle qui a généré le plan — Gemini génère -> Groq relit, et inversement — plutôt
// que par le même modèle qui se relirait lui-même. Une IA qui relit sa propre
// production a un angle mort structurel (elle est encline à valider ses propres choix
// de structure/formulation) ; l'autre IA, qui n'a jamais vu ce plan, apporte un vrai
// second regard indépendant. Coût : double le nombre d'appels IA pour la relecture
// (latence/quota en plus), accepté sciemment pour la rigueur supplémentaire.
function oppositeProvider(provider) {
...
function buildEnduranceExpertRules({ fitnessLevel, trainingExperience, sportType, runningSubtype, phaseKey, hoursPerWeek, maxSessionsPerWeek } = {}) {
  const level = Number(fitnessLevel) || 3;
  const expRank = EXPERIENCE_RANK[trainingExperience] || 3;
...
function buildWorkoutSchema({ maxSessionsPerWeek, offDays, sportType, fitnessLevel, trainingExperience, runningSubtype, phaseKey, hoursPerWeek, triathlonFormat } = {}) {
  const restCount = maxSessionsPerWeek ? 7 - Number(maxSessionsPerWeek) : null;
  const expertRules = buildEnduranceExpertRules({ fitnessLevel, trainingExperience, sportType, runningSubtype, phaseKey, hoursPerWeek, maxSessionsPerWeek });
...
Réponds UNIQUEMENT avec ce JSON :
{
  ...
  try {
    ...
    return { N: sanitized.N, 'N+1': sanitized['N+1'], reviewIssues: [], reviewOk: false, reviewError: err?.message };
  }
}

export async function generatePlanWithAI({ wizardData, profile, feedbackHistory, healthHistory, manualPaceZones, language = 'fr', provider = 'gemini' }) {
...
{ "workouts": { "N": [...], "N+1": [...] } }`;
  ...
  const reviewed = await reviewPlanCoherenceWithAI({ sanitized, wizardData, resolvedProfile, phase, currentPhaseKey, isTriathlon, provider: oppositeProvider(provider) });
  ...
  // Transparence sur la relecture croisée elle-même (pas seulement sur ce qu'elle a
  // trouvé) : si l'autre IA n'a pas pu relire (panne/quota), le plan reste valide (il
  // a quand même passé tous les garde-fous déterministes) mais ce round n'a bénéficié
  // que d'un seul regard IA plutôt que deux — même logique de transparence que
  // soloNote côté co-génération (lib/coGeneration.js).
  if (!reviewed.reviewOk) {
    const reviewer = oppositeProvider(provider) === 'groq' ? 'Groq' : 'Gemini';
    autoFixNotes.push(`Relecture croisée par ${reviewer} indisponible cette fois (${reviewed.reviewError || 'erreur inconnue'}) — plan conservé tel quel, cohérence déjà garantie par les garde-fous déterministes.`);
  }
 ...
 */
export async function regenerateWeekWithAI({ weekKey = 'N+1', profile, workouts, trainingPlan, constraints, feedbackHistory, healthHistory, manualPaceZones, language = 'fr', provider = 'gemini' }) {
...
{ "workouts": { "${weekKey}": [...] } }`;
  ...
  const reviewed = await reviewPlanCoherenceWithAI({ sanitized: sanitizedForReview, wizardData, resolvedProfile, phase, currentPhaseKey, isTriathlon, provider: oppositeProvider(provider) });
...
// GARDE-FOU AJOUTÉ (suite au bug ci-dessus, lib/workouts.js:mergeWorkoutPatches) : même avec
// un seul niveau d'historique désormais (plus de chaîne récursive), chaque séance modifiée
// transporte encore un `previous` complet (copie intégrale de la séance) — pur bookkeeping
// pour l'affichage du diff AVANT/APRÈS côté client (pages/index.js), sans aucune utilité pour
// le prompt IA, qui ne raisonne que sur l'état ACTUEL des séances. Sur un plan avec plusieurs
// séances déjà modifiées au fil de la conversation, ce champ à lui seul pouvait doubler la
// taille de "Séances actuelles" envoyée à CHAQUE appel — l'un des facteurs qui faisait
// dépasser le budget tokens/minute du palier gratuit Groq (HTTP 413 "Request too large").
// Retire donc `previous`/`added` avant sérialisation, sans jamais toucher aux données
// originales (fonction pure, pas de mutation de `workoutsObj`).
function stripBookkeepingFields(workoutsObj) {
  const stripWeek = (week) => (Array.isArray(week) ? week.map(({ previous, added, ...rest }) => rest) : week);
  return {
    ...workoutsObj,
    N: stripWeek(workoutsObj?.N),
    'N+1': stripWeek(workoutsObj?.['N+1']),
  };
}

export async function chatWithCoach({ message, profile, workouts, trainingPlan, intent, sportType, constraints, feedbackHistory, healthHistory, manualPaceZones, language = 'fr', provider = 'gemini' }) {
...
${constraints?.maxSessionsPerWeek ? `⚠️ Ajouter une séance fait dépasser les ${constraints.maxSessionsPerWeek} séances/semaine déclarées au questionnaire — précise-le clairement à l'athlète dans ta réponse et propose plutôt d'alléger une séance existante si le volume ${constraints.hoursPerWeek ? `(${constraints.hoursPerWeek}h/sem déclarées) ` : ''}ne permet pas d'en absorber une de plus, sauf si l'athlète insiste explicitement.` : ''}`
  ...
  const modifyScopeRule = `RÈGLE ABSOLUE SUR LA PORTÉE D'UN PATCH "modify" : un patch qui corrige un point précis (allure, durée,
intensité, description, structure...) ne doit JAMAIS changer, en plus, le "day" ou le "type" (discipline)
de la séance visée — recopie-les à l'IDENTIQUE depuis la séance d'origine. N'ajoute "moveRequested": true
(seul cas où "day"/"type" peuvent différer de l'original) QUE si l'athlète demande explicitement de
déplacer/échanger/permuter/remplacer cette séance par une autre discipline ou un autre jour dans son
message ci-dessous — jamais en réponse à une simple remarque sur une allure, une durée ou un ressenti.`;
...
Séances actuelles : ${JSON.stringify(stripBookkeepingFields(workouts))}
...
Réponds UNIQUEMENT avec :
{
  ...
  "patches": [ /* chaque patch inclut patchMode: "add"|"modify", et "moveRequested": true UNIQUEMENT si le "day"/"type" doivent changer (voir règle ci-dessus) */ ]
}`;
...
// lib/coGeneration.js — double-check Gemini + Groq, demande explicite de
// l'athlète). `plannedWorkout` est null si aucune séance du plan n'a pu être
// associée automatiquement (voir lib/stravaMatch.js) : dans ce cas l'IA commente
// uniquement la séance réalisée, sans comparaison.
//
// PAR INTERVALLES (27/08/2026, demande explicite de l'athlète) : `intervalBlock`
// (voir lib/intervals.js:buildIntervalPromptBlock, calculé déterministiquement
// depuis les streams seconde-par-seconde AVANT cet appel) donne à l'IA le VRAI
// découpage de la séance en blocs homogènes (allure/watts/FC/cadence par bloc,
// patron répété "N x Ymin en ZX" si détecté) — sans ça, une séance fractionnée
// (ex: 3x10min au seuil avec récup) n'était vue qu'à travers sa MOYENNE globale
// sur toute la durée, qui mélange effort et récupération et rend impossible de
// dire ce qui a réellement été fait. `intervalBlock` est une chaîne vide si les
// streams n'étaient pas disponibles (activité importée en masse, capteur absent,
// séance trop courte pour être segmentée) : l'IA retombe alors sur les moyennes
// globales seules, comme avant cette fonctionnalité.
//
// Réponse structurée en JSON (plutôt que la prose libre d'avant) pour permettre à
// coAnalyzeStravaActivity (lib/coGeneration.js) de comparer Gemini et Groq de façon
// déterministe sur le verdict `respectePlan`, comme le reste du protocole de
// co-génération — la prose elle-même (`analyse`) ne se compare pas telle quelle,
// mais le verdict structuré, si.
export async function analyzeStravaActivity({ activity, plannedWorkout, profile, language = 'fr', intervalBlock = '', provider = 'gemini' }) {
  const langNote = language !== 'fr' ? `Réponds dans le champ "analyse" entièrement en ${AI_LANGUAGE_NAMES[language] || 'français'}.\n` : '';
...
${intervalBlock ? `\n${intervalBlock}` : "\n(Détail par intervalles non disponible pour cette activité — base-toi uniquement sur les moyennes ci-dessus.)"}`;
...
- Zone d'effort cible : ${plannedWorkout.effortZone || '-'}
...
${intervalBlock ? "Utilise le découpage par intervalles ci-dessus pour identifier CONCRÈTEMENT ce que l'athlète a fait (ex: \"tu as fait un 3x10min au seuil avec 3min de récup\") avant de comparer au plan — ne te contente jamais de commenter uniquement la moyenne globale quand un découpage par blocs est disponible." : ''}

Réponds STRICTEMENT en JSON, sans aucun texte autour :
{
  "seanceDetectee": "une phrase courte et concrète décrivant ce qui a été RÉELLEMENT fait (structure/blocs si connus, ex: '3x10min au seuil, récup 3min' ou 'sortie continue en endurance')",
  "respectePlan": ${plannedWorkout ? 'true|false' : 'null'},
  "analyse": "analyse COURTE (5-8 lignes MAXIMUM), ton motivant mais honnête : ${plannedWorkout ? "1) la séance réalisée correspond-elle à ce qui était prévu (allure/puissance/FC/durée, bloc par bloc si un découpage est disponible) ? sois précis et chiffré, sans inventer de valeur absente ; 2) un point positif concret ; 3) un point de vigilance ou conseil pour la suite, seulement s'il y a une vraie raison de le mentionner." : "commente la séance réalisée (effort, régularité perçue via FC/puissance/allure par bloc si dispo) et donne un conseil de récupération ou d'enchaînement adapté."} Jamais de markdown (pas de **gras**, pas de listes à puces) dans ce champ : du texte simple, en phrases."
}
Si "respectePlan" est demandé (pas null), base ce booléen sur un écart réel et significatif (pas une nuance de quelques secondes/watts) entre la séance prévue et la séance réalisée.`;

  try {
    const json = await callAIJSON(prompt, provider);
    const seanceDetectee = typeof json?.seanceDetectee === 'string' ? json.seanceDetectee : null;
    const analyse = typeof json?.analyse === 'string' ? json.analyse : null;
    if (!analyse) throw Object.assign(new Error('Réponse IA sans champ "analyse" exploitable'), { code: 'PARSE_ERROR' });
    return {
      analysis: analyse,
      seanceDetectee,
      respectePlan: plannedWorkout ? Boolean(json?.respectePlan) : null,
      status: 'ok',
    };
  } catch (err) {
    console.error('[gemini] analyzeStravaActivity error:', { provider, code: err?.code, message: err?.message });
    return { analysis: null, seanceDetectee: null, respectePlan: null, status: 'error', errorCode: err?.code || 'UNKNOWN' };
  }
}
