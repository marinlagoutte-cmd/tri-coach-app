...
import { defaultPaceZones, zoneForValue } from './zones';
 ...
 */
// BUG RÉEL CORRIGÉ (retour utilisateur : allure toujours identique avant/après malgré une
// correction demandée et confirmée par le coach) : cette fonction exigeait `min > 0` pour
// les 5 zones, y compris Z1 (récupération) — or Z1 a par construction une borne basse à 0
// km/h dès qu'elle n'a jamais été éditée manuellement (voir defaultPaceZones dans
// lib/zones.js, PACE_PCTS[0] = 0), ce qui est un cas parfaitement légitime, pas une donnée
// manquante. Résultat : dès que Z1 valait 0, TOUTE la calibration manuelle de l'athlète
// (y compris ses zones Z2-Z5 correctement éditées) était rejetée par cette fonction, et
// fallbackEfSpeedKmh/paceSanityBoundsKmh retombaient silencieusement sur le calcul
// théorique générique (VMA × 75%) — écrasant une correction pourtant valide proposée par
// l'IA lors de sanitizeWorkout, d'où un "APRÈS" identique à l'"AVANT". Même bug potentiel
// pour hrZones (Z1 FC peut aussi légitimement démarrer à 0 selon le référentiel choisi) —
// mais seul paceZones est concerné ici. `>= 0` (au lieu de `> 0`) aligne cette fonction sur
// isPlausiblePaceZones (lib/zones.js) et sur le check `hasManual` de computeRunZones
// (lib/gemini.js), qui acceptaient déjà correctement un Z1 à 0.
...
/**
 * GARDE-FOU AJOUTÉ (retour utilisateur : "4'13 ce n'est pas mon allure en Z2, regarde ce
 * que je t'ai renseigné") : `paceSanityBoundsKmh` ci-dessus ne détecte qu'une allure
 * GROSSIÈREMENT aberrante (hors de l'étendue totale Z1→Z5), pas une allure qui tombe dans
 * une AUTRE zone que celle annoncée par la séance elle-même (ex: une allure de Z3 affichée
 * sur une séance labellisée "Z2 (endurance fondamentale)") — un patch de chat pouvait donc
 * répondre "j'ai recalé sur ta Z2" tout en laissant une valeur de Z3/Z4, sans qu'aucun
 * garde-fou ne s'en aperçoive. Vérifie chaque séance course à pied d'une liste (uniquement
 * celles dont `effortZone` annonce clairement UNE zone, ex: "Z2" ou "Z2 (endurance
 * fondamentale)" — on ignore les plages du type "Z2-Z3", trop ambiguës pour ce contrôle)
 * contre les zones calibrées de l'athlète (manuelles en priorité, sinon dérivées de la VMA,
 * mêmes règles de priorité que fallbackEfSpeedKmh ci-dessus). Renvoie la liste des séances
 * en désaccord, jamais une correction silencieuse — c'est à l'athlète/au coach de trancher.
 */
export function findPaceZoneMismatches(weekWorkouts, profile) {
  const mismatches = [];
  const zones = hasValidPaceZones(profile?.paceZones)
    ? [...profile.paceZones].sort((a, b) => Number(a.min) - Number(b.min))
    : (profile?.vma ? defaultPaceZones(profile.vma) : null);
  if (!zones) return mismatches;

  (weekWorkouts || []).forEach((w) => {
    const type = String(w?.type || '').toUpperCase();
    if (!type.includes('C.A.P') && !type.includes('RUN')) return;
    const zoneMatch = String(w?.effortZone || '').match(/^Z(\d)\s*(?:\(|$)/i);
    if (!zoneMatch) return; // plage ("Z2-Z3") ou zone non précisée : pas de contrôle possible
    const announcedZoneIdx = Number(zoneMatch[1]) - 1;
    const paceMatch = String(w?.intensity || '').match(/(\d+):(\d{2})\s*\/?\s*(?:min\/)?km/i);
    if (!paceMatch) return;
    const speedKmh = 60 / (Number(paceMatch[1]) + Number(paceMatch[2]) / 60);
    const actualZone = zoneForValue(zones, speedKmh);
    const actualZoneIdx = zones.findIndex((z) => z === actualZone);
    if (actualZoneIdx !== -1 && actualZoneIdx !== announcedZoneIdx) {
      mismatches.push({
        day: w.day,
        title: w.title,
        announcedZone: `Z${announcedZoneIdx + 1}`,
        actualZone: `Z${actualZoneIdx + 1}`,
...
export function mergeWorkoutPatches(workoutsObj, patches, profile) {
  ...
  patches.forEach(patch => {
    ...
    if (index !== -1) {
      ...
      // BUG RÉEL CORRIGÉ (payload qui grossit à chaque échange de chat, jusqu'à dépasser le
      // quota Groq — "Request too large... Requested 11454... Limit 8000") : si la séance
      // avait déjà été modifiée une fois, `copy[weekKey][index]` contenait déjà un champ
      // `previous` (l'état d'AVANT sa PRÉCÉDENTE modification). En le recopiant tel quel ici,
      // chaque nouvelle modification empilait un `previous.previous.previous...` de plus en
      // plus profond — jamais affiché à l'athlète (seul le dernier niveau sert au diff), mais
      // toujours réexpédié tel quel dans `workouts` à CHAQUE appel IA suivant (voir
      // lib/gemini.js:chatWithCoach, "Séances actuelles : JSON.stringify(workouts)"), faisant
      // gonfler le prompt à chaque échange jusqu'à dépasser le budget de tokens/minute du
      // palier gratuit Groq. On ne garde donc qu'UN seul niveau d'historique (l'état juste
      // avant CETTE modification), jamais de chaîne récursive.
      delete previous.previous;
      // BUG RÉEL CORRIGÉ (retour utilisateur : "AVANT: Lundi SWIM" / "APRÈS: Vendredi RUN"
      // pour une simple demande de correction d'allure) : un patch "modify" ciblant une
      // séance existante (matché par id) pouvait silencieusement changer le JOUR et/ou la
      // DISCIPLINE (type) de cette séance — l'IA a parfois "corrigé" une allure en
      // déplaçant/convertissant la séance entière au lieu de ne toucher QUE le champ
      // concerné, produisant un diff avant/après incohérent (jour et discipline différents
      // des deux côtés) sans que l'athlète l'ait demandé. On n'autorise donc désormais un
      // changement de jour ou de discipline QUE si le patch porte explicitement
      // `moveRequested: true` (déclenché uniquement quand l'athlète demande clairement de
      // déplacer/échanger/remplacer une séance par une autre — voir lib/gemini.js
      // chatWithCoach) ; sinon on conserve le jour et la discipline d'origine, tout en
      // appliquant normalement les autres champs corrigés (allure, durée, desc...).
      const allowMove = patch.moveRequested === true;
      const day = allowMove ? normalizeDayName(patch.day, previous.day) : previous.day;
      const type = allowMove ? (patch.type || previous.type) : previous.type;
