...
/**
 * Volume RÉELLEMENT effectué (en heures) par discipline pour la semaine calendaire en
 * cours (Lundi→Dimanche), à partir des activités Strava synchronisées. À comparer au
 * volume PRÉVU (computeWeeklyDurationByDiscipline) pour la semaine en cours.
 * Contrairement au prévu, ceci n'existe que pour la semaine en cours (et le passé) :
 * il n'y a par construction aucune activité réalisée pour la semaine N+1, à venir —
 * cette fonction ne calcule donc jamais rien pour N+1, conformément à la règle en
 * tête de fichier (pas de donnée inventée).
 */
export function computeActualWeeklyDurationByDiscipline(activities) {
  const disciplines = ['NATATION', 'CYCLISME', 'C.A.P'];
  const totalsHours = Object.fromEntries(disciplines.map((d) => [d, 0]));
  const currentMonday = mondayOfWeek(new Date()).getTime();

  (activities || []).forEach((act) => {
    const dateStr = act.start_date_local || act.start_date;
    if (!dateStr) return;
    if (mondayOfWeek(new Date(dateStr)).getTime() !== currentMonday) return;
    const disc = stravaSportToDiscipline(act.sport_type || act.type);
    if (!disc || totalsHours[disc] === undefined) return;
    totalsHours[disc] += (Number(act.moving_time_s) || 0) / 3600;
  });

  const hasAnyActivityThisWeek = (activities || []).some((act) => {
    const dateStr = act.start_date_local || act.start_date;
    return dateStr && mondayOfWeek(new Date(dateStr)).getTime() === currentMonday;
  });

  return {
    label: 'Semaine en cours',
    disciplines: disciplines.map((d) => ({ key: d, label: d === 'C.A.P' ? 'Course' : d === 'CYCLISME' ? 'Vélo' : 'Natation', color: DISCIPLINE_COLOR[d] })),
    series: Object.fromEntries(disciplines.map((d) => [d, Math.round(totalsHours[d] * 10) / 10])),
    hasAnyActivityThisWeek,
  };
}
...
/**
 * Lundi 00:00 (heure locale) de la semaine calendaire réelle contenant `date` — même
 * convention que isInCurrentCalendarWeek dans lib/stravaMatch.js (semaine Lundi→Dimanche,
 * cohérente avec DAYS_OF_WEEK qui commence à Lundi partout ailleurs dans l'app).
 */
function mondayOfWeek(date) {
  const d = new Date(date);
  d.setHours(0, 0, 0, 0);
  const mondayOffset = (d.getDay() + 6) % 7; // jours écoulés depuis lundi (0=lundi...6=dimanche)
  d.setDate(d.getDate() - mondayOffset);
  return d;
}

/**
 * Classification par sport façon Strava (Training Log) — plus fine que
 * stravaSportToDiscipline ci-dessus (qui regroupe tout le vélo/course en 2 catégories) :
 * ici chaque type Strava garde sa propre couleur/étiquette pour la légende et le filtre
 * du graphe à bulles (voir components/TrainingLogChart.js), exactement comme la légende
 * Run/Ride/Swim/Weight Training/Walk du Training Log Strava d'origine.
 */
export function stravaSportGroup(sportType) {
  const s = String(sportType || '').toLowerCase();
  if (s.includes('swim')) return { key: 'Swim', label: 'Natation', color: '#38BDF8' };
  if (s.includes('ride') || s.includes('cycl') || s.includes('handcycle') || s.includes('velomobile')) {
    return { key: 'Ride', label: 'Vélo', color: '#FC4C02' };
  }
  if (s.includes('trailrun')) return { key: 'TrailRun', label: 'Trail', color: '#15803D' };
  if (s.includes('run')) return { key: 'Run', label: 'Course', color: '#4ADE80' };
  if (s.includes('walk')) return { key: 'Walk', label: 'Marche', color: '#A3E635' };
  if (s.includes('hike')) return { key: 'Hike', label: 'Rando', color: '#65A30D' };
  if (s.includes('weighttraining') || s.includes('strengthtraining') || s.includes('crossfit') || s.includes('workout')) {
    return { key: 'WeightTraining', label: 'Renfo', color: '#92400E' };
  }
  if (s.includes('yoga')) return { key: 'Yoga', label: 'Yoga', color: '#A855F7' };
  if (s.includes('rowing') || s.includes('canoe') || s.includes('kayak')) return { key: 'Rowing', label: 'Aviron', color: '#0EA5E9' };
  if (s.includes('ski') || s.includes('snowboard')) return { key: 'Ski', label: 'Ski', color: '#7DD3FC' };
  return { key: 'Other', label: sportType || 'Autre', color: '#9CA3AF' };
}

/**
 * Regroupe les activités Strava par semaine calendaire (Lundi→Dimanche, semaines les plus
 * RÉCENTES en premier — même ordre que le Training Log Strava) puis par jour, pour le
 * graphe à bulles de components/TrainingLogChart.js (calqué sur le Training Log Strava :
 * une bulle par activité, taille = distance/temps/intensité, couleur = sport).
 *
 * `intensityPoints` est une estimation MAISON (pas le "Relative Effort"/suffer_score
 * propriétaire de Strava, jamais synchronisé par cette app — voir lib/strava.js:toActivityRow)
 * : durée (min) × (FC moyenne / FC max du profil), un proxy TRIMP simplifié. `null` si la
 * FC moyenne de l'activité ou le FC max du profil sont absents — jamais une valeur inventée.
 */
export function computeTrainingLogWeeks(activities, { weeksCount = 8, weeksOffset = 0, profile } = {}) {
  const currentMonday = mondayOfWeek(new Date());
  const WEEK_MS = 7 * 24 * 60 * 60 * 1000;
  // Semaines les plus récentes en premier, décalées de `weeksOffset` semaines vers le passé
  // (pagination "voir plus ancien" côté composant).
  const weekStarts = Array.from({ length: weeksCount }, (_, i) => (
    new Date(currentMonday.getTime() - (weeksOffset + i) * WEEK_MS)
  ));

  const byMonday = new Map(weekStarts.map((ws) => [ws.getTime(), ws]));
  const buckets = new Map(weekStarts.map((ws) => [ws.getTime(), Array.from({ length: 7 }, () => [])]));
  ...
  (activities || []).forEach((act) => {
    ...
    if (!byMonday.has(mondayTime)) return; // hors de la fenêtre affichée
    const dayIdx = (date.getDay() + 6) % 7; // 0=lundi...6=dimanche

    const sportGroup = stravaSportGroup(act.sport_type || act.type);
    const distanceKm = Number(act.distance_m) > 0 ? Number(act.distance_m) / 1000 : 0;
    const durationMin = Number(act.moving_time_s) > 0 ? Number(act.moving_time_s) / 60 : 0;
    const fcMax = Number(profile?.fcMax) > 0 ? Number(profile.fcMax) : null;
    const avgHr = Number(act.average_heartrate) > 0 ? Number(act.average_heartrate) : null;
    const intensityPoints = fcMax && avgHr ? Math.round(durationMin * (avgHr / fcMax) * 10) / 10 : null;

    buckets.get(mondayTime)[dayIdx].push({
      id: act.id,
      name: act.name || sportGroup.label,
      sportGroup,
  ...
  const weeks = weekStarts.map((ws) => {
    const days = buckets.get(ws.getTime());
    const end = new Date(ws.getTime() + 6 * 24 * 60 * 60 * 1000);
    const fmt = (d) => new Intl.DateTimeFormat('fr-FR', { day: '2-digit', month: 'short' }).format(d);
    const allActs = days.flat();
    return {
      key: ws.toISOString().slice(0, 10),
      label: `${fmt(ws)} – ${fmt(end)}`,
      isCurrent: ws.getTime() === currentMonday.getTime(),
      days,
      totalDistanceKm: Math.round(allActs.reduce((s, a) => s + a.distanceKm, 0) * 10) / 10,
      totalDurationH: Math.round((allActs.reduce((s, a) => s + a.durationMin, 0) / 60) * 10) / 10,
      // Somme des points d'intensité de la semaine — mêmes limites que par activité
      // (voir intensityPoints ci-dessus) : ignore les activités sans FC moyenne plutôt
      // que de leur compter 0 (ce qui sous-estimerait la charge réelle de la semaine).
      totalIntensityPoints: (() => {
        const withValue = allActs.filter((a) => Number.isFinite(a.intensityPoints));
        if (!withValue.length) return null;
        return Math.round(withValue.reduce((s, a) => s + a.intensityPoints, 0) * 10) / 10;
      })(),
      hasActivity: allActs.length > 0,
    };
  });

  const sportsPresent = new Map();
  weeks.forEach((w) => w.days.flat().forEach((a) => sportsPresent.set(a.sportGroup.key, a.sportGroup)));

  return { weeks, sportsPresent: Array.from(sportsPresent.values()) };
}

const SPORT_FILTERS = {
  ALL: () => true,
  'C.A.P': (act) => stravaSportToDiscipline(act.sport_type || act.type) === 'C.A.P',
  CYCLISME: (act) => stravaSportToDiscipline(act.sport_type || act.type) === 'CYCLISME',
  NATATION: (act) => stravaSportToDiscipline(act.sport_type || act.type) === 'NATATION',
  // "Autre" = tout ce que stravaSportToDiscipline ne reconnaît pas (musculation, ski, yoga...).
  AUTRE: (act) => stravaSportToDiscipline(act.sport_type || act.type) === null,
};

/**
 * Volume hebdomadaire (distance en km OU durée en heures, selon `metric`) sur les `weeks`
 * dernières semaines calendaires (Lundi→Dimanche, la semaine en cours étant la dernière de la
 * série), filtré par discipline — pour le widget "progression" façon Strava (SettingsModal
 * n'a rien à voir ici : ce widget vit dans l'onglet Objectif, à côté de TrainingLoadChart).
 * Semaines sans aucune activité correspondante = 0 (jamais interpolé/deviné).
 */
export function computeWeeklySportSeries(activities, { weeks = 12, sportFilter = 'ALL', metric = 'distance' } = {}) {
  const matchesFilter = SPORT_FILTERS[sportFilter] || SPORT_FILTERS.ALL;
  const filtered = (activities || []).filter(matchesFilter);

  const currentMonday = mondayOfWeek(new Date());
  const WEEK_MS = 7 * 24 * 60 * 60 * 1000;
  // Bornes des `weeks` dernières semaines, la plus ancienne en premier.
  const weekStarts = Array.from({ length: weeks }, (_, i) => new Date(currentMonday.getTime() - (weeks - 1 - i) * WEEK_MS));

  const totals = weekStarts.map(() => ({ distanceM: 0, durationS: 0, elevationM: 0 }));

  filtered.forEach((act) => {
    const dateStr = act.start_date_local || act.start_date;
    if (!dateStr) return;
    const actMonday = mondayOfWeek(new Date(dateStr)).getTime();
    const idx = weekStarts.findIndex((ws) => ws.getTime() === actMonday);
    if (idx === -1) return; // hors de la fenêtre des `weeks` dernières semaines
    totals[idx].distanceM += Number(act.distance_m) || 0;
    totals[idx].durationS += Number(act.moving_time_s) || 0;
    totals[idx].elevationM += Number(act.total_elevation_m) || 0;
  });

  const values = totals.map((t) => {
    const raw = metric === 'time' ? t.durationS / 3600 : t.distanceM / 1000;
  ...
  const currentWeek = {
    distanceKm: Math.round((current.distanceM / 1000) * 10) / 10,
    hours: Math.round((current.durationS / 3600) * 10) / 10,
    elevationM: Math.round(current.elevationM),
    activityCount: filtered.filter((act) => {
      const dateStr = act.start_date_local || act.start_date;
      return dateStr && mondayOfWeek(new Date(dateStr)).getTime() === currentMonday.getTime();
    }).length,
  };

  return { labels, values, currentWeek, hasAnyActivity: filtered.length > 0 };
}
