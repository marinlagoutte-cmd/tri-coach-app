...
const DICT = {
  ...
  tools: {
    fr: { tirePressureSubTab: '🚴 Pression pneus', equipmentSubTab: '🔧 Matériel', routeSubTab: '🗺️ Parcours', recordsSubTab: '🏆 Records' },
    en: { tirePressureSubTab: '🚴 Tire pressure', equipmentSubTab: '🔧 Gear', routeSubTab: '🗺️ Route', recordsSubTab: '🏆 Records' },
    es: { tirePressureSubTab: '🚴 Presión de neumáticos', equipmentSubTab: '🔧 Material', routeSubTab: '🗺️ Ruta', recordsSubTab: '🏆 Récords' },
  },
  auth: {
  ...
  notifications: {
    fr: {
      bellTitle: 'Notifications', panelTitle: '🔔 Récap de la semaine',
      description: 'Reçois un résumé de tes séances de la semaine, en notification, chaque dimanche à 19h.',
      enable: 'Activer', disable: 'Désactiver le récap', enabling: 'Activation…',
      statusOn: 'Récap hebdo activé sur cet appareil', statusOff: 'Récap hebdo désactivé sur cet appareil',
      errorUnsupported: "Les notifications ne sont pas prises en charge par ce navigateur.",
      errorDenied: "Autorisation refusée — active les notifications pour Tri Coach dans les réglages de ton téléphone/navigateur.",
      errorNotConfigured: "Fonctionnalité en cours de configuration côté serveur, réessaie plus tard.",
      errorGeneric: "Une erreur est survenue, réessaie.",
      requiresAccount: "Connecte-toi pour activer le récap hebdomadaire.",
    },
    en: {
      bellTitle: 'Notifications', panelTitle: '🔔 Weekly recap',
      description: 'Get a summary of your sessions for the week, as a notification, every Sunday at 7pm (Paris time).',
      enable: 'Enable', disable: 'Disable recap', enabling: 'Enabling…',
      statusOn: 'Weekly recap enabled on this device', statusOff: 'Weekly recap disabled on this device',
      errorUnsupported: 'Notifications are not supported by this browser.',
      errorDenied: 'Permission denied — enable notifications for Tri Coach in your phone/browser settings.',
      errorNotConfigured: 'This feature is still being set up on the server, try again later.',
      errorGeneric: 'Something went wrong, try again.',
      requiresAccount: 'Sign in to enable the weekly recap.',
    },
    es: {
      bellTitle: 'Notificaciones', panelTitle: '🔔 Resumen semanal',
      description: 'Recibe un resumen de tus sesiones de la semana, como notificación, cada domingo a las 19h (hora de París).',
      enable: 'Activar', disable: 'Desactivar resumen', enabling: 'Activando…',
      statusOn: 'Resumen semanal activado en este dispositivo', statusOff: 'Resumen semanal desactivado en este dispositivo',
      errorUnsupported: 'Las notificaciones no son compatibles con este navegador.',
      errorDenied: 'Permiso denegado — activa las notificaciones para Tri Coach en los ajustes de tu teléfono/navegador.',
      errorNotConfigured: 'Función aún en configuración del lado del servidor, inténtalo más tarde.',
      errorGeneric: 'Ha ocurrido un error, inténtalo de nuevo.',
      requiresAccount: 'Inicia sesión para activar el resumen semanal.',
    },
  },
  settings: {
    fr: {
      ...
      zonesModeTitle: "Zones d'entraînement", zonesModeAuto: 'Automatique (Strava)', zonesModeManual: 'Manuel',
      zonesModeHint: "En automatique, les zones FC/Puissance/Allure (onglet Profil) sont recalculées depuis tes séances Strava réellement synchronisées (protocole \"test de 20 minutes\") au lieu d'être éditées à la main. Basculer sur Manuel ne perd jamais tes bornes déjà saisies : elles réapparaissent telles quelles.",
      ...
      stravaMissingScopeHint: "Le kilométrage de ton matériel (Outils > Matériel) reste bloqué à 0 : ton compte a été lié avant l'ajout de l'autorisation nécessaire pour lire vélos/chaussures. Reconnecte Strava pour la donner (aucune activité perdue).",
      stravaReconnect: 'Reconnecter Strava',
      stravaSync: 'Importer mes activités récentes', stravaSyncing: 'Import en cours…',
      stravaSyncHint: "Récupère tes activités Strava des 12 dernières semaines pour compléter le calendrier et les graphes de l'onglet Profil (dont la courbe de progression). Le webhook s'occupe déjà des nouvelles activités automatiquement — ce bouton sert surtout à rattraper l'historique récent.",
      ...
      ) + (limited ? ' Beaucoup d\'activités récentes : import limité aux 7 derniers jours.' : ''),
      stravaSyncError: "L'import a échoué. Réessaie dans un instant.",
      aiDiagnosticsButton: 'IA',
      aiDiagnosticsTitle: 'Diagnostic IA',
      aiDiagnosticsSubtitle: "Teste chaque modèle IA (Gemini + Groq) individuellement pour voir lequel répond et repérer d'éventuels bugs — ces détails techniques ne sont volontairement affichés nulle part ailleurs dans l'app.",
      aiDiagnosticsRun: 'Lancer le test', aiDiagnosticsRunning: 'Test en cours…',
      aiDiagnosticsOk: 'OK', aiDiagnosticsFail: 'Échec',
      aiDiagnosticsLatency: 'Latence', aiDiagnosticsSample: 'Réponse', aiDiagnosticsError: 'Erreur',
      aiDiagnosticsEmpty: 'Lance le test pour voir l\'état de chaque modèle.',
      aiDiagnosticsFetchError: "Le diagnostic n'a pas pu s'exécuter. Réessaie dans un instant.",
      aiDiagnosticsLastRun: 'Dernier test',
    },
    en: {
      ...
      zonesModeTitle: 'Training zones', zonesModeAuto: 'Automatic (Strava)', zonesModeManual: 'Manual',
      zonesModeHint: 'In automatic mode, the HR/Power/Pace zones (Profile tab) are recalculated from your actually synced Strava sessions ("20-minute test" protocol) instead of being edited by hand. Switching back to Manual never loses bounds you already entered — they reappear as-is.',
      ...
      stravaMissingScopeHint: "Your gear mileage (Tools > Gear) stays stuck at 0: your account was linked before the permission needed to read bikes/shoes was added. Reconnect Strava to grant it (no activities are lost).",
      stravaReconnect: 'Reconnect Strava',
      stravaSync: 'Import my recent activities', stravaSyncing: 'Importing…',
      stravaSyncHint: "Fetches your Strava activities from the last 12 weeks to fill in your calendar and the Profile tab's charts (including the progression graph). New activities are already picked up automatically by the webhook — this button is mainly to catch up on recent history.",
      stravaSyncSuccess: ({ imported = 0, skipped = 0, limited = false } = {}) => (imported > 0
        ? `${imported} activit${imported > 1 ? 'ies' : 'y'} imported${skipped > 0 ? ` (${skipped} already known)` : ''}.`
        : 'Already up to date, nothing new to import.'
      ) + (limited ? ' Lots of recent activity: import limited to the last 7 days.' : ''),
      stravaSyncError: 'Import failed. Try again in a moment.',
      aiDiagnosticsButton: 'AI',
      aiDiagnosticsTitle: 'AI Diagnostics',
      aiDiagnosticsSubtitle: 'Test each AI model (Gemini + Groq) individually to see which one responds and spot any bugs — these technical details are deliberately not shown anywhere else in the app.',
      aiDiagnosticsRun: 'Run test', aiDiagnosticsRunning: 'Testing…',
      aiDiagnosticsOk: 'OK', aiDiagnosticsFail: 'Failed',
      aiDiagnosticsLatency: 'Latency', aiDiagnosticsSample: 'Response', aiDiagnosticsError: 'Error',
      aiDiagnosticsEmpty: 'Run the test to see the status of each model.',
      aiDiagnosticsFetchError: 'The diagnostics could not run. Try again in a moment.',
      aiDiagnosticsLastRun: 'Last run',
    },
    es: {
      ...
      zonesModeTitle: 'Zonas de entrenamiento', zonesModeAuto: 'Automático (Strava)', zonesModeManual: 'Manual',
      zonesModeHint: 'En automático, las zonas de FC/Potencia/Ritmo (pestaña Perfil) se recalculan a partir de tus sesiones de Strava realmente sincronizadas (protocolo "test de 20 minutos") en lugar de editarse a mano. Volver a Manual nunca pierde los límites que ya introdujiste: reaparecen tal cual.',
      ...
      stravaMissingScopeHint: 'El kilometraje de tu material (Herramientas > Material) sigue bloqueado en 0: tu cuenta se vinculó antes de añadir el permiso necesario para leer bicis/zapatillas. Reconecta Strava para concederlo (no se pierde ninguna actividad).',
      stravaReconnect: 'Reconectar Strava',
      stravaSync: 'Importar mis actividades recientes', stravaSyncing: 'Importando…',
      stravaSyncHint: 'Recupera tus actividades de Strava de las últimas 12 semanas para completar el calendario y los gráficos de la pestaña Perfil (incluida la curva de progresión). El webhook ya se encarga de las nuevas actividades automáticamente — este botón sirve sobre todo para recuperar el historial reciente.',
      stravaSyncSuccess: ({ imported = 0, skipped = 0, limited = false } = {}) => (imported > 0
      ...
      ) + (limited ? ' Muchas actividades recientes: importación limitada a los últimos 7 días.' : ''),
      stravaSyncError: 'La importación falló. Inténtalo de nuevo en un momento.',
      aiDiagnosticsButton: 'IA',
      aiDiagnosticsTitle: 'Diagnóstico IA',
      aiDiagnosticsSubtitle: 'Prueba cada modelo de IA (Gemini + Groq) individualmente para ver cuál responde y detectar posibles errores — estos detalles técnicos no se muestran deliberadamente en ningún otro lugar de la app.',
      aiDiagnosticsRun: 'Ejecutar test', aiDiagnosticsRunning: 'Probando…',
      aiDiagnosticsOk: 'OK', aiDiagnosticsFail: 'Fallo',
      aiDiagnosticsLatency: 'Latencia', aiDiagnosticsSample: 'Respuesta', aiDiagnosticsError: 'Error',
      aiDiagnosticsEmpty: 'Ejecuta el test para ver el estado de cada modelo.',
      aiDiagnosticsFetchError: 'El diagnóstico no se pudo ejecutar. Inténtalo de nuevo en un momento.',
      aiDiagnosticsLastRun: 'Última ejecución',
    },
  },
  wizard: {
    fr: {
  ...
  chat: {
    fr: { addIntent: "➕ Ajout d'une séance supplémentaire", modifyIntent: '✏️ Modification de séance', placeholder: 'Ex: Décale ma séance de vélo à jeudi...', thinking: 'Coach analyse et recalcule tes cibles...', welcome: (n) => `👋 Salut${n ? ' ' + n : ''} ! Ton plan d'entraînement est opérationnel. Quelle séance souhaites-tu passer en revue ?`, feedbackBtn: '📝 Feedback séance', feedbackPickTitle: 'Sur quelle séance donner ton ressenti ?', feedbackNone: 'Aucune séance en attente de ressenti pour le moment.', feedbackClose: 'Annuler', showDetails: 'Voir le détail', hideDetails: 'Masquer', suggestExplain: 'Pourquoi ?', suggestLighten: 'Alléger', suggestMove: 'Décaler', suggestProgress: 'Où en est mon plan ?' },
    en: { addIntent: '➕ Add an extra session', modifyIntent: '✏️ Modify a session', placeholder: 'E.g. Move my bike session to Thursday...', thinking: 'Coach is analyzing and recalculating your targets...', welcome: (n) => `👋 Hi${n ? ' ' + n : ''}! Your training plan is ready. Which session would you like to review?`, feedbackBtn: '📝 Session feedback', feedbackPickTitle: 'Which session do you want to give feedback on?', feedbackNone: 'No session awaiting feedback right now.', feedbackClose: 'Cancel', showDetails: 'Show details', hideDetails: 'Hide', suggestExplain: 'Why?', suggestLighten: 'Lighten', suggestMove: 'Move', suggestProgress: "How's my plan going?" },
    es: { addIntent: '➕ Añadir una sesión extra', modifyIntent: '✏️ Modificar una sesión', placeholder: 'Ej: Cambia mi sesión de bici al jueves...', thinking: 'El coach está analizando y recalculando tus objetivos...', welcome: (n) => `👋 Hola${n ? ' ' + n : ''}! Tu plan de entrenamiento está listo. ¿Qué sesión quieres revisar?`, feedbackBtn: '📝 Feedback de sesión', feedbackPickTitle: '¿Sobre qué sesión quieres dar tu opinión?', feedbackNone: 'Ninguna sesión pendiente de feedback por ahora.', feedbackClose: 'Cancelar', showDetails: 'Ver detalle', hideDetails: 'Ocultar', suggestExplain: '¿Por qué?', suggestLighten: 'Aligerar', suggestMove: 'Mover', suggestProgress: '¿Cómo va mi plan?' },
  },
  outlook: {
