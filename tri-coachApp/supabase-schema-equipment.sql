...
--
-- Si tu as DÉJÀ exécuté une version précédente de ce fichier (contrainte zone limitée à
-- 'transmission'/'roues'/'cockpit'/'shoe') : n'exécute PAS ce fichier tel quel, utilise
-- plutôt supabase-migration-zones-2026-08.sql fourni à côté, qui fait la même chose sans
-- toucher aux données déjà en place.
...
--
--    Zones : 'transmission-avant' (pédalier/chaîne/pédales), 'transmission-arriere'
--    (cassette/dérailleur arrière), 'roues' (pneus/disques/plaquettes/roues), 'cockpit'
--    (cintre/ruban/durites/selle), 'shoe' (chaussures, une seule pièce générique).
create table if not exists public.equipment_components (
  id             uuid primary key default gen_random_uuid(),
  equipment_id   uuid not null references public.equipment(id) on delete cascade,
  zone           text not null check (zone in ('transmission-avant', 'transmission-arriere', 'roues', 'cockpit', 'shoe')),
